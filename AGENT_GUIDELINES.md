...
2. **Every other surface either derives it at read/build time or drops the number entirely** — prose says "see `plugin.json`", never restates "24 hooks". A doc file may still describe *what* a count means (e.g. the `type: command` vs `type: prompt` split), but should avoid re-asserting the raw total wherever practical.
