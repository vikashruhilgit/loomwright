...
All hooks are centralized in `hooks.json`. As of v14.34.0 there are **20 hook entries**: `SubagentStop` validators (worker, execute-manager, code-reviewer, supervisor-runner, qa-executor, plan-reviewer, **launch-pad-runner**) + 3 telemetry + 1 webhook `type: command` hooks; `Stop`; `TaskCompleted`; `WorktreeCreate`; `StopFailure`; **`PreToolUse[AskUserQuestion]`**; **`Notification`**; **`SessionStart`**; and **`PostToolUse[Bash]`** (the v14.34.0 `gh pr create` until-mergeable review-drain backstop). CODE_REVIEW_RESULT is validated at `schema_version: 3`. **The authoritative, always-current hook table lives in the root `CLAUDE.md` §"Plugin Hooks (Quality Gates)"** — this guide deliberately does not duplicate it, because that duplicate silently drifted from v10 (9 hooks) to v14 (19 hooks).
