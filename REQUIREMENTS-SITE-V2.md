...
| 4 | **Isolation with explicit lanes** | git worktrees per worker, ordered merges, a `PostToolUse` worktree observer + read-only orphan reader (`worktree-audit.sh`, v15.66.0 — the earlier `WorktreeCreate`/`WorktreeRemove` hooks were removed because a `WorktreeCreate` hook replaces the harness's own worktree creation), overlap detection that fails closed in CI | `loomwright/scripts/worktree-audit.sh`, `loomwright/hooks/hooks.json` |
