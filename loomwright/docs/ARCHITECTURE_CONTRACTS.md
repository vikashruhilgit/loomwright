...
| Execute Manager | internal | Supervisor (Phase 3) | Owns poll loop in isolated context, 60 tool-call budget; re-verifies each worker's `provides` ON DISK via `scripts/verify-provides.sh` (+1 Bash per subtask) — disk wins over the worker's self-report |
...
| Worker | internal | Execute Manager / Supervisor | Above threshold: one subtask per worktree. Single-Agent Path (below threshold, the default): one worker executes ALL acceptance criteria in the project root, no worktree. No git ops either way, emits WORKER_RESULT + `.worker-summary.md`; self-verifies its `provides` via the same `scripts/verify-provides.sh` the consumer re-runs. Optionally records `deviations` — bounded plan-drift entries (`plan:`/`edge:`/`open:`/`test:`) that survive worktree removal in `state.md`'s `## Worker Results` and reach the Phase 4.5 review lens as an advisory (`skills/self-heal-advisory/SKILL.md` step 1g) |
...
- **Command hooks:** the default (33 of 36 — the other three are the prompt hooks named above). Deterministic, zero model tokens. Every one is **exit-0-by-contract** — it signals via stdout JSON, never via exit status — which is what makes the `|| true` convention safe. The validators' stdout JSON is the documented command-hook decision (`{}` allow / `{"decision": "block", "reason": …}` block), NOT the prompt-hook `{"ok": …}` response schema — the latter is ignored by the runtime when a command hook prints it (probed 2026-09-21; `HOOKS.md` §"Command-validator decision shape"). A *blocking* gate must therefore never be written as a `type: command` hook carrying `|| true`.
...
| `.supervisor/jobs/done/` | Supervisor (move from in-progress); `scripts/reconcile-jobs.sh --repair` (v15.39.0); `session-resume.sh` via `reconcile-jobs.sh --repair-merged` at every SessionStart for a merge the disk proves (v15.84.0) | Read-only after move. Committed to version control once `/setup memory` is applied (v15.84.0: `done/` and `failed/` are part of the judgement trail; `pending/` and `in-progress/` stay ignored). `scripts/stamp-requirement-status.sh` READS this dir exclusively to close out the source requirement — which is why a brief stranded in `in-progress/` structurally blocks that reconciler too. It reads the pointer through the same shared `scripts/brief-pointer.sh` extractor and runs the same containment guards, so the two reconcilers cannot disagree about which requirement a brief names |
...
| `agent-memory-proposals/` | consumed | surprise-only proposals written by agents under the `AGENT_GUIDELINES.md` proposal contract; `/dreaming` surfaces each as `PROMOTE PENDING PROPOSAL` (`write-agent-memory.sh --proposal` on Accept), `harvest-conventions.sh` reads the queue as corpus | never (a proposal leaves the queue only by `/dreaming` Accept or Reject) |
| `orientation-proposals/` | consumed | orientation memos proposed by the Supervisor Phase 4.5 completion tail (`skills/self-heal-advisory` §2.6); `/dreaming` promotes each via `add-orientation.sh` | never (a proposal leaves the queue only by `/dreaming` Accept or Reject) |
| `capability/` | consumed | `/capability-check --save` reports; no runtime reader — retained as provenance | never |
...
| Fail-safe | The writer is a safe no-op (exit 0) when no sha256 tool is available; the reader emits nothing and exits 0 in the same case. Writes are atomic (temp + same-filesystem rename inside the twin dir), bounded by a contract-file cap (`SYSTEM_TWIN_MAX_CONTRACTS`, default 200) with write-time eviction, and de-duplicate identical, **ledger-trusted** contract bodies. The trust qualifier is load-bearing: an unchanged body whose hash has no chain-valid `add` is NOT deduped — it re-provenances instead, because a writer that treats an out-of-band-edited file as a no-op can never repair its own store (2026-09-01 dark-store incident). |
...
The loop **NEVER merges a PR** (the only executor is the `/automate` gate in `skills/automate-loop/SKILL.md` §10, whose condition 1 needs this loop's `READY` and whose condition 6 — `scripts/classify-risk.sh` high-risk, no override — is decided by the gate itself, never here) — its terminal `decision` values are `PASS` (clean diff, PR left open for a human), `ESCALATED` (findings posted + best-effort notify, PR left open), and — under `--until-mergeable` only — `READY` (merge-identical to `PASS`/`ESCALATED`; distinguished in the run record by `termination_reason: converged | bound_hit | sub_floor_converged | ci_untrusted`, none of which are auto-merge-eligible for `sub_floor_converged`/`ci_untrusted` — see `automate-loop/SKILL.md` §10 condition 1). `ci_untrusted` (ci-trust-probe-01) is ESCALATED-only, never READY — `scripts/ci-run-probe.sh` classified the only remaining READY-blocking required check(s) `untrusted_infra` (a narrow, evidence-based live-infra probe; never a timestamp). Authoritative loop semantics live in `skills/review-heal/SKILL.md`; the `REVIEW_HEAL_RESULT` block is defined in `docs/RESULT_SCHEMAS.md`.
...
| Dispatch shape | Launches a brand-new detached **headless** `claude -p --permission-mode dontAsk --allowedTools <pinned list> --disallowedTools WebFetch,WebSearch --agent loomwright:review-pr-runner <pr-url>` operating-system process — a true fresh session where the runner is the main agent and can therefore spawn `code-reviewer` / `general-purpose` children (sidesteps the subagents-cannot-spawn-subagents limit). `-p`/`--print` is required: `--agent` only selects the agent, it does NOT imply headless, and the no-`-p` interactive form can hang when detached with no TTY (mirrors `dispatch-pr-postmortem.sh`). **Permission regime is PINNED, never inherited (red-team-hardening item 01, v15.86.0+):** `--permission-mode dontAsk` + an explicit `--allowedTools` allowlist (Read, Grep, Glob, Task, the scoped `git`/`gh` `Bash(...)` entries, plus any doubly-declared test/lint runner) + `--disallowedTools WebFetch,WebSearch`, constructed by ONE shared function both the `DRY_RUN_DISPATCH` diagnostic and the real launch read. A pre-dispatch **regime probe** resolves the ambient `permissions.defaultMode` (project local > project shared > user settings); a permissive/unreadable resolution with no `LOOMWRIGHT_DRAIN_ALLOW_PERMISSIVE=1` opt-out (process-env only, never named in any prompt file) degrades the dispatch to review-only (`LOOMWRIGHT_PR_IS_FORK=1` semantics) and records `regime=permissive_refused`. If this `claude` build's `--help` cannot pin a headless allowlist-honouring mode, the dispatcher exits 0 with `PERMISSION_REGIME_UNPINNABLE` and does NOT dispatch — refuse, don't guess (decision R1). Full contract: `scripts/dispatch-pr-review.sh` header PERMISSIONS section. |
...
| Death detection + bounded re-dispatch (red-team-hardening item 04, v15.88.0+) | The wrapper's `trap cleanup EXIT` checks, AFTER the runner process exits, `grep -q 'REVIEW_HEAL_RESULT' "$_log"` — the SOLE mechanism deciding whether a run completed vs silently died (a scoped-check wait `wait-for-checks.sh` the model backgrounded, then ended its turn on — under `claude -p` that IS process exit). Absent ⇒ writes `.supervisor/review-dispatch/<hash>.died` (tab-separated `ts`/`pr_url`/`exit_code`/`last_log_line`/`attempt` via `printf`), appends `DRAIN_DIED` to the run log, fires `notify-desktop.sh` + `send-webhook.sh --gate-type drain_died` best-effort. The dispatcher's own `①` marker guard then treats `<hash>` + `<hash>.died` (attempt 1) TOGETHER as "not dispatched" for exactly ONE automatic re-dispatch by whichever caller invokes it again; a SECOND death (`.died` `attempt=2`) makes that SAME guard treat the pair as genuinely dispatched/exhausted, refusing a third — bounded, never unbounded. Surfaced at `session-resume.sh` ("Drains that died without a result"), `status-line.sh` (`⚠ drain died`), and `/automate`'s RECONCILE (`skills/automate-loop/SKILL.md` §4, `owned_drain_result: died` / `pause_reason: drain_died`, never `awaiting_merge`). Test coverage + the BLOCKING mutation control proving the grep is load-bearing: `scripts/test-dispatch-pr-review.sh` cases 37–42. |
...
| Bound | Value | Rationale |
|-------|-------|-----------|
| Tool-call budget | ≤ 6 tool calls | Hard ceiling for the whole gate (`git log`, `gh pr list` + per-PR file listing, classification reads, the cited-line premise check). |
| Marginal cost (common path) | ~2–3 tool calls | Reuses the `git fetch origin "$BASE_BRANCH"` already performed in Phase 1 ACQUIRE, so the CLEAR path adds little; the `unverified` / `--skip-preflight-sync` paths cost less. **Unchanged when the goal/requirement cites no `path:line` refs** (signal (d) below has nothing to check). |
...
| Signal (d) — cited-line premise check (harness-port/03) | +1 tool call, ONLY when refs exist and budget allows | ADVISORY ONLY (`skills/preflight-sync/SKILL.md` signal (d)); runs LAST, after (a)/(b)/the corroboration control/(c); ONE batched Bash call regardless of ref count; skipped silently (`premise_check=skipped_budget`) when fewer than 2 calls of the ≤6 cap remain (i.e. 5 or more already used). Can never cause `preflight_sync = unverified` and never counted against the common-path figure above when 0 refs are cited. |
...
- `.supervisor/salvage/` is owned by `worktree-salvage.sh` (v15.67.0) — in the DISPATCHING checkout's `.supervisor/` for the drain (passed as `--dest`, beside the per-PR marker), in the true primary checkout (the parent of `git rev-parse --git-common-dir`, the script's default) for FINALIZE: before every plugin-owned removal — the review drain's three `--force` sites in `dispatch-pr-review.sh` and FINALIZE step 4 — it writes `<basename>-<ts>/` there (`BASE_SHA`, `tracked.patch`, `modified/`, `untracked/`, `README.md`) only when the worktree held uncommitted content; gitignored runtime state is never captured; a salvage failure never blocks the removal; retention/pruning of that directory is reconciler-repair item 06's, not this bullet's
...
| `review-pr`         | 39187 | 35873¹ | 2 |
| `rubric-grader`     | 2330  | 1896  | 0 |
| `supervisor`        | 27488 | 26181¹ | 0 |
| `worker`            | 8510  | 4158  | 0 |
...
>
> **Raise log (harness-port/01-worker-rules, ONE bundled `worker.md` edit per decision H1):** `worker` 7459 → 8510 — the three new Critical Rules bullets (no-self-promotion, shared-local-services-read-only), the new Inputs bullet (Subtask ordinal), the new Step 5.75 honest-limits section, and the new `not_verified` Output Format line + invariant bullet moved measured 6781 → 7736 (live `bash scripts/check-token-budget.sh` re-measure taken immediately before this raise), breaching 7459 by 277; restored to measured + ~10% per the initial-budget convention. `worker` stays an unmarked, frozen-baseline row (`measured` cell/field stays 4158) per its row convention. **Also re-measured, no raise:** the same PR's `subtask_ordinal` addition — both the preloaded `async-orchestration/SKILL.md` prose AND the corresponding `Subtask ordinal: {N}` field actually added to `agents/execute-manager.md` Step 3's own worker `Task()` prompt (Phase 4.5 review round 1 finding: the SKILL.md prose originally claimed Step 3 carried this field when the prompt string had not yet been edited to include it) — moved `execute-manager`'s live weight to 36310 against the unchanged 38646 budget (2336 headroom, no breach) — not re-raised, per the anti-treadmill rule.
>
> **Re-measure log (no raise — PR #233, 2026-09-17, orca-03 worker checkpoints):** the new "Session-log pointer" input bullet plus the advisory checkpoint guidance added to `agents/worker.md` moved its live weight to 5876 against the unchanged 5905 budget (29 headroom, no breach, per `bash scripts/check-token-budget.sh`). `worker` stays an unmarked, frozen-baseline row (`measured` cell/field stays 4158) per its row convention; the live figure is recorded in the JSON `note` only, not in this table.
>
> **Raise log (v15.80.0, completion authority):** `execute-manager` 34860 → 38646 — the new terminal-lifecycle-event join in the v12 outputs_verified gate (`scripts/check-children-settled.sh --agent-id`, the `provides_present_agent_unsettled`/`unverifiable` branches, and the `ended_without_result` surfacing) moved measured 33300 → 35132, breaching by 272; restored to measured + ~10% per the initial-budget convention. `supervisor` 24900 → 27488 — the Single-Agent Path step 3b + Sequential Path terminal-lifecycle-event join, the FINALIZE Point-5 children-settled gate, and the `--skip-children-check` flag rows moved measured 23300 → 24989, breaching by 89; restored to measured + ~10% per the same convention. Full reasoning in each agent's `note` in `prompt-token-budgets.json`.
>
> **Raise log:** `execute-manager` 31400 → 34860 — v15.67.0 (salvage before removal) adds one `bash scripts/worktree-salvage.sh …` line above each `git worktree remove` in the preloaded `async-orchestration/SKILL.md`'s three cleanup/FINALIZE blocks plus the one-sentence salvage-before-remove rule under FINALIZE step 4 (~308 proxy tokens). The live measure before the change was ~31383 against 31400 — **17 proxy tokens of headroom**, unrelated growth since the 29197 end-of-v15.20.0 re-measure — so the required prose breached by 291. Restored to the live re-measure 31691 + ~10% per the initial-budget convention. Re-measured at the end of the same PR (review iteration 1 reworded the preloaded `async-orchestration/SKILL.md` provenance note): 31691 → 31701, so the `measured` cell/field now carries 31701 (this is a LIVE re-measure row, footnote 1); budget stays 34860 — not re-raised to chase in-PR drift, per the anti-treadmill rule above.

> **Re-measure log (no raise — v15.71.0, verify-provides.sh consumer gate, 2026-09-13):** the three ¹-marked rows this PR grew are set to the live figure the gate prints: `execute-manager` 31701 → 33300 (the §"v12 outputs_verified gate" disk re-check + Step 2b pointer; 1560 headroom), `orchestrator` 8688 → 8924 (mirror phrasing; the field already lagged origin/main's 8893 before this PR; 634 headroom), `supervisor` 22606 → 23300 (Single-Agent / Sequential gate re-runs; already lagged origin/main's 22819; 1600 headroom). No budget moved — none breached, and re-raising to chase in-PR drift is the treadmill the anti-treadmill rule forbids. `worker` stays the frozen 4158 by its row convention; its live 2026-09-13 figure (5569 / 5905) is recorded in the JSON note only.
>
...
>
> **Raise log (ci-trust-probe-01, harness-port/07):** `review-pr` 34614 → 39187 — the preloaded `review-heal` skill gained the CI trust probe wiring (`checks_untrusted[]`/`ci_untrusted` §U4 state vars and probe-and-classify block, the new `ci_untrusted` READY-blocker/ESCALATED branches, the READY-redefinition sentence, the Terminal-states bullet, and two new Pinned Canonical Names rows) plus `agents/review-pr.md`'s matching outcome-model bullet — a real new contract (a new fail-safe live-infra probe script + its drain wiring), not bloat; measured 33893 → 35624 mid-PR, breaching by 1010. Restored the same ~10% margin per the raise rule (ceil(35624×1.10)=39187). **Re-measured again (no further raise) after PR #266 review round 1 fixes** (replacing the free-text job/step-name pattern match with the structured `startup_failure` conclusion signal, plus the `detailsUrl`→`run_id` derivation line): measured moved 35624 → 35873 (+249), still 3314 proxy tokens of headroom against the unchanged 39187 budget — the table cell above reflects this live end-of-PR figure.
...
> **Raise log (worker-deviations item, six-phase-loop-gaps/01):** `worker` 5905 → 7314 — the new Step 5.7 "Optional — record deviations from plan" section (four `plan:`/`edge:`/`open:`/`test:` kinds with one example each, the ≤12/≤200 bound, the transient-vs-durable split against `memory_candidates`, and the byte-for-byte `.worker-summary.md` echo rule), the new `deviations` output-block line placed directly after `memory_candidates`, and the new invariant-block sentence moved measured 5876 → 6649, breaching the 5905 budget by 744; restored to measured + ~10% (6649 × 1.10 = 7313.9, rounded up to 7314) per the initial-budget convention — a real new contract (a new WORKER_RESULT field with its own prompt-contract prose), not drift. `worker` stays an unmarked, frozen-baseline row (`measured` cell/field stays 4158) per its row convention; the live figure is recorded in the JSON `note` only, not in this table.

> **Raise log (test-integrity-guard item, six-phase-loop-gaps/02):** `worker` 7314 → 7459 — Step 5's new "Run tests" sub-bullet (the diagnose-before-fix rule of explanation: a failing test's assertion edit needs a `test: <name> — <code wrong | assertion wrong | env>: <why>` deviations entry, mirroring the FIX_RESULT conditional-mandatory wording item 01 already landed) moved measured 6649 → 6781, leaving 533 proxy tokens of headroom against the 7314 budget (no breach); raised anyway per this item's own explicit instruction to restore the full margin rather than merely stay under budget. Restored to measured + ~10% (6781 × 1.10 = 7459.1, rounded up to 7459) per the initial-budget convention. `worker` stays an unmarked, frozen-baseline row (`measured` cell/field stays 4158) per its row convention; the live figure is recorded in the JSON `note` only, not in this table.
>
> **Re-measure log (no raise — harness-port/03, cited-line-premise-check, 2026-09-24):** `launch-pad` — the new Phase 3 action 10 (cited-line premise check: reference extraction, resolution, judgment, and the honest `find -mmin`-only age column) plus Phase 5 action 4b's STALE-row risk carry-over moved measured 38111 → 39676, leaving 2247 proxy tokens of headroom against the unchanged 41923 budget (no breach) — not raised, per the anti-treadmill rule. Re-measured again after fixing a `.git/FETCH_HEAD`-portability bug found by direct verification (a git worktree's `.git` is a pointer FILE, not a directory, so a literal `.git/FETCH_HEAD` path errors instead of reporting absent — fixed to resolve via `git rev-parse --git-common-dir`): measured 39676 → 39784, 2139 headroom (no breach). Re-measured a third time (drain round 1, `claude-review` finding) after fixing a real path-resolution bug — the suffix-match fallback was gated on "no `/` in path" instead of "direct `git show` failed," which defeated resolution on this repo's own dominant citation convention of omitting the `loomwright/` prefix — plus tightening the honest-age existence-check ordering: measured 39784 → 40026, 1897 headroom (no breach). Re-measured a fourth time (drain round 2, `claude-review` finding) after clarifying STALE-row multiplicity — one Phase 3 table row + one Phase 5 Risk Assessment row PER STALE ref, never combined — in both the action 10 emission rule and Phase 5 action 4b: measured 40026 → 40081, 1842 headroom (no breach), taken deliberately after the last prose edit per the `supervisor` v15.19.0 end-of-PR re-measure precedent. This is a LIVE re-measure row (footnote 1); the table cell above and the JSON `measured` field both carry the final figure. `plan-reviewer` — Criterion 1's new one-sentence cited-line-premise sub-check moved measured 9132 → 9214, leaving 832 proxy tokens of headroom against the unchanged 10046 budget (no breach) — not raised. Re-measured again (drain round 2, `claude-review` finding) after renaming the sub-check's issue category from `file_path` to the new canonical `cited_line_premise` (a STALE ref names an EXISTING file whose content moved, distinct from `file_path`'s missing-file meaning) and clarifying one-finding-per-STALE-ref multiplicity: measured 9214 → 9293, 753 headroom (no breach) — not raised. `plan-reviewer` stays an unmarked, frozen-baseline row (`measured` cell/field stays 6392) per its row convention; the live figure is recorded in the JSON `note` only, not in this table. Full reasoning in each agent's `note` in `prompt-token-budgets.json`.

> **Re-measure log (no raise — harness-port/06, shared-local-services, 2026-09-24):** `execute-manager` — the preloaded `async-orchestration/SKILL.md` gained the `Shared local services: {line}` field and this agent's own Step 3 `Task()` prompt gained the matching paste plus field description, moving measured 35132 → 37027, leaving 1619 proxy tokens of headroom against the unchanged 38646 budget (no breach) — not raised, per the anti-treadmill rule. This is a LIVE re-measure row (footnote 1); the table cell above and the JSON `measured` field both carry the final figure. (Drain-review finding: the PR's CHANGELOG initially claimed this re-measure while the actual JSON/table mirror had not landed — fixed by this update.)
...
| phase45-multi-voter-verification (code-reviewer + red-team-reviewer voter) | inherit | **sonnet** (both voter spawns follow the code-reviewer override; the refute spawn too; **N/A when `--voter-provider` routes the voter through `lens-run.sh`** — that path is a Bash shell-out, not a Task spawn, so `--cheap` cannot set `model:` and the model comes from the `name:model` suffix) |
...
Makes single-run-per-repo **structural**, not merely documented (`automate-loop/SKILL.md` §11 used to read "assumed constraint, not enforced" — no longer). Lock is `.supervisor/run.lock` (a directory — `mkdir` atomicity, no `flock` dependency), with a `meta` TSV file (`pid`, `owner`, `session_id`, `ts`, `pid_source` — NOT JSON, matching `dispatch-pr-review.sh`'s `write_lock_meta` convention). `pid` is the **holder's liveness pid**, resolved in order and recorded as `pid_source`: `claude_pid` — `$CLAUDE_PID` when set and alive (observed on Claude Code 2.1.281 as the Bash-tool shell's parent, the `claude` binary; undocumented, so never the sole defence); `ancestor` — under Claude Code (`CLAUDECODE` set), the first non-shell ancestor of the script, i.e. `claude` again; `ppid` — otherwise the invoking shell (a terminal, CI, a test harness; under Claude Code this source means the lock has degraded to the TTL). Never the script's own `$$`, and never the Bash-tool shell, both of which exit the moment `acquire` returns (the pre-fix recording made every lock look dead-pid immediately, degrading it to a bare 1800s lease that a later different-owner acquire reclaimed — observed in `/automate` run `automate-2026-09-26-115755`). Reclaim requires BOTH the recorded pid dead AND age ≥ 1800s (mirrors `dispatch-pr-review.sh`'s `acquire_lock` TTL contract exactly — a dead pid alone, or a fresh lock alone, both refuse).
...
- **Re-entrant for the SAME session (nested entry).** `/automate` RUN inlines `/autonomous`, which inlines `/supervisor` — all in one session, each acquiring with its own owner label. When the lock is held and `--session-id` equals the recorded `session_id`, `acquire` exits 0, prints `run_lock_reentrant owner=<outer> session_id=<id>`, and **keeps the outer owner** (re-stamping `pid`/`ts`). The inner run's own `release --owner <inner>` therefore matches nothing and is a no-op; the lock is released by the outermost run. An empty `--session-id` never re-enters, and a different session is refused while the holder pid lives, regardless of age.
...
> `loomwright/scripts/gen-color-legend.sh` from each agent's frontmatter (`name:` minus the
> `loomwright:` prefix, `color:` verbatim, and the colour word looked up from the published CSS
> named-colour table keyed by hex). Regenerate with
> `bash loomwright/scripts/gen-color-legend.sh --write`; `scripts/check-doc-currency.sh` runs the
> same generator in `--check` mode and fails CI closed on any drift. A hex with no CSS name
> renders `—` rather than a guess. This replaces the hand-maintained table that drifted to 12
> rows while 14 agents declared a colour.
>
> **Naming convention differs from every other agent table here, deliberately.** The `Agent`
> column carries the frontmatter identifier (`launch-pad-runner`, `supervisor-runner`,
> `review-pr-runner`), not the display name used by CLAUDE.md §"The 14 Agent Roles" and
> `README.md` (`Launch Pad`, `Supervisor`, `Review-PR`). Cross-reference by dropping the
> `-runner` suffix. A display-name column would need a hand-maintained map keyed on the agent
> set — the exact thing whose drift this table was generated to end.
...
| review-pr-runner | Dark Turquoise | `#00CED1` |
| rubric-grader | Yellow Green | `#9ACD32` |
| supervisor-runner | Dodger Blue | `#1E90FF` |
...
**Core/adapter architecture rule** (personal-memory `portability-core-vs-adapter`, in the operator's own cross-project memory store, outside this repo): this plugin's CORE — agents, skills, commands, and the runtime scripts under `loomwright/scripts/` that a non-Claude harness would have to run unmodified — must stay **vendor-neutral**. Loomwright is meant to run under Cursor/Codex too, not just Claude Code. Any integration with a THIRD-PARTY tool that is specific to how that tool works (its own CLI, its own JSON shapes, its own invocation conventions) belongs in an **ADAPTER**, never in core. `scripts/check-vendor-coupling.sh` (see the `CORE`/`COUPLED`/`ADAPTER` classification in `loomwright/docs/vendor-coupling-manifest.json`) already ratchets literal *Claude Code* vendor tokens the same way; adapters for OTHER third-party tools are the same principle applied one level further out — the adapter directory below is where they live so core never has to name them.

**`loomwright/scripts/adapters/`** is the directory for these third-party-tool adapters. Each adapter gets its own subdirectory (`loomwright/scripts/adapters/<tool>/`), is entirely opt-in (the third-party tool is runtime-detected, never a build/test dependency), and is fail-safe (never blocks, never fails the caller, degrades to a silent no-op when the tool is absent or misbehaving). Nothing under `agents/`, `skills/`, or `commands/` may name a third-party adapter tool — that boundary is what keeps core portable, and it is the thing every adapter's own test suite is expected to assert (see the Orca example below).

**First example: the Orca adapter** (`loomwright/scripts/adapters/orca/orca-mirror.sh` + `test-orca-mirror.sh`, source requirement `.supervisor/requirements/orca-derived/06-orca-adapter.md`). [Orca](https://onorca.dev) is a separate CLI/orchestration tool this plugin happens to run unmodified inside — real `claude` PTY, `~/.claude` picked up, repo hooks run — but which has none of this plugin's own moat (rules, rubric, churn ledger, lessons, review lenses) and all of the undifferentiated worktree/orchestration shell. `orca-mirror.sh` mirrors this plugin's OWN recorded lifecycle/checkpoint events (`phase_transition`, `pr_created`, `worker_checkpoint`, `session_end`) into Orca's own CLI, ONLY when `orca` happens to be on `PATH` and its `orca status --json` probe succeeds — never a dependency, and `orca` is confirmed NOT installed on this project's own dev machine (2026-09-11), so the adapter's whole test suite (`test-orca-mirror.sh`) runs against a STUB `orca` placed on `PATH` that records its own argv.

**The probe-once contract, stated explicitly (any future adapter of this shape should follow it):** an adapter script invoked once per event (not a long-running daemon) must not re-probe the third-party tool's liveness/configuration on every single invocation — that would mean one subprocess per event for a signal that rarely changes mid-run. `orca-mirror.sh`'s probe (`orca status --json`) is cached to a file whose path the CALLER can scope per run via an env var (`LOOMWRIGHT_ORCA_STATUS_CACHE`), so every mirror invocation within one Supervisor run shares a single cached probe result; an unset env var falls back to a bounded-TTL machine-wide cache file rather than either re-probing every call or freezing a stale verdict forever. `test-orca-mirror.sh` proves this is load-bearing with a mutation control: breaking the cache-read gate on a copy of the script turns "1 probe across N invocations" into "N probes across N invocations."

**Known false-positive in the literal core-cleanliness grep:** the mechanical check for "core stays free of a named adapter tool" is `grep -rl <tool> loomwright/ --exclude-dir=adapters` returning only documentation. For Orca specifically, this grep ALSO matches every file that cites its own SOURCE REQUIREMENT path — the requirements queue several already-shipped items (agent-lifecycle-ledger, completion-authority, worker-checkpoints, rate-limit-park) were derived from is itself named `.supervisor/requirements/orca-derived/`, and citing that path in a header comment (e.g. `orca-derived/03-worker-checkpoints.md`) or a budget-table key (`orca-03`) is a legitimate, unrelated provenance citation, not adapter coupling. `test-orca-mirror.sh`'s core-cleanliness case accounts for this explicitly: it excludes the literal substrings `orca-derived` and `orca-03` before judging whether a non-doc file still names the real Orca CLI, rather than either loosening the grep silently or papering over the collision.

**Second example: the provider-lens adapter** (`loomwright/scripts/adapters/providers/lens-run.sh` + one `provider-<name>.sh` table entry per supported CLI — `claude`, `cursor`, `codex`, `gemini` as of this writing — plus `test-lens-run.sh`; source requirement `.supervisor/requirements/orca-derived/07-provider-lens.md`). Where the Orca adapter mirrors THIS plugin's lifecycle out to a third-party orchestration tool, `lens-run.sh` runs a third-party agent CLI (Anthropic's own `claude -p`, or `cursor-agent`/`codex`/`gemini` where installed) as a second-opinion REVIEW LENS and normalizes its output into the `issues[]` shape `docs/RESULT_SCHEMAS.md`'s multi-voter merge rule already consumes — see `skills/self-heal-advisory/SKILL.md` §"Multi-voter verification" for the call site (a non-default `--voter-provider`/`.voter_provider` invokes it via Bash instead of spawning the `loomwright:red-team-reviewer` Task; default behavior is unchanged). It runs the target CLI inside a throwaway independent sandbox repo (deliberately never `git worktree add` — a worktree shares the parent repository's `.git/config`, so `git remote remove origin` inside one removes origin for the ENTIRE shared repo, not just the sandbox; see `lens-run.sh`'s own SAFETY POSTURE header comment for the full incident writeup), populated by a shallow `file://` fetch of the reviewed tip only (`git clone --local` / local-path clones silently ignore `--depth`, which would otherwise copy full history of every local ref). Origin removal is fail-closed (the CLI is never started if `git remote remove origin` fails or any remote remains). A post-run integrity check covers the working tree *and* security-relevant `.git` internals (remotes, local config, refs/HEAD, non-sample hooks) — `git status --porcelain` alone is blind to `.git/` and was empirically bypassed by planting a hook / adding a remote. The child environment is `env -i` (no ambient credential, including `GH_TOKEN`, survives). A wall-clock bound kills the CLI process group on expiry. **Honesty required by the source requirement:** this is DETECTION of sandbox mutation plus blast-radius reduction, not a confirmed deny-by-default sandbox — never call it "enforced". A CLI with bash/tool access can still write OUTSIDE the sandbox (the main checkout, `~/.claude`) and can still `git push <explicit-url>` (removing `origin` is not "nowhere to go"; there is no network-egress denial). The reviewed diff and caller prompt are folded verbatim into the instruction stream of a fully agentic CLI — prompt injection via reviewed content is a distinct UNMITIGATED residual risk, and deny-by-default tool-permission behavior is UNVERIFIED/DEFERRED for providers whose live headless mode has not been probed. The `cursor-agent -f`/deny-by-default probe named in the source requirement's Mechanism section is DEFERRED (installed but not authenticated on this dev machine, 2026-09-18).
