...
| Signal (d) — cited-line premise check (harness-port/03) | +1 tool call, ONLY when refs exist and budget allows | ADVISORY ONLY (`skills/preflight-sync/SKILL.md` signal (d)); runs LAST, after (a)/(b)/the corroboration control/(c); ONE batched Bash call regardless of ref count; skipped silently (`premise_check=skipped_budget`) when fewer than 1 call of the ≤6 cap remains. Can never cause `preflight_sync = unverified` and never counted against the common-path figure above when 0 refs are cited. |
