...
| `context-keeper`    | 3483  | 3166¹ | 0 |
| `execute-manager`   | 28550 | 26357¹ | 3 |
| `launch-pad`        | 34478 | 31343 | 7 |
| `orchestrator`      | 9558  | 7877  | 1 |
...
**`measured` and the "Preloaded skills" column are frozen, decorative authoring-time metadata, not enforced values, EXCEPT the three ¹-marked v15.16.0 rows above** — the gate compares live weight only against `budget`; the `measured` column here and in the JSON are authoring snapshots (v15.10.0, or the noted v15.16.0 re-measure for `context-keeper`/`execute-manager`/`supervisor`) that the gate never re-derives, and the preloaded-skill counts (also in each JSON `note`) are decorative (the gate computes the live count per run but does not assert it against them). The proxy also sizes the whole `.md`/`SKILL.md` **including frontmatter** — a deliberate, safe over-count for a fail-closed ratchet, not the literal injected system prompt. Do **not** chase or re-bump them each release (same spirit as CLAUDE.md's frozen-example-value convention) — the v15.16.0 rows are a one-time, explicitly-scoped exception tied to a specific prompt-deletion change, not a new routine. **When you raise a budget in `prompt-token-budgets.json`, update this table row in the same edit** — this is machine-enforced: `check-token-budget.sh` asserts every budget cell in this table equals its `.agents[<stem>].budget` in the JSON (drifted, missing, or ghost rows fail CI closed).
