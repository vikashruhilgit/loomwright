...
>
> **Raise log (drain-bounding-earned-checks, v15.22.0):** `review-pr` 27955 → 31453 — the preloaded `review-heal` skill's §U4 loop body grew the mechanized shared drain bound (`scripts/drain-rounds.sh` ledger calls replacing the prose-only `rounds`/`while rounds < max_rounds` counter, AC1/AC2) plus the termination-only severity floor (`--severity-floor`, the `sub_floor_converged` terminal state, the SHA-bound confirming required-check pass and its full pseudocode, AC9/AC11/AC12/AC13) — a real new contract, not bloat; measured 25413 → 28593 (+3180), breaching by 638. Restored the same ~10% authoring margin per the raise rule.
