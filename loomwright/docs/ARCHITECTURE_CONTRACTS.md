...
>
> **Raise log (ci-trust-probe-01, harness-port/07):** `review-pr` 34614 → 39187 — the preloaded `review-heal` skill gained the CI trust probe wiring (`checks_untrusted[]`/`ci_untrusted` §U4 state vars and probe-and-classify block, the new `ci_untrusted` READY-blocker/ESCALATED branches, the READY-redefinition sentence, the Terminal-states bullet, and two new Pinned Canonical Names rows) plus `agents/review-pr.md`'s matching outcome-model bullet — a real new contract (a new fail-safe live-infra probe script + its drain wiring), not bloat; measured 33893 → 35624, breaching by 1010. Restored the same ~10% margin per the raise rule (ceil(35624×1.10)=39187); measured set to the live re-measure taken after this item's full doc/schema/test wiring landed (3563 headroom).
