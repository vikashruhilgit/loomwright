...
**Second example: the provider-lens adapter** (`loomwright/scripts/adapters/providers/lens-run.sh` + one `provider-<name>.sh` table entry per supported CLI — `claude`, `cursor`, `codex`, `gemini` as of this writing — plus `test-lens-run.sh`; source requirement `.supervisor/requirements/orca-derived/07-provider-lens.md`). Where the Orca adapter mirrors THIS plugin's lifecycle out to a third-party orchestration tool, `lens-run.sh` runs a third-party agent CLI (Anthropic's own `claude -p`, or `cursor-agent`/`codex`/`gemini` where installed) as a second-opinion REVIEW LENS and normalizes its output into the `issues[]` shape `docs/RESULT_SCHEMAS.md`'s multi-voter merge rule already consumes — see `skills/self-heal-advisory/SKILL.md` §"Multi-voter verification" for the call site (a non-default `--voter-provider`/`.voter_provider` invokes it via Bash instead of spawning the `loomwright:red-team-reviewer` Task; default behavior is unchanged). It runs the target CLI inside a throwaway `git clone --local` sandbox (an ISOLATED local clone, deliberately never `git worktree add` — a worktree shares the parent repository's `.git/config`, so `git remote remove origin` inside one removes origin for the ENTIRE shared repo, not just the sandbox; see `lens-run.sh`'s own SAFETY POSTURE header comment for the full incident writeup) with a scrubbed `env -i` child environment (no ambient credential, including `GH_TOKEN`, survives) and a post-run `git status --porcelain` mutation check (`lens_mutated_tree` — result discarded, sandbox removed either way). **Honesty required by the source requirement:** this is DETECTION of sandbox mutation, not a confirmed deny-by-default sandbox — a CLI with bash/tool access can still write OUTSIDE the sandbox clone (the main checkout, `~/.claude`, a push to some other remote), which is UNMITIGATED here. The `cursor-agent -f`/deny-by-default probe named in the source requirement's Mechanism section is DEFERRED (installed but not authenticated on this dev machine, 2026-09-18) — never call the current mitigations "enforced" elsewhere in this repo's docs/comments until that probe actually runs.
