...
Makes single-run-per-repo **structural**, not merely documented (`automate-loop/SKILL.md` §11 used to read "assumed constraint, not enforced" — no longer). Lock is `.supervisor/run.lock` (a directory — `mkdir` atomicity, no `flock` dependency), with a `meta` TSV file (`pid`, `owner`, `session_id`, `ts` — NOT JSON, matching `dispatch-pr-review.sh`'s `write_lock_meta` convention). `pid` is the **holder's liveness pid**: `$CLAUDE_PID` (the session's long-lived `claude` process, exported into every Bash-tool shell) when set and alive, else the invoking shell's `$PPID` — never the script's own `$$`, and never the Bash-tool shell, both of which exit the moment `acquire` returns (the pre-fix recording made every lock look dead-pid immediately, degrading it to a bare 1800s lease that a later different-owner acquire reclaimed — observed in `/automate` run `automate-2026-09-26-115755`). Reclaim requires BOTH the recorded pid dead AND age ≥ 1800s (mirrors `dispatch-pr-review.sh`'s `acquire_lock` TTL contract exactly — a dead pid alone, or a fresh lock alone, both refuse).
