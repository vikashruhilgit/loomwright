...
| Worker | internal | Execute Manager / Supervisor | Above threshold: one subtask per worktree. Single-Agent Path (below threshold, the default): one worker executes ALL acceptance criteria in the project root, no worktree. No git ops either way, emits WORKER_RESULT + `.worker-summary.md`; self-verifies its `provides` via the same `scripts/verify-provides.sh` the consumer re-runs |
