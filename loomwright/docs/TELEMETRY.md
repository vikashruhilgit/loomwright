...
**Rejected alternatives (four).** The first three are the spike's Unknown-B options; the fourth is
the source requirement's own recommendation, which the owner reversed.
...
| **(d)** Drop the QA fan-out entirely **and** retire Rubric C as dormant (the source requirement's recommendation) | Reversed by the owner on 2026-08-20. Dropping the fan-out silently deletes a live telemetry signal to save one matcher, and banner-marking Rubric C dormant would misreport a rubric that is still exercised. Both limbs are rejected: the fan-out is kept **and** Rubric C stays live. |

**Chosen:** keep the fan-out in loomwright, matched on the selvedge agent — the spike's option (d).
