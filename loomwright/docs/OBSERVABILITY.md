...
## Per-project auto-labeling (`service.name`)
...
> **One-session-lag caveat:** `settings.local.json` is read at session start, *before* this SessionStart hook writes it — so a brand-new repo's very first session may emit the prior (or absent) `service.name` value; the label takes effect from the NEXT session. The `/setup observability` init-tail invocation (which labels the current repo synchronously during init) and a best-effort `$CLAUDE_ENV_FILE` write cover the enabling session itself. Removal: `/setup observability remove` best-effort strips the CURRENT repo's label; other repos' auto-written labels are left in place but are INERT while telemetry is off.

> **Untracked-file caveat:** the auto-write targets `<project>/.claude/settings.local.json`, which is gitignored **by convention** (Claude Code's own default project scaffolding ignores it). In a consuming repo that does *not* gitignore that path, the auto-written file shows up as an untracked (and potentially committable) change. The written value is only the repo basename + plugin version — non-sensitive — but if you don't want the file appearing, add `.claude/settings.local.json` to that repo's `.gitignore` (or run `/setup observability remove` to strip the label and disable telemetry). The label is only written when telemetry is enabled.
...
The stack's compose project is always **`loomwright-observability`** (named volumes: `loomwright-observability_*`), pinned two ways: every documented command carries `-p loomwright-observability` explicitly, and the generated `.env` contains `COMPOSE_PROJECT_NAME=loomwright-observability` (compose v2 loads `.env` from the `-f` file's directory, so even a bare `docker compose -f … up -d` joins the same project). A command with neither would derive project `observability` from the directory basename and silently create a second parallel stack on fresh empty volumes — orphaning the existing traces and fighting over ports.
...
If you prefer to keep the collector non-root (remove the `user: "0:0"` override), pre-seed the volume ownership before first boot instead (the `-p` flag is load-bearing here — it makes the pre-seed act on the real stack's `loomwright-observability_otel_collector_queue` volume, not a freshly created `observability_*` one):

  ```bash
  docker compose -p loomwright-observability -f ~/.claude/loomwright/observability/docker-compose.yml run --rm --user 0:0 --entrypoint sh otel-collector -c 'chown -R 10001:10001 /var/lib/otelcol/file_storage'
  ```
