...
REVIEW_HEAL_RESULT:
  ...
  termination_reason: enum [converged, bound_hit, sub_floor_converged] | null   # OPTIONAL — WHY the drain terminated where it did. `converged`: a genuine empty-yield round (all channels re-scanned, nothing found). `bound_hit`: `--max-rounds` exhausted (ESCALATED). `sub_floor_converged`: a round whose entire FIXED yield was below `--severity-floor` terminated READY without a further all-channel re-scan (still ran the SHA-bound confirming required-check pass). Absent/null on other ESCALATED paths (fail-closed channel-unknown, human-judgment findings, unreadable round ledger) — NOT every exit sets this field.
