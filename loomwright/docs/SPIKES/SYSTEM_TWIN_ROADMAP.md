...
- **`cmd:` trust mitigation — content-keyed stamp gate shipped (red-team-hardening item 05, v15.90.0); no longer an interim valve.** The M2b slice 1a ground-truth runner executes arbitrary `bash -c` from a brief's `## Executable Acceptance` `cmd:` bullets, automatically and unattended in Phase 4.5 (incl. under `/autonomous`, where the brief is machine-authored). `--no-cmd` is still passed by Supervisor Phase 4.5 only when `NON_INTERACTIVE == true` (`--non-interactive-fallback`, not `/autonomous` alone — a plain interactive `/autonomous "goal"` run leaves it unset), so it was never sufficient coverage on its own — that gap is what item 05 closed: a `cmd:`/bare bullet sourced from `--brief` now executes ONLY when the brief's `## Configuration` carries a content-keyed sha256 stamp matching the CURRENT bullet list (`scripts/exec-acceptance-hash.sh` / `run-ground-truth.sh`; absent/stale stamp → `unverified`/`cmd_unapproved`, no execution). This is the prompt-level control this section previously described as owed:
  - [x] **Plan Reviewer check** — Criterion 14 (`agents/plan-reviewer.md`) now escalates to **NEEDS_HUMAN** (not merely a LOW advisory issue) when ≥1 `cmd:`/bare bullet has no valid stamp; a valid stamp keeps the existing LOW/visibility-only behavior. The "M3 graduation" forward note this bullet used to point at is retired — this is the shipped behavior, not a future milestone.
  - [x] **Launch Pad authoring convention** — machine-authored briefs emit `corpus-task:` bullets only, never `cmd:`/bare shell (`agents/launch-pad.md` Phase 5 + `skills/supervisor-readiness/SKILL.md` §"`## Executable Acceptance`"). `cmd:` is reserved for human authorship; on the interactive path a `cmd:`/bare bullet now routes through Phase 6 action 2a (`approve-and-stamp`/`strip-cmd-bullets`/`discard`, quoting the flagged bullets verbatim); on `--non-interactive`/`--non-interactive-fallback` it auto-strips instead of proceeding unreviewed.
  - [x] Whether the interactive path should also default to `--no-cmd` (the question this bullet left open) is superseded: the stamp gate makes execution content-keyed to a human-reviewed set of bullets on BOTH paths, which is a stronger property than defaulting `--no-cmd` on either one.
