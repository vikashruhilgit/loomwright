...
```
  brain (graph + wiki)
        │  READ  (Phase 1)
        ▼
  plan / execute / review / QA / self-heal   ← loomwright
...
- **Corpus specs** live in-repo as version-controlled fixtures:
  `loomwright/scripts/brain-baseline-corpus/*.md` (a sibling of `eval-corpus/`; one file
...
New skill: `loomwright/skills/brain-context/SKILL.md`. **Read-on-demand, NOT preloaded**
...
The skill defines:
- **Detection:** `graphify-out/graph.json` present in cwd? `LOOMWRIGHT_BRAIN_ROOT` set and
...
- **offer / apply (wiring options, robust → simple):**
  1. **Config-pointed path** (recommended, portable): write `LOOMWRIGHT_BRAIN_ROOT` +
...
4. ~~**Who owns the eval corpus** — does it live in the plugin repo (`.supervisor/eval/`) or the brain?~~
   **Resolved (Phase 0 landing):** corpus *input fixtures* are version-controlled in the plugin repo at
   `loomwright/scripts/brain-baseline-corpus/` (a sibling of `eval-corpus/`); only the
