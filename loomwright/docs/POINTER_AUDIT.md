...
| 6c | Worker `Shared local services: <line>` paste (harness-port/06) | `skills/async-orchestration/SKILL.md` §Worker contract (Sequential-/Single-Agent-path Task template); `agents/execute-manager.md` Step 3 Task template (the SEPARATE, independently-authored parallel-path worker prompt — see row 5) | <0.3k (already bounded at the producer: ≤10 non-empty lines, fences stripped, `(truncated)` marker) | **EXCEPTION (kept, added this item)** | Worktree workers cannot read the brief; ≤10 lines, fences stripped, bounded at the producer (Launch Pad Phase 1 action 8 / Phase 5 step 3a) — mirrors rows 6/6b's justification style. OPTIONAL: omitted entirely from both spawn sites when the brief carries no `Shared local services` line. |
...
| 10 | Phase 4.5 reviewer enrichment lines (`{prior_churn summary}` / `{house_rules summary}` / `{brief_conformance summary}`) | `skills/self-heal-advisory/SKILL.md` Part 2 §"Review-and-fix loop" reviewer spawn prompt — the three ADVISORY lines (PRIOR-CHURN / HOUSE-RULES / BRIEF-CONFORMANCE) directly after the DIFFERENT-LENS DIRECTIVE and ANTI-OVERLAP paragraphs | bounded reader outputs | **ALREADY COMPLIANT (no change)** | These are the *summary half* of the pointer pattern by design: `read-postmortem.sh` / `read-bridge.sh` / `read-rules.sh` emit bounded digests of file-backed corpora. Pointing the reviewer at the raw corpora instead would UNBOUND the content and break two invariants: the readers' fail-safe self-gating (empty output ⇒ omit the line) and the "corpus is fed to the REVIEW lens only, never to workers/fixers" seam. **Annotation (2026-08-17, graphify-tier retirement):** `read-bridge.sh` and the `{area_knowledge summary}` enrichment line this row originally cited (dropped from the placeholder list 2026-09-13, when the pointer became a descriptive anchor) were retired with the tier; the `read-postmortem.sh` / `read-rules.sh` half of the row stays live. Historical row kept — this register is a point-in-time audit, annotated rather than rewritten. **Annotation (2026-09-13, brief-conformance):** a third enrichment line, `{brief_conformance summary}` (BRIEF-CONFORMANCE ADVISORY), joined the two live ones — the acceptance-criteria paste is bounded by the cap in Part 1 of the same skill and the line carries `{brief_path}` as the pointer for anything past the cap (pointer, not payload), so the row's verdict stands. |
| 11 | Rubric grader (`{numbered list of rubric_bullets}`) | `skills/self-heal-advisory/SKILL.md` Part 2 §"Outcomes Rubric grading" grader prompt — the "Rubric items" numbered list | 0.2–0.7k (3–7 one-line bullets by authoring rule) | **BELOW THRESHOLD (no change)** | Under 1k by construction, and the grader must score EVERY item — a partial read is a wrong grade. |
...
Sites 1–5 and 18 were converted in this change; sites 6, **6b**, **6c**, 7–9 are the justified
exceptions; sites 10–17 were audited and found already compliant, below threshold, or out of
scope. (Row 6b — the worker `lanes:` paste — was added later, in v15.20.0/D6, and row 6c — the
worker `Shared local services:` paste — later still, in harness-port/06; both are folded into
