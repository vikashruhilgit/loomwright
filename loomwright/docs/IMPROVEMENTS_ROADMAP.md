...
> - **OPEN (0):** none — item 6 (the last OPEN item) closed in v15.5.0 and was RE-OPENED AND CLOSED DIFFERENTLY in v15.66.0: both the `WorktreeCreate` and `WorktreeRemove` hooks were REMOVED from `loomwright/hooks/hooks.json` (the former had been aborting every native worktree creation); the plugin's own worktrees are now recorded by the `PostToolUse (Bash)` observer `worktree-audit.sh record` and read back by `worktree-audit.sh report`
>
