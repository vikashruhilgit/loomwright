...
| A button says the write was **REFUSED by the server guard** | The token reached the server and was wrong — almost always a tab left over from a **previous** run, since the token dies with the server that minted it. The page discards it, so the *next* click reports the no-token case instead. Run `setup-ui.sh check` for the current `open:` URL. The refusal names which of the four parts said no. |
