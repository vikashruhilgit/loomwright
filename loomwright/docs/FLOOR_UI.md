...
| One **lane per agent** in the shown session | `sessions.detail.current.agents[]` | Ordered by `last_ts` descending. Label: name (or `main thread`, or `identity unknown`) · event count · last-event age. A typed lane takes the colour of the matching roster row. An untyped lane is neutral, and dashed **only** when nothing identifies it: a row the projector scoped `main` — the thread of the session itself, identified from the transcript path its own events carried — is named `main thread` with a plain chip, because "no role" and "no identity" are different facts. |
| The **session note** under the lanes | `sessions.detail.current.{cc_session_id,branch,selection,sessions_not_plugin_work}` | Which session the page is showing and **why that one**. The projector prefers a session that recorded **plugin work** — an agent whose `agent_type` starts `loomwright:`, or an `autonomous_session_start` — over the merely most recent, because a person working directly in Claude Code emits the same hook events as a Supervisor run and, being the thing they are typing into, is almost always newest. The view kept landing on the operator's own chat while their `/automate` run sat unseen. `selection` names the rule that produced the row and **fails open**: with nothing qualifying, the newest recorded session is shown and labelled `newest_recorded`, since showing nothing would be worse. Sessions set aside are **counted**, never silently dropped. This only became possible with the `PostToolUse[Task]` identity hook — before it, `agent_type` was absent on 976 of 1,040 agent ids and there was nothing to filter on, so a session recorded before that hook cannot qualify. |
...
| The **rules browser** | `rules.detail` | Every rule in the committed `.agent/rules/` store, grouped by category and then by `applies_to` scope. `applies_to`, `check` and `supersedes` are each read by **key presence**, not truthiness, so absent / present-and-`null` / a value / present-but-malformed are four distinct texts — a rule whose `applies_to` was written as a bare string says so rather than reading as though no scope were declared. `provenance` is rendered as its `source` and `added` fields, never as the object. Supersession history shows chains in order, dangling pointers, and cycles whole. A file the projector could not parse is NAMED with its reason, and `read_completeness` (`all` / `partial` / `none`) states whether anything went unread — "could not examine" is never displayed as "examined and clean". **The globs in a `scoped to …` heading are clickable, and they are buttons rather than links on purpose:** every current browser refuses to follow a `file:` URL from a page served over http, and refuses *silently*, so a control shaped like a link would do nothing and say nothing. Clicking one **copies the path** — joined against the directory the served index names for the document on screen, which the note under the basis line states, so a project registered below its git root (where `build-floor.sh` actually runs) shows the discrepancy rather than hiding it. A malformed or non-array `applies_to` keeps its own heading text and gains no button: the chips are rendered only when they spell that heading back exactly. The note persists until the next click or a project switch — a self-clearing flash would be a second timer, and there is one. The section also carries a static line saying how a rule gets **into** the store (`/rules add`, or `add-rule.sh … --confirm`), because authoring is the one thing this view shows that the page itself cannot do. |
| The **churn view** | `postmortem.detail` | The review-churn ledger as two distributions — root-cause class and flow stage — each sorted **by key, never by count**, because ordering by size is the ranking this view refuses to do. The flow-stage basis is printed from the projection's own `flow_stage_basis` field rather than restated here: the ledger carries a second, disagreeing representation, and `flow_stage_counter_disagreements` publishes how far apart they are. Malformed lines are counted and named, never folded into a class. Correlations between a rule's scope and the ledger are labelled observations and carry their evidence; an uncomputable one is omitted rather than shown as a zero. A correlation is drawn as **one row per ledger line**, not one per match: a match is a (line, path, glob) triple, so a line whose commit touched many in-scope paths would otherwise reprint that line's evidence once per path. The paths are not dropped — they sit under the row, grouped by the glob that claimed them. |
...
- **`serve` prints one URL carrying this run's token in the `#fragment`.** Open *that* URL. A fragment is never transmitted to any server, so the token cannot reach a request line, an access log, a proxy or a referrer header; the page strips it from the address bar as it reads it, so it cannot leak through history, a bookmark or a screenshot either. Opening a bare `http://127.0.0.1:<port>/` still works — the page reads everything — but the four buttons will be refused, and the page says so rather than failing silently.
- **That URL is recoverable, and it has to be.** The token is minted per run and printed on one line, so for as long as that line was the only copy, a reader who scrolled past it — or who simply came back the next day and typed the address they remembered — could not reach the four buttons again and had no way to be told why. `serve` therefore records the whole `open:` line in **a SIBLING of the ui directory** (never a file inside it), at mode **0600**; the name is derived from that directory's own basename — `ui-serve.token` for the default `.../ui` — because a *fixed* name in the parent would collide for any two `--ui-dir` values sharing it (`.../ui` and `.../ui-staging`, say), and one file for two running servers means `check` hands back whichever `serve` ran last and `stop` on either blinds the other; `check` reprints it while the server is up, and `stop` deletes it. **Beside, not inside, is the whole security argument** — the handler serves the entire ui directory, which is exactly why `GET /serve.log` returns the serve log, so a token file placed in there would be fetchable over this loopback port by any local process and its `0600` mode would count for nothing: the server reads it as the owning user and hands the bytes to whoever asked. Outside the served root, the only route to it is the filesystem, so it grants precisely the access the server's own **process environment** already grants that same user, and no more. This is the same reasoning that keeps the project registry a sibling. Offering the token over HTTP instead *would* be a real widening, which is why it is not offered.
...
- **A lane's number is events, not turns and not progress.** It counts recorded log lines for that `agent_id` in the session shown — which is the one that recorded plugin work, not necessarily the most recent (see the session-note row above). Two lanes with the same count have not necessarily done the same amount of work.
...
| A button says **REFUSED by the server guard (`token-missing-or-wrong`)** | The token reached the server and was wrong — almost always a tab left over from a **previous** run, since the token dies with the server that minted it. The page discards it, so the *next* click reports the no-token case instead. Run `setup-ui.sh check` for the current `open:` URL. |
| A button says **REFUSED by the server guard (`origin-not-this-server` / `host-not-loopback`)** | Not about the token, and the page **keeps** it: the guard refused the request's `Origin` or `Host`, which is what a browser extension or a proxy rewriting those headers looks like. Re-pasting the URL reproduces it — what needs changing is whatever rewrites the headers. The guard names which of its four parts said no, and the page reads that name rather than treating every 403 as a stale credential. |
| A button says the page **holds no token** | Nothing was sent — this refusal is the page's own, before any request. You are on a URL with no `#token=`, which is what a bookmark, an omnibox completion, a restored session or a fresh tab all give you. `setup-ui.sh check` reprints the current `open:` URL; paste it into the tab and press Enter — the page picks it up without a reload. Every one of the four buttons also has a command-line verb that needs no token at all. |
