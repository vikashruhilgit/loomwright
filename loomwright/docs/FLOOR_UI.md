...
| **`Host` validated** | must be `127.0.0.1`, `localhost` or `[::1]`, with or without the port | **attack 2 on every mutating route**, and this is the only part that does: a rebound name reaches this socket carrying the attacker's own name in `Host` | a genuinely same-origin request, which is the point — and it is not reached on a `GET` at all: see the note directly below |
