...
| `record_worker_result` | Record worker output | worker_id, subtask_id, result {files_modified, lines_added, lines_removed, tests_run, tests_passed, status, error, out_of_lane, deviations, not_verified} | `"Worker {id} result: {subtask_id} {status}, +{added} -{removed}"` |
