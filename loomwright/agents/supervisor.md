...
1. **Fail-closed probe (run FIRST):** `command -v node` AND `test -f "${CLAUDE_PLUGIN_ROOT}/sdk-spike/dist/runner.js"`. If either fails, ABORT the run with `error: "sdk_runner_unavailable"` — NEVER silently fall back to the default path. Error guidance: `dist/` is gitignored and marketplace installs ship source only — build once with `npm install && npm run build` inside `${CLAUDE_PLUGIN_ROOT}/sdk-spike`.
