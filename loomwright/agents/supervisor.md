...
4. **`workflow-management` (~4,267 proxy tokens) — ROUTED, no replacement Read needed.** The skill's own content (7-phase state machine diagram, tool-call budget tables, resume protocol, permission batching, plugin-hooks description) is a near-duplicate of content already stated directly in THIS file's own "7-Phase Workflow", "Context Management", and Quality Checklist sections — cross-checked section by section. The only body citation (the Phase-numbering-convention footnote just above the 7-Phase Workflow heading, "mirrors `skills/workflow-management/SKILL.md`") is decorative attribution: the convention it describes is stated inline at that exact spot, not sourced from the skill at runtime. No phase genuinely needs to Read it.
...
6. **`supervisor-readiness` (~6,926 proxy tokens) — ROUTED, no replacement Read needed.** The single body citation is Phase 2's "Decomposition Threshold" reference for path selection (Single-Agent vs. Sequential vs. Parallel) — but that threshold rule is *applied* by the Orchestrator (who decides subtask count), not by the Supervisor, and per the skill's own text, "Orchestrator does not preload this skill and cites this section by path" either. The Supervisor's Phase 2 action 4 merely counts the subtasks the Orchestrator already produced (`== 1` / `--sequential with > 1` / otherwise) — a simple comparison, not a rule requiring the skill's content. The rest of the skill (pre-flight checklist, Supervisor-Ready Brief template, jobs-folder convention) is Launch-Pad-facing, not Supervisor-facing.
