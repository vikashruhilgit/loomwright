...
1. For each subtask (in order):
   ...
   - **Lane reports on this path are RECORD-ONLY — no collision escalation, by construction.**
     ...
     automated gate here.
   - **Deterministic gate (zero-token, no reviewer spawn):** run `bash "${CLAUDE_PLUGIN_ROOT}/scripts/verify-provides.sh" {brief_path} {subtask_id} --root .` and let the disk result decide (cross-checking the worker's own `outputs_verified`/`outputs_gap`, D1 routing for `unverifiable`, `provides_mismatch` / `worker_result_absent` recorded) plus tests/lint on the branch — same check as the Single-Agent Path step 3 above. Disk gap, or `status != completed` with no disk contradiction → retry (bounded) or pause, per existing WORKER_RESULT handling.
