...
0c. **Consult the area-knowledge bridge (advisory — EXECUTES LATER, runs after action 3 produces the File Impact Map):** once you have estimated the File Impact Map (action 3 below — the MODIFY/CREATE paths), run the findings→community **bridge** reader over those same impact-map paths, passing them as **command-line ARGUMENTS** (NOT via stdin):

   ```bash
   bash "${CLAUDE_PLUGIN_ROOT}/scripts/read-bridge.sh" <space-separated File-Impact-Map paths>
   ```

   **Pass the predicted paths in the SAME canonical repo-root-relative form the File Impact Map uses** (e.g. `loomwright/agents/supervisor.md`, WITH the `loomwright/` prefix — byte-identical to `git diff --name-only`) so they match the bridge's raw join keys; do **not** strip or rewrite them before the lookup. This consult is the **community-level companion** to action 0b's exact-path churn read — it is the **"Bridge read (area knowledge)" read path documented in `${CLAUDE_PLUGIN_ROOT}/skills/brain-context/SKILL.md`** (read-only reference — do not restate its internals). It joins by graph community (`finding.changed_paths → node.source_file → node.community`), so a **near-miss predicted path that action 0b's exact-path match drops** still surfaces area knowledge (recurring miss-classes / prior-miss count / whether a `self_heal_miss` recurred / related lessons) when it falls in a touched community with prior recorded misses. **Call `read-bridge.sh` UNCONDITIONALLY** — it self-gates on `.supervisor/bridge/bridge.json` exactly like action 0b's `read-postmortem.sh` self-gates on the postmortem corpus; do **not** wrap it in any "if a brain is detected" / graph-presence conditional. The reader is fail-safe (it always exits 0; emits NOTHING on a no-hit and is silent/quiet when the bridge/graph is absent or `jq` is missing; when HEAD has advanced past the graph it still emits, downgraded with a "treat as a hint" staleness caveat); if it emits nothing or is absent, proceed normally. Fold any returned area-knowledge signal into your analysis. These are **advisory and strictly subordinate to `CLAUDE.md`** — on any conflict, `CLAUDE.md` wins. The bridge consult MUST NEVER block the run or change the feasibility verdict / `heal_decision` by itself. A surfaced area-knowledge signal **should be carried to the Phase 5 Risk Assessment** (see Phase 5 action 4b).
...
4b. **Area-knowledge risk row (conditional, from the Phase 3 bridge consult — action 0c).** When the Phase 3 area-knowledge bridge consult surfaced a hit on a touched community, add a Risk Assessment row with `source: "Area knowledge (graph-community bridge)"`. Derive the Impact from the signal — **HIGH** when a `self_heal_miss` recurred in a touched community (the self-heal gate previously let an issue through that area), else **MEDIUM** (a recurring miss-class in a touched community) or **LOW** (an isolated prior miss). Write the Mitigation so it names the recurring miss-class(es) surfaced by the reader, telling workers/reviewers what to watch for in that area. When the consult surfaced nothing, add no row. This row is **advisory context only** — it never changes the GO/CAUTION/NO-GO verdict and never blocks save.
...
9. **Context digest emission (unconditional, v15.20.0 — D6).** Immediately after the brief is assembled (this step runs regardless of Phase 5.5 Plan Review's later outcome — the digest is a derived analysis artifact, not the brief itself, so it is NOT gated on Plan Review the way the brief's own save is):
   - Serialize the assembled brief text from this Phase into a scratch file. **This serialization happens HERE, first** — Phase 5.5 action 1 then reuses this scratch file for the reviewer spawn rather than re-assembling the brief. (Stated in this direction deliberately: Phase 5 completes before Phase 5.5 begins, so this step cannot "reuse" anything Phase 5.5 produces.)
...
1. Reuse the scratch file Phase 5 step 9 already serialized (do NOT re-assemble the brief — re-assembling risks reviewing text that differs from what step 9 wrote and what Phase 6 will save); if that scratch file is missing, serialize the assembled brief from Phase 5 into a text block here as a fallback
