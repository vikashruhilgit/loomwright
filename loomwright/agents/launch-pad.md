...
10. **Cited-line premise check (advisory, harness-port/03 — SHARED wording with `skills/preflight-sync/SKILL.md` signal (d); that skill cross-references this action rather than restating it, so any edit here MUST be mirrored there):** every `path:line` reference cited in the goal string / resolved requirement file is resolved against `origin/$BASE_BRANCH` and judged HOLDS / STALE / UNCLEAR from a short excerpt — advisory only, and it can NEVER change a Feasibility verdict, an acceptance criterion, or any other Phase 3 finding by itself.
    ...
    - **Resolution:** `git show "origin/$BASE_BRANCH:<path>"`; when `<path>` is partial (no `/`), resolve by UNIQUE suffix match against `git ls-tree -r --name-only "origin/$BASE_BRANCH"` — zero matches or more than one match ⇒ `resolves: no`, which always carries `premise: UNCLEAR`. Capture lines `N-4..N+4` around the cited line. If the goal/requirement's described defect or phrase is not present in that window, search the same file once for the ref's named symbol or quoted phrase before judging — when found at a different line, the row is `HOLDS (moved to <line>)`, never `STALE`.
