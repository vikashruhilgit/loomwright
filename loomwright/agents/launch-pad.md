...
10. **Cited-line premise check (advisory, harness-port/03 — SHARED wording with `skills/preflight-sync/SKILL.md` signal (d); that skill cross-references this action rather than restating it, so any edit here MUST be mirrored there):** every `path:line` reference cited in the goal string / resolved requirement file is resolved against `origin/$BASE_BRANCH` and judged HOLDS / STALE / UNCLEAR from a short excerpt — advisory only, and it can NEVER change a Feasibility verdict, an acceptance criterion, or any other Phase 3 finding by itself.
    ...
    **Emission:** when ≥1 ref is found, emit `### Cited-line premise check` under the Phase 3 output (below) with a table `ref | resolves | premise | deciding line | as of`. When 0 refs are found, emit **NOTHING** — no heading, no "none" line (same convention as the `- **Source requirement:**` Environment line). Any **STALE** row is additionally carried forward to the Phase 5 Risk Assessment as a **MEDIUM** row with `source: "Cited-line premise (Phase 3)"` (see Phase 5 action 4b) — this is a RISK row, not a Plan Reviewer finding, and it can never change the GO/CAUTION/NO-GO verdict or block save.
...
4b. **Cited-line premise risk row (conditional, from the Phase 3 cited-line premise check — action 10).** When the Phase 3 `### Cited-line premise check` subsection surfaced a **STALE** row, add a Risk Assessment row with `source: "Cited-line premise (Phase 3)"`, Impact **MEDIUM** (fixed — not derived), naming the stale ref and the file it no longer holds at. When the check surfaced no STALE row (including when the subsection is entirely absent — 0 refs cited), add no row. This row is **advisory context only** — it never changes the GO/CAUTION/NO-GO verdict and never blocks save.
