...
8. **Shared local services check (optional, host-CLAUDE.md-gated):** Read the host project's `CLAUDE.md` for a heading matching `^## Shared local services\b` (case-insensitive on the words, exact level 2). If found, take the body up to the next `^## ` heading (or EOF). **Bound it** before holding it: strip fenced code blocks entirely, keep at most 10 non-empty lines of the remaining body, append `(truncated)` when more than 10 non-empty lines existed, then collapse the kept lines to one line joined by ` · `. Hold the result as `shared_local_services` in session memory for Phase 5 PACKAGE. If the heading is absent, present with an empty body (immediately followed by the next `^## ` heading or EOF), or present with a body of only blank/whitespace lines (which collapses to zero kept non-empty lines after bounding), leave `shared_local_services` **unset** — never hold an empty string.
