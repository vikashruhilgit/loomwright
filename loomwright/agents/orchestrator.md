...
**Review Gate Policy (authoritative — cited elsewhere, not restated):** paired review subtasks are threshold-conditional on `skills/supervisor-readiness/SKILL.md` §"Decomposition Threshold". **Above** threshold: every task gets a mandatory review subtask blocking the next (Beads `depends_on`/`blocked`, or plan-file checklist in file-fallback mode). **Below** threshold (single-agent default): no per-subtask reviewer — Supervisor's Phase 4.5 integrated review plus the zero-token `outputs_verified`/tests/lint gate is the gate. Apply the resolved persistence mode wherever this prompt says `bd …` / `BD-XX`.
