...
**Self-heal lens (Supervisor Phase 4.5 / `/review-pr`):** during a self-heal or standalone PR-heal review, ALSO apply the repo-agnostic **Self-Heal Miss-Class Checklist** in `skills/quality-checklist/SKILL.md` — validation parity (backend mirrors frontend), no numeric-falsy coercion, no positional args to options-object functions, branch coverage for new conditionals, count/version/restated-list drift, cross-reference precision drift, `brief_conformance` (a stated acceptance criterion with no corresponding change in the diff — checkable only in Supervisor Phase 4.5, where the spawn prompt carries the brief's criteria; see "Brief conformance" in the Review Process below), and `deviations` (worker- or fixer-recorded plan drift that contradicts a stated criterion or rubric bullet — checkable only in Supervisor Phase 4.5, where the spawn prompt carries the DEVIATIONS ADVISORY line; an unprefixed entry reads as `other:`; see "Deviations conformance" in the Review Process below). It is repo-agnostic: it catches these classes on external app repos where the `consistency_audit` triggers above do not fire.

**Test-assertion diagnosis + gate-configuration lens (six-phase-loop-gaps/02, presence-scoped):** when a DEVIATIONS ADVISORY line is present and a hunk changes a test assertion and the advisory carries no `test:` entry naming that test (worker- or fixer-recorded), or carries one whose diagnosis says `code wrong`, raise a HIGH `category: new` finding "assertion changed without diagnosis". When the advisory is ABSENT (the standalone `/review-pr` drain has no advisory carriage today) or reports truncation (`omitted N`), raise nothing — a truncated or missing advisory is not evidence of an undiagnosed edit. Also HIGH, unconditionally (no advisory needed): any hunk touching `.claude/settings*.json`, `hooks.json`, or `scripts/guard-*.sh` — an armed session should never need to touch its own gate configuration. **Same-test-twice cap:** the same test flagged by this lens on two consecutive Phase 4.5 iterations escalates the review to `NEEDS_HUMAN` rather than allowing a third round — the loop already knows how to honour this bound the same way it honours `max_heal_iterations`.
...
5a. **Brief conformance (Supervisor Phase 4.5 only — when the spawn prompt carries a BRIEF-CONFORMANCE ADVISORY line)**
   ...
   - `not_addressed` — the diff contains no corresponding change. Emit exactly ONE `category: new`, severity **HIGH** finding per such criterion, quoting the criterion verbatim in `description` and stating what is missing in `suggestion`; `file:` is the path the criterion most plausibly targets when evident, otherwise the sentinel `brief` (the spawn-prompt line above is the authoritative statement of this convention). This is an ordinary finding: it enters the same FAIL/PASS decision and the same `new` + BLOCKING/HIGH fix floor as every other finding — no new gate, no new field.
...
5b. **Deviations conformance (Supervisor Phase 4.5 only — when the spawn prompt carries a DEVIATIONS ADVISORY line)**

   The Phase 4.5 spawn prompt (authoritative wording: `skills/self-heal-advisory/SKILL.md` Part 2 §"Review-and-fix loop"; the collection and bounding rules live in its Part 1 §"Deviations advisory" and are NOT restated here) may hand you worker- and fixer-recorded departures from plan (`WORKER_RESULT.deviations` / `FIX_RESULT.deviations`), each by convention prefixed `plan:`/`edge:`/`open:`/`test:` — an unprefixed entry reads as `other:`. The question for each is **"does this deviation break a stated criterion?"**, never "would I have done it differently?": a MERELY unexpected deviation is NOT a finding. For each deviation that CONTRADICTS a stated acceptance criterion or an `## Outcomes Rubric` bullet, emit exactly ONE `category: new` finding quoting BOTH the deviation and the contradicted criterion/bullet — an ordinary finding, entering the same FAIL/PASS decision and fix floor as every other finding, no new gate, no new field. A standalone `/review-pr` review has no `state.md` and never carries this line — the step simply does not apply there.
