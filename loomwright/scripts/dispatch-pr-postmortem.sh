...
#   launch a brand-new detached headless `claude -p "/loomwright:pr-postmortem
...
#   LOOMWRIGHT_POSTMORTEM_DISPATCH_DRY_RUN=1
...
DRY_RUN="${LOOMWRIGHT_POSTMORTEM_DISPATCH_DRY_RUN:-}"
DEFAULT_THRESHOLD=2
...
if [ -n "$DRY_RUN" ]; then
  printf 'DRY_RUN_DISPATCH: %s -p "/loomwright:pr-postmortem %s"\n' "$CLAUDE_BIN" "$PR_URL"
...
#
# The slash command MUST be NAMESPACED (`/loomwright:pr-postmortem`), NOT the bare
...
# `claude -p "/loomwright:pr-postmortem <url>"` ran the skill end-to-end and
# appended a results.jsonl line). The sibling dispatch-pr-review.sh sidesteps this entirely by
# launching a REGISTERED agent (`--agent loomwright:review-pr-runner`); /pr-postmortem
...
( nohup "$CLAUDE_BIN" -p "/loomwright:pr-postmortem $PR_URL" >>"$RUN_LOG" 2>&1 </dev/null & ) >/dev/null 2>&1 || true
