...
#   - LOOMWRIGHT_REVIEW_DISPATCH_DRY_RUN=1 makes the chained
#     dispatch-pr-review.sh dry-run (emit DRY_RUN_DISPATCH + write the per-PR
#     marker) instead of launching claude.
#   - LOOMWRIGHT_HOOK_CURRENT_BRANCH is the test seam controlling the
...
run_wrapper() {
  ...
  RUN_OUT="$( cd "$wd" && LOOMWRIGHT_REVIEW_DISPATCH_DRY_RUN=1 \
      LOOMWRIGHT_HOOK_CURRENT_BRANCH="$br" \
