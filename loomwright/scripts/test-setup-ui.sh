...
# `23 commands` is excluded because a per-release `Counts: 14 agents / N commands / N skills /
...
# parenthesised `(23 commands` IS swept, because that is the current-state form README uses
...
for f in $Z_SURFACES; do
  [ -r "$f" ] || continue
  z_seen=$((z_seen + 1))
  for pat in '23 slash commands' 'Slash commands (23)' '23 entry points' '(23 commands'; do
    if has_lit "$f" "$pat"; then z_stale="$z_stale [$(basename "$f"):$pat]"; fi
  done
done
if [ "$z_seen" -lt 5 ]; then
  no "(z13) no stale '23 commands' residue on the release surfaces" \
     "only $z_seen of 6 surfaces were readable from this suite, so the sweep would be near-vacuous"
elif [ -z "$z_stale" ]; then
  ok "(z13) no stale '23 commands' residue across $z_seen release surfaces"
else
  no "(z13) no stale '23 commands' residue across the release surfaces" "still stale in:$z_stale"
fi
...
for f in "$REPO_ROOT/README.md" "$REPO_ROOT/CLAUDE.md" "$REPO_ROOT/.claude-plugin/README.md" "$HELP_MD"; do
  ...
  if has_re "$f" '24 slash commands|Slash commands \(24\)|24 entry points|\(24 commands'; then :; else
