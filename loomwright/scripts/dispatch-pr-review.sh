...
#   runner agent  : loomwright:review-pr-runner
#   until-mergeable signal (S2-pinned env-var contract — §"Until-Mergeable Dispatch Signal"):
#     LOOMWRIGHT_UNTIL_MERGEABLE     — export "1" by default (opt-out)
#     LOOMWRIGHT_CHECK_WAIT_TIMEOUT  — optional; export when --check-wait-timeout / config set
#     LOOMWRIGHT_REVIEW_CHECK_PATTERN — optional; export when --review-check-pattern / config set
...
#   detached runner is launched with LOOMWRIGHT_UNTIL_MERGEABLE=1 EXPORTED by
...
#   LOOMWRIGHT_CHECK_WAIT_TIMEOUT / LOOMWRIGHT_REVIEW_CHECK_PATTERN.
#
...
#   detached HEADLESS `claude -p --agent loomwright:review-pr-runner
...
#   LOOMWRIGHT_REVIEW_DISPATCH_DRY_RUN=1
...
CLAUDE_BIN="${LOOMWRIGHT_CLAUDE_BIN:-claude}"
RUNNER="loomwright:review-pr-runner"
DRY_RUN="${LOOMWRIGHT_REVIEW_DISPATCH_DRY_RUN:-}"
...
# Default ON: export LOOMWRIGHT_UNTIL_MERGEABLE=1 into the detached runner so
...
if [ -n "$DRY_RUN" ]; then
  ...
  if [ "$UNTIL_MERGEABLE" -eq 1 ]; then
    ENV_PREFIX="LOOMWRIGHT_UNTIL_MERGEABLE=1 "
...
# sites stay unwired FROM THE AUDIT LOG on purpose — the reader's ground-truth rule
# (a path git no longer lists is not an orphan) already covers a removal the log
# never saw. (They ARE wired to the salvage — see SALVAGE_BIN above.)
...
if [ "$UNTIL_MERGEABLE" -eq 1 ]; then
  export LOOMWRIGHT_UNTIL_MERGEABLE=1
  [ -n "$CHECK_WAIT_TIMEOUT" ] && export LOOMWRIGHT_CHECK_WAIT_TIMEOUT="$CHECK_WAIT_TIMEOUT"
  [ -n "$REVIEW_CHECK_PATTERN" ] && export LOOMWRIGHT_REVIEW_CHECK_PATTERN="$REVIEW_CHECK_PATTERN"
  log "until-mergeable drain ENABLED (LOOMWRIGHT_UNTIL_MERGEABLE=1)"
else
...
[ -n "$HEAD_REF" ] && export LOOMWRIGHT_PR_HEAD_REF="$HEAD_REF"
...
# (a trap function sees its OWN empty positionals on EXIT, not the script's $1..$8).
