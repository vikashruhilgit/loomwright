...
#
# Authoritative spec: loomwright/docs/TELEMETRY.md §"Webhook Notifications"
#
# ----------------------------------------------------------------------------
...
#     LOOMWRIGHT_WEBHOOK_URL still applies (must be set, but any
#     non-empty value works — e.g. LOOMWRIGHT_WEBHOOK_URL=test).
...
#   `headers`, not inside `url`. To gate on LOOMWRIGHT_WEBHOOK_URL the URL
...
DRY_RUN="${LOOMWRIGHT_WEBHOOK_DRY_RUN:-}"
...
if [ "$HOOK_EVENT" = "PreToolUse" ] && [ "$TOOL_NAME" = "AskUserQuestion" ]; then
  ...
  # webhooks outside plugin context; LOOMWRIGHT_NOTIFY_SCOPE=all fires on
  # every AskUserQuestion. Three independent OR'd markers.
  SCOPE="${LOOMWRIGHT_NOTIFY_SCOPE:-plugin}"
  ...
  # LOOMWRIGHT_WEBHOOK_FORMAT=ntfy. The earlier broad `*ntfy*` glob was
  ...
  [ "${LOOMWRIGHT_WEBHOOK_FORMAT:-}" = "ntfy" ] && NTFY=1
