...
# harness-port/01-worker-rules item's acceptance criteria (same pattern this section used for
# the six-phase-loop-gaps/01 worker-deviations item that introduced it), that "the MANIFEST
...
if grep -qE "$MANIFEST_ROW_RE" "$GUARD" && grep -q 'not_verified: object\[\]' "$RESULT_SCHEMAS"; then
  echo "ok    WORKER_RESULT MANIFEST row carries not_verified AND RESULT_SCHEMAS.md documents it (paired)"
  pass=$((pass+1))
else
  echo "FAIL  WORKER_RESULT MANIFEST row / RESULT_SCHEMAS.md not_verified pairing missing"
fi

# MUTATION: revert a COPY's MANIFEST row to omit not_verified (RESULT_SCHEMAS.md — the real,
...
sed -E "s/${MANIFEST_ROW_RE}/worker|worker.md|WORKER_RESULT|schema_version,task_id,status,files_modified,summary,outputs_verified,outputs_gap,out_of_lane,deviations/" \
  "$GUARD" > "$REVERTED"
...
if grep -qE "$MANIFEST_ROW_RE" "$REVERTED"; then
  echo "FAIL  negation check: reverted copy still carries not_verified in its MANIFEST row — the mutant did not mutate what it claims"
else
  if grep -q 'not_verified: object\[\]' "$RESULT_SCHEMAS"; then
    echo "ok    negation check: reverted MANIFEST row omits not_verified while RESULT_SCHEMAS.md still documents it — a real, catchable mismatch"
    pass=$((pass+1))
  else
