...
#        # judged and passes `.high_risk` straight through; `null` (unclassifiable), missing, or
#        # anything but an explicit `false` ⇒ fail-closed PARK (`!= "false"`, the cond-3 shape).
...
gate_eval() {
  ...
  # WITHOUT the falsy-coercing `//`, an explicit has()/!= null check, `tostring`, and PARK
  # unless the value is the explicit `false` — so `true`, `null` (unclassifiable), a missing
  # key, or a non-boolean string all fail CLOSED. No `--trust-*` flag, config key, or
  # `.agent/risk.json` key is consulted here: nothing overrides this condition by design.
  local hr
  hr="$("$JQ" -r 'if has("high_risk") and (.high_risk != null) then (.high_risk|tostring) else "__MISSING__" end' "$ctx")"
  if [ "$hr" != "false" ]; then
    local rr
