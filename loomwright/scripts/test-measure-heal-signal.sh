...
( cd "$TMP9b" && LOOMWRIGHT_HEAL_SIGNAL_REPOS="$TMPe" bash "$SH_WRAP" --no-ledger ) >/dev/null 2>&1
njE="$(S '.pooled.n' "$TMP9b/.supervisor/heal-signal/_summary.json")"
if [ "$njE" = 7 ]; then ok "wrapper honored \$LOOMWRIGHT_HEAL_SIGNAL_REPOS override (n=7)"; else no "env override wrong (n=$njE)"; fi
