...
A="$(attrs_of "$R")"
grep -q 'deployment.environment=dev' < <(printf '%s' "$A") && ok "deployment.environment=dev preserved" || no "deployment.environment dropped (got: [$A])"
grep -q "service.name=mergeproj" < <(printf '%s' "$A") && ok "service.name replaced with new computed value" || no "service.name not updated (got: [$A])"
grep -q "service.name=old" < <(printf '%s' "$A") && no "stale service.name=old still present (got: [$A])" || ok "stale service.name=old removed (not duplicated)"
grep -q "service.version=${FIXTURE_VERSION}" < <(printf '%s' "$A") && ok "service.version present" || no "service.version missing (got: [$A])"
