...
#   1. PATH-FORM JOIN (claim 3 / Notes 8): graph source_file carries the "loomwright/"
...
#   community 1 — SPECIFIC, finding-bearing: loomwright/agents/supervisor.md (prefixed!)
...
write_graph() {
  ...
  mkdir -p "$repo/graphify-out"
  {
    ...
    printf '    {"id":"n_sup","source_file":"loomwright/agents/supervisor.md","community":1},\n'
    printf '    {"id":"n_sup2","source_file":"loomwright/agents/supervisor.md","community":1},\n'
    ...
    printf '    {"id":"n_lf","source_file":"loomwright/skills/lessonfile.md","community":1},\n'
    # community 2 — ubiquitous god-node: 14 distinct member source_files (>= threshold 12)
    local i
    for i in $(seq 1 14); do
      local comma=","
      printf '    {"id":"n_g%s","source_file":"loomwright/god/file%s.md","community":2}%s\n' "$i" "$i" "$comma"
    done
    # also give the god-node a finding-bearing input file so it would emit if NOT suppressed
    printf '    {"id":"n_gx","source_file":"loomwright/god/touched.md","community":2}\n'
    printf '  ],\n  "links": []\n}\n'
...
write_findings() {
  ...
  mkdir -p "$repo/.supervisor/postmortem"
  {
    "$JQ" -cn --arg repo "$slug" '{schema_version:1, number:101, repo:$repo, branch:"b1",
       pr_url:"u1", changed_paths:["loomwright/agents/supervisor.md"],
    ...
    "$JQ" -cn --arg repo "$slug" '{schema_version:1, number:102, repo:$repo, branch:"b2",
       pr_url:"u2", changed_paths:["loomwright/god/touched.md"],
    ...
    if [ -n "$cross" ]; then
      ...
      "$JQ" -cn --arg repo "$cross" '{schema_version:1, number:777, repo:$repo, branch:"bx",
         pr_url:"ux", changed_paths:["loomwright/agents/supervisor.md"],
...
write_joined() {
  ...
  ( cd "$repo" \
      && mkdir -p loomwright/agents \
      && printf 'pm-101 change\n' >> loomwright/agents/supervisor.md \
      && git add loomwright/agents/supervisor.md \
...
RAWHIT="$(B '.file_to_communities["loomwright/agents/supervisor.md"] // empty | tostring' "$BJSON")"
...
out_raw="$( bash "$READ_SH" "loomwright/agents/supervisor.md" 2>/dev/null < /dev/null; )"
# the reader cd's to its own GITROOT, so run it from inside the fixture repo for a real lookup.
out_raw="$( cd "$R1" && bash "$READ_SH" "loomwright/agents/supervisor.md" </dev/null 2>/dev/null )"; rcr=$?
...
out_god="$( cd "$R1" && bash "$READ_SH" "loomwright/god/touched.md" </dev/null 2>/dev/null )"; rcg=$?
...
out_les="$( cd "$R1" && bash "$READ_SH" "loomwright/skills/lessonfile.md" </dev/null 2>/dev/null )"; rcl=$?
...
run_build "$Rs"
out_stale="$( cd "$Rs" && bash "$READ_SH" "loomwright/agents/supervisor.md" </dev/null 2>/dev/null )"; rcst=$?
...
out_absent="$( cd "$Rn" && bash "$READ_SH" "loomwright/agents/supervisor.md" </dev/null 2>/dev/null )"; rca=$?
...
{"built_at_commit":"${hc:0:7}","nodes":[
  {"id":"a","source_file":"loomwright/skills/alpha/shared.md","community":1},
  {"id":"b","source_file":"loomwright/skills/beta/shared.md","community":5},
  {"id":"c","source_file":"loomwright/skills/beta/other.md","community":5},
  {"id":"d","source_file":"loomwright/.mcp.json","community":7}
],"links":[]}
EOF
