...
- `qa-executor: <target>` — recognized but DEFERRED to M2b slice 1b (records `unverified`; spawns nothing). The kind name is **reserved and kept** across the QA move to the `selvedge` plugin, so briefs authoring it stay valid; its eventual dispatch target is `selvedge:selvedge:qa-executor` when selvedge is installed (decision anchor: `scripts/run-ground-truth.sh`'s header block).
