...
- `loomwright/scripts/pr-postmortem-gather.sh` — the read-only gather script whose single JSON object this protocol consumes.
- `loomwright/commands/pr-postmortem.md` — the thin inline command shell that points here.
- `loomwright/scripts/run-eval.sh` — the `record_result` append-only JSONL pattern this skill mirrors.
- `loomwright/skills/quality-checklist/SKILL.md` — the Self-Heal Miss-Class Checklist used to set `self_heal_miss`.
