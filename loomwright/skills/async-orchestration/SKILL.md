...
### Single-Agent / Sequential Paths: Skip Worktrees
...
1. **Pre-merge safety gate** (ALL must pass before any merge):
   ...
   **Point 5 in full — children settled:** run
   ...
   - `rejected_stop_ids[]` (v15.83.0, diagnostic only) names every child with at least one `subtask_complete` row carrying `rejected: true` — a stop `validate-worker-result.py` rejected (`decision: block`), which is NOT a terminal row. An id that is in BOTH `unsettled_agent_ids[]` and `rejected_stop_ids[]` is a worker that was told to continue and has not yet produced an accepted result (or was forced to stop at the runtime's continuation cap with a malformed one) — surface it in the same line; the verdict itself is unchanged (`status` alone decides). See `docs/RESULT_SCHEMAS.md` §"Completion authority join".
...
**`## Not verified` section (optional, harness-port/04):** aggregated by reading `state.md`'s `## Worker Results` section and collecting every worker's `not_verified` sub-list — one bullet per item, across ALL workers, `- **<surface>** — <reason> (subtask <id>)`. **OMITTED ENTIRELY** (no heading, no "none" line) when every worker's `not_verified` list is absent/empty — a zero-item PR body is byte-identical to the template above with the `## Not verified` block removed. REPORT-ONLY: this section never gates FINALIZE, Phase 4.5, or a merge decision.
...
**Recording `not_verified` on the Sequential and Single-Agent paths (harness-port/04):** these two paths record a worker's result directly in `agents/supervisor.md` (§"Single-Agent Path" step 2, §"Sequential Path" step) rather than through the Execute Manager poll loop documented above — that is where `out_of_lane`/`deviations` are already forwarded, and `not_verified` is now forwarded the same way, unconditionally, into `state.md`'s `## Worker Results`. Recording only, same as `out_of_lane`: with no concurrent sibling on either path there is nothing for a collision/gating rule to fire on.
```
Task(
