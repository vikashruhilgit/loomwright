...
- **All gate webhook POSTs go through `${CLAUDE_PLUGIN_ROOT}/scripts/send-webhook.sh --event-type gate ...`** when `--notify` is set and a webhook URL is resolvable — `LOOMWRIGHT_WEBHOOK_URL` (env, highest precedence) **or** the USER-SCOPE `~/.claude/loomwright/egress.json` entry for this repo's slug (v15.87.0, red-team-hardening item 02 — resolved via `resolve-egress-config.sh`; a repo-relative `.supervisor/config.json`/`.supervisor/notify-config.json` `.webhook_url` is read only as an informational request, never a grant — `send-webhook.sh` resolves all of this internally; the call site does not need to check). Never construct gate-event JSON inline (jq-only payload construction is enforced by send-webhook.sh; the autonomous-loop call site must honor that boundary).
- **Fail-loud-on-misconfig (v14.2.2, resolution updated v15.87.0):** at INIT step 0, if `--notify` is set but NO webhook URL resolves (neither `$LOOMWRIGHT_WEBHOOK_URL` nor the user-scope `~/.claude/loomwright/egress.json` entry for this repo's slug carries a non-empty `webhook_url`), print ONE visible warning — `"--notify set, but no webhook URL resolvable (env var or ~/.claude/loomwright/egress.json); gate webhooks will NOT fire. Desktop banners are unaffected."` — then proceed (do NOT abort; desktop notifications and the rest of the loop are independent of the webhook). A repo-relative `.supervisor/config.json`'s `.webhook_url` does NOT satisfy this check on its own — it is informational only.
...
- **`.supervisor/state.md`:** the `## Session` block (`session_id`/`branch`/`status`/`phase`) is not written by any agent — it is derived by the hook-triggered `scripts/emit-progress-event.sh` (fired at the `loomwright:worker` `SubagentStop` hook) and projected by `scripts/build-state.sh`, per `skills/state-management/SKILL.md` §"Progress state". Context-Keeper remains the writer of the OTHER sections — `## Decisions Log` / `## Worker Results` / `## Error Log` / `## Phase Flags`. (Scope guard: this clarifies the autonomous-loop cross-link only; the authoritative write-path description lives in `agents/context-keeper.md` and `skills/state-management/SKILL.md`.) Separately, the autonomous loop **does not** grep state.md for `inter_subtask_gap` — Signal 2 detection uses only the three iteration-scoped sources (failed-brief contents, `SUPERVISOR_RESULT.error`, `SUPERVISOR_RESULT.summary`) to avoid any false-positive risk from pre-rewrite stale content.
