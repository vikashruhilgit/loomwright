...
- **The stamp.** A genuine `--confirm` (or `RULES_CHECK_CONFIRM=1` without `--if-stamped` / TTY-`y`) run writes `{git_common_dir, repo_root, hash, ts}` — keyed by `git_common_dir` — to `rules-check-stamp.json` in the per-user Loomwright config directory under `$HOME` (exact path: `commands/rules.md` §"Stamp semantics") — USER scope, the SAME directory red-team-hardening item 02 established for `egress.json`, but its OWN, separate file (never `egress.json` itself, which is telemetry-specific). `hash` is a sha256 over the sorted `id\tcheck` lines of EXACTLY the must-rule set that confirmed run selected — the stamp write and the `--if-stamped` comparison read the SAME in-memory selection, so the two can never independently disagree on what "the set" means.
- **`--if-stamped` (non-interactive, no prompt).** Recomputes the live hash of the same selection and compares it to the stamp recorded for the CURRENT repository — keyed by the PHYSICAL `git rev-parse --git-common-dir` (resolved via `cd … && pwd -P`), which every linked worktree of one repository shares, so a set confirmed in the main checkout replays in an `/automate` worktree of the same repo; a separate clone has its own key. `repo_root` in the record is informational only (the checkout that last confirmed). Equal ⇒ execute with no prompt, same output shape as `--confirm`. Unequal or absent ⇒ `[SKIP] all (unstamped)`, `Checks passed: 0/0`, exit 0 — the per-rule loop is not even entered.
- **Precedence** (extends §8's existing chain): `--no-cmd` **>** argv `--confirm` (or `RULES_CHECK_CONFIRM=1` **only when `--if-stamped` is absent**) **>** `--if-stamped` **>** TTY-yes **>** default-skip. `--no-cmd` still wins over everything, including a VALID stamp. **Anti-laundering:** under `--if-stamped` the `RULES_CHECK_CONFIRM` env var is IGNORED — only an explicit argv `--confirm` confirms — so an ambient env confirm in an unattended caller can never run an unstamped set or write the stamp that would launder it. `--if-stamped` is evaluated BEFORE the TTY prompt, so it never prompts, even on a terminal.
...
- **Honest limits.** (1) The stamp write takes NO lock: two confirming runs racing on one machine are last-writer-wins, and a lost record just replays `unstamped` until re-confirmed (fail-closed). A stamp file that is valid JSON but not an object is warned about on stderr and never clobbered; a failed write is warned about too. (2) A harvested `check_candidate` (commands/dreaming.md) is only ever SCOPED — a repo-wide or invalid-ERE candidate is never proposed. Its absence template, `grep -rnE -- '<p>' <paths>; test $? -eq 1`, is fail-CLOSED: if a scoped path later disappears grep exits 2 and the check FAILS. Its ratchet template, `test "$(grep -rlE -- '<p>' <paths> | wc -l)" -le <N>`, is NOT: a vanished scope counts 0 files and passes vacuously, so after a rename or move re-derive or retract such a rule rather than trusting its PASS. (3) The hash binds the check TEXT, never any file the check executes: a stamped human-written check such as `bash scripts/lint.sh` replays unattended against whatever `scripts/lint.sh` the current checkout holds — including a PR branch's edited copy in a linked worktree, since every worktree shares the key. Harvested candidates are grep-only and unaffected; Phase 4.5 already runs PR code (tests, ground-truth `cmd:`), so this adds no new privilege, but only stamp checks whose invoked files you would trust from any branch.
