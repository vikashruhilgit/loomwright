...
| `gate-eval` | §10 | The 6-condition fail-CLOSED trusted auto-merge gate — the **only** executor of `gh pr merge --squash`. Condition 6 (`high_risk`) comes from `scripts/classify-risk.sh`, re-run by the loop on the judged SHA; nothing overrides it. |
...
5. **Required checks green AND rubric satisfied.**
   ...
   - **Rubric satisfied — read from `SUPERVISOR_RESULT.rubric_score`** (the Supervisor Phase 4.5 Rubric Grader, which **runs even under single-iteration `/autonomous`**). **Do NOT rely on the autonomous EVALUATE rubric loop** — single-iteration short-circuits EVALUATE (`autonomous-loop/SKILL.md` §"AC-2 single-iteration short-circuit"), so its rubric gate never fires. Satisfied = **N == M**. A **null / absent `rubric_score`** (the item had no `## Outcomes Rubric`) makes this condition **N/A — NOT a blocker**. **Loop contract (load-bearing):** for a no-rubric item the loop MUST pass the literal string `"na"` into the gate's `ctx.json` `rubric_satisfied` field — **never JSON `null`/absent**. `gate-eval` treats `"na"` as N/A (merge-eligible), but a `null`/absent value coerces to `__MISSING__` and fails **CLOSED** (`PARK: rubric_unsatisfied`). The `"na" ⇒ merge-eligible` path is covered by the gate self-tests; the absent-rubric → `"na"` mapping itself lives in this loop prose, so passing `null` here would silently PARK a mergeable item (safe, but wrong).

6. **NOT a high-risk diff — re-classified at GATE time, NO override (owner decision R5).** The loop runs `classify-risk.sh main <ready_sha> --root <checkout>` (the plugin's `scripts/classify-risk.sh`, invoked the same way as `automate-helpers.sh` in §1.5) on the **fetched PR head it is about to judge** (the same `ready_sha` cond 2 pins — after `git fetch`, so the ref resolves) and passes the script's `.high_risk` and `.reasons` into the gate `ctx.json` as `"high_risk": true|false|null` and `"risk_reasons": [...]`. The script is the ONE implementation of the high-risk heuristic (branches a/b/c — security/financial/migration surfaces, workflow/orchestration surfaces, sheer size — plus the add-only `.agent/risk.json` project extension; `classify-risk.sh --kind-table` prints the rules and `docs/RESULT_SCHEMAS.md` §SUPERVISOR_RESULT carries the one committed copy — not restated here). `gate-eval` reads `high_risk` in EXACTLY cond 3's `unresolved_human_thread` fail-CLOSED shape (`has()` + `type == "boolean"`, MERGE only on the JSON boolean `false`): `true`, `null` (unclassifiable — bad ref, git or jq failure), a missing key, or ANY string — including `"false"` — all park as `PARK: high_risk_diff (<first 3 reasons; "; ">)`. **Nothing overrides this condition** — not `--trust-unprotected` (cond 4 only), not a config key, not an `exclude` list (`.agent/risk.json` has none; projects may only ADD surfaces).
   - **Loop contract (load-bearing):** the loop MUST re-run the script itself at GATE time — it MUST NOT copy `SUPERVISOR_RESULT.risk_classification` (Phase 4.5 classified `$BASE_BRANCH...HEAD` at an earlier point; the drain may have pushed since). Record the Supervisor's value beside the gate's in `## Progress` (`risk: supervisor=<true|false|null|absent> gate=<true|false|null>`) for comparison only. Pass the script's `.high_risk` straight through as the JSON boolean it is (`jq` passthrough, never `tostring` / never re-quoted — the gate parks on the string `"false"`); never coerce `null` to `false`, never omit the key. A `PARK: high_risk_diff` reaches the existing park+notify path unchanged: `pause_reason: awaiting_merge`, and the `--notify` gate line carries the quoted reasons so the human sees WHY it parked.
