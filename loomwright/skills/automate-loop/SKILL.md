...
**Two independent not-ready skips exist for the folder / backlog-dir-fallback sources (harness-port/02):** (1) the pre-existing `proposed/` **subfolder** convention — `"$dir"/*.md` does not recurse, so a file kept inside a `proposed/` subfolder (as `/propose` writes, and `commands/propose.md` documents) is simply never scanned; and (2) a `## Status: proposed` or `## Status: parked` **heading stamp** on a file that IS scanned — this file is read, its stamp checked, and it is then skipped (excluded from the Queue), the same way a `## Status: done` file is excluded. Either way, a skipped file is **not checked off** in `## Queue` and does **not** count toward the run's own `## Status: done` — it simply never entered the Queue in the first place. (Run-file resume semantics — `resume-glob` over `.supervisor/automate/*.md`, whose `## Status:` vocabulary is `running|paused|done` — are untouched by this stamp; see §4.)
