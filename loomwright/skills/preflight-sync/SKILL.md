...
4. **Classify CLEAR | OVERLAP | SUPERSEDED** using these required signals:
   ...
   - **(c) brief-staleness churn (ADVISORY ONLY, v15.19.0 — 4g(b)) — does NOT classify:** when the `job:` brief's `## Environment` section carries a `- **Base commit:** {sha}` line (stamped by Launch Pad Phase 5 PACKAGE step 3a, see `agents/launch-pad.md`), compute churn over the SAME anticipated file set already derived in action 3, from that stamp forward to the current base tip:
     ```bash
