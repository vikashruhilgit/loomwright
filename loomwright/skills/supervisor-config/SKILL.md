...
2. Check for resume state:
   ...
   - **Resume validation gate (fail-closed — runs BEFORE any loaded state is consumed):** when a state file WAS loaded (from either location), validate it against `skills/state-management/SKILL.md` §"Resume validation gate" (the authoritative contract) before reading any field out of it or acting on its content:
     ...
     d. **Ground-truth reconciliation (v15.15.0):** run `bash "${CLAUDE_PLUGIN_ROOT}/scripts/reconcile-resume-state.sh" <state_file> .` — `CLEAN` (exit 0) proceeds; `STALE` (exit 3) REFUSES with `error: "resume_state_stale"`; `UNKNOWN` (exit 4) REFUSES (fail CLOSED). **Never `|| true`** — a correctness gate, not an emitter. Checks a–c prove the file PARSES; this proves it is not LYING about completed work. A `STALE` refusal must relay the reconciler's per-subtask divergence lines and the branch holding the real work, so the operator resumes from the true position rather than re-executing merged subtasks.
