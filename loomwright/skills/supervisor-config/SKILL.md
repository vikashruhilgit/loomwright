...
2. Check for resume state:
   ...
   - **Resume validation gate (fail-closed — runs BEFORE any loaded state is consumed):** when a state file WAS loaded (from either location), validate it against `skills/state-management/SKILL.md` §"Resume validation gate" (the authoritative contract) before reading any field out of it or acting on its content:
     ...
     d. **Ground-truth reconciliation:** run `bash "${CLAUDE_PLUGIN_ROOT}/scripts/reconcile-resume-state.sh" <state_file> .` — `CLEAN` (exit 0) proceeds; `STALE` (exit 3) REFUSES with `error: "resume_state_stale"`; `UNKNOWN` (exit 4) REFUSES (fail CLOSED). **Never `|| true`** — a correctness gate, not an emitter. Checks a–c prove the file PARSES; this proves it is not LYING about completed work. A `STALE` refusal must relay the reconciler's per-subtask divergence lines and the branch holding the real work, so the operator resumes from the true position rather than re-executing merged subtasks.
     On ANY violation of a–c: REFUSE the resume. Emit `SUPERVISOR_RESULT` with `status: failed` and `error: "resume_state_invalid"`, plus a clear user message: inspect or delete `.supervisor/state.md` (and the scratchpad copy, if that is what failed), or start fresh without `--continue`. NEVER silently fall back to a fresh start — that would mask corruption. There is NO escape-hatch flag for this gate (deleting the bad state file IS the escape hatch). A MISSING state file is NOT a violation — "no state found → start fresh" is unchanged; the gate only fires on a file that loaded but does not parse against the contract.
...
5a. **Base-branch + non-interactive preamble (v14.0.0):**
   ...
   0. **Run-lock acquire (red-team-hardening/06 — FIRST action of this preamble, before any other step below).** `scripts/run-lock.sh acquire --owner "supervisor:<session_id>" --session-id "<session_id>"`, where `<session_id>` is the Claude Code runtime session id for THIS `/supervisor` invocation — known at session start, i.e. BEFORE this preamble derives/loads the later `state.md` `session_id` field, which is a distinct value. This mirrors `/automate` PICK's own acquire call exactly (`skills/automate-loop/SKILL.md` §6 step 1: `run-lock.sh acquire --owner "automate:<run_id>" --session-id "<session_id>"`). The explicit `--session-id` populates the lock's meta `session_id` field, which is what lets `close-stranded-run.sh` (crash recovery — matches by `session_id`, not `owner`) release a stranded lock; omitting it leaves that field empty and defeats crash recovery for this acquirer. A lock already held by another run/process in this checkout PARKS Phase 0 immediately: emit `SUPERVISOR_RESULT` with `status: failed`, `error: "run_lock_held"`, quoting the printed `run_lock_held owner=<label> pid=<pid> age=<s>` line verbatim — the run NEVER proceeds past this point silently. A `run_lock_reentrant owner=<outer> …` line (exit 0) is NOT a park: the lock is held by the `/automate`/`/autonomous` run that inlined this one in the same session — proceed; this run's own completion-tail release is then a no-op and the outer run frees the lock. On success, the lock is held for the rest of the session and released at the Phase 4.5 completion tail (or any Phase 4 abort exit) — see `agents/supervisor.md` Phase 0 for the short phase-stanza mirror of this step and `docs/ARCHITECTURE_CONTRACTS.md` §"Run lock" for the shared `run-lock.sh` contract (also used by `/automate` PICK and `/autonomous` INIT).
   0.5. **`--max-tokens N` token ceiling (red-team-hardening/06).** Parse `--max-tokens N` from argv (mirrors `--cheap`'s flag-vs-forwarded resolution — it arrives either directly or forwarded by `/autonomous`/`/automate`, unconditional-when-present). Default: unset (no ceiling — opt-in, byte-identical behavior). Record `MAX_TOKENS` in session memory AND as a Phase Flag (same re-read-after-context-loss convention as `NON_INTERACTIVE`, step 2 below) so Phase 3 can re-read it after a context-summarization round-trip. Consumed by the Phase 3 poll loop — see `agents/supervisor.md` Phase 3 "per-iteration token-ceiling check".
   ...
   2.9a. Parse `--voter-provider <name>[:<model>]` from argv (only meaningful when `MULTI_VOTER_HEAL == true`, resolved in 2.9 above; flag-vs-config precedence mirrors 2.6a). If the flag is present, record `VOTER_PROVIDER = <its value>` (flag wins over config). Else if `.supervisor/config.json` `.voter_provider` (string) is present and non-empty, record `VOTER_PROVIDER = <config value>`. Else (both absent), record `VOTER_PROVIDER = ""` (the DEFAULT — absent-both keeps the existing behavior unchanged). Consumed by the Supervisor's Phase 4.5 **Multi-voter verification** step (`skills/self-heal-advisory/SKILL.md` Part 2 §"Multi-voter verification"): `VOTER_PROVIDER == ""` spawns the `loomwright:red-team-reviewer` Task exactly as before (byte-identical default); `VOTER_PROVIDER != ""` (e.g. `cursor:gpt-5`) instead shells out via Bash to `${CLAUDE_PLUGIN_ROOT}/scripts/adapters/providers/lens-run.sh --provider "$VOTER_PROVIDER" --role review ...`, normalizing that CLI's findings into the same `issues[]` shape the merge rule consumes. The refute check, merge rule, and `heal_decision` derivation are UNCHANGED by which branch ran — this only changes WHICH CLI produced the second opinion, never the gate shape.
