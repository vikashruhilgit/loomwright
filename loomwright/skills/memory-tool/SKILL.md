...
The plugin's **file-based memory** lives at `.claude/agent-memory/{agent-id}/` (typically a `MEMORY.md` plus sibling notes), is markdown-only, and is **pre-loaded into the agent's context at spawn time** via the `memory: project` frontmatter field. Six agents already use it — Launch Pad, Code Reviewer, Red Team Reviewer, Product Owner, QA Strategist, QA Executor — see `CLAUDE.md` §"Persistent Memory". Four are loomwright's permanently; the two QA agents ship in loomwright today and move to the companion `selvedge` plugin, after which "in this plugin" no longer covers all six. The agent does not issue tool calls to read it; the plugin loader pulls the directory into the prompt automatically. **It does not write it either.** Since v15.33.0 the store has a SOLE WRITER — `scripts/write-agent-memory.sh` — and `memory: project` agents may NOT write their own store directly: they write a proposal to the gitignored `.supervisor/agent-memory-proposals/` (surprise-only) and a human promotes it through that writer under `--confirm`. See `AGENT_GUIDELINES.md` §"Agent memory write permission". **`MEMORY.md` in particular is writer-OWNED**: `rebuild_memory_index` regenerates it from the entry files on every write, so a hand-written index is unvalidated and is silently replaced on the next write.
...
- Building inside this Claude Code plugin? Use `memory: project` in the agent frontmatter, and write entries through the sole writer — `bash "${CLAUDE_PLUGIN_ROOT}/scripts/write-agent-memory.sh" <agent-slug> <entry-slug> <summary> <body-file> --confirm` — which validates before it writes and owns `MEMORY.md`. Never hand-edit `MEMORY.md`; it is regenerated on every write. Don't add the Memory Tool — the plugin host doesn't propagate it to subagents anyway, and the pre-loaded markdown convention is the established path for the six memory-using agents.
...
- `.claude/agent-memory/` — the convention for **Claude Code plugin agents**. Pre-loaded into the prompt, written ONLY via the sole writer `scripts/write-agent-memory.sh` (agents propose, a human promotes — never `Edit`/`Write` by hand), lives at `.claude/agent-memory/{agent-id}/` in the project. This is the path for any agent in this plugin.
...
- Six agents using file-based memory: Launch Pad, Code Reviewer, Red Team Reviewer, Product Owner, QA Strategist, QA Executor (the last two move to the `selvedge` plugin)
