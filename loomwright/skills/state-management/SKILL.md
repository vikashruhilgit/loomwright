...
**Readers of this table must be shape-tolerant, and must omit rather than guess.** The row above is what a writer emits; it is not the only shape a reader meets. `.supervisor/history/*.md` are literal `cp` copies of `state.md` (Phase 4 FINALIZE), so every shape in the archive is a `state.md` shape — including `| ID | Title | Status | Depends On | Files |` and `| # | Title | Status | Review |`, with ids spelled `1`, `subtask_1`, `ST1` and `BD-1.2`, statuses in either case, and statuses annotated rather than bare (`COMPLETED (36c39de)`, `DONE (review PASS)`, `COMPLETED WITH ESCALATION`). What holds across all of them, and is the contract to key on, is that **ID is data column 1 and Status is data column 3**, with the status keyword the FIRST TOKEN matched case-insensitively. `reconcile-resume-state.sh` is the reference implementation and the single authority for the keyword set. A reader that keys on the id cell's *shape* instead silently drops whole tables; one that substitutes an empty status for a missing column prints a wrong count instead of none. `status-line.sh` reuses reconcile's vocabulary as a declared partition — `FAILED`/`SKIPPED`/`ABANDONED` are terminal for a resume gate but must never score as progress — and `test-status-line.sh` §(h) extracts reconcile's keyword arm at test time so the partition cannot drift.
