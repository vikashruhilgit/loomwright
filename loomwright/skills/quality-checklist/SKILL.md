...
- [ ] **Deviations conformance (`deviations` — Supervisor Phase 4.5 only).** When the spawn prompt carries worker- or fixer-recorded plan drift (the DEVIATIONS ADVISORY line, `skills/self-heal-advisory/SKILL.md` step 1g), each listed deviation is checked against the brief's stated acceptance criteria and `## Outcomes Rubric` bullets: a deviation that CONTRADICTS one ⇒ one `new` finding quoting both the deviation and the contradicted criterion/bullet; a deviation that is merely unexpected (not a contradiction) is NOT a finding. Class signal: a `plan:`/`edge:`/`open:`/`test:` (or unprefixed `other:`) entry whose stated departure conflicts with what the brief asked for. Absent the line (no `state.md` deviations recorded this run), the class does not apply.
