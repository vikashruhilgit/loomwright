...
- `status: pass` requires zero failing checks (and ≥1 check executed); `status: advisory_failures` requires ≥1 failing check and a non-empty `findings[]` (each `severity: advisory`); `status: unverified` is the fail-safe tooling path (no `jq`, or checks resolved but none could be verified — e.g. only deferred `qa-executor` checks; that kind stays deferred, and therefore stays on this fail-safe path, after the QA agents move to the `selvedge` plugin).
- `qa-executor:` checks are recognized but DEFERRED to M2b slice 1b — the runner records them `unverified` (reason `qa_executor_dispatch_deferred_m2b_1b`) and they never block a `pass`. The kind name is **reserved and kept** across the QA move to the `selvedge` plugin; its eventual dispatch target is `selvedge:selvedge:qa-executor` when selvedge is installed (decision anchor: `scripts/run-ground-truth.sh`'s header block). Because it dispatches nothing today, it stays **outside** the trust boundary described next. **Trust boundary (not a sandbox):** the runner itself does no repo writes and no network, but a `cmd:` check runs arbitrary `bash -c` with full shell privileges — so a `## Executable Acceptance` `cmd:` bullet is a trust-sensitive surface (review it at Plan Review, especially for `/autonomous`-generated briefs). `corpus-task` ids are constrained to a single path segment so they cannot escape `eval-corpus`.
