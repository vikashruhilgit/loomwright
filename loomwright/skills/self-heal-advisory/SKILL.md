...
#   1. no `brief_path` — a direct `/supervisor task:"..."` invocation without Launch Pad (the same
#      back-compat case the completion tail's "job file is not in `in-progress/`" rule handles);
