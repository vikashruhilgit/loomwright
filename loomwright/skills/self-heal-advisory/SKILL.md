...
4. **Record the outcome:** `record_decision(phase: SELF_HEAL, decision: "{PASS|ESCALATED|loop_skipped}", rationale: "{final reason}")`.
   - **Terminal status is derived, not flipped here.** `scripts/build-state.sh` derives `.supervisor/state.md`'s `- status:` (`completed` / `completed_with_escalation` / `failed`) from the run's `session_end` event — see `docs/TELEMETRY.md`. **Documented residual (Risk R4):** `session_end` is itself agent-emitted, so a missed emission leaves `state.md` at `running`. Degradation is graceful (a stale `running` over-reports activity rather than hiding merged work); do not close this by re-adding a status-flip instruction here.
