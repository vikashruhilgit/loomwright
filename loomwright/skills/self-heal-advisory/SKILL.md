...
**Gate shape UNCHANGED (invariants):** `heal_decision` semantics (PASS only from a code-reviewer PASS; ESCALATED on NEEDS_HUMAN / max iterations / thrash), the `--heal-iterations` bound, never-merge, the completion-tail guard, and the completion-tail procedure below are all IDENTICAL to the single-voter path — multi-voter only edits the `fixable_issues` set inside each iteration. Edge rules:
