...
Triggers: loomwright/agents/code-reviewer.md
Scope expanded: loomwright/commands/code-reviewer.md, loomwright/.claude-plugin/plugin.json, .claude-plugin/marketplace.json, CLAUDE.md, README.md
...
CODE_REVIEW_RESULT:
  ...
  decision: PASS
  issues: []
  summary: Repo-consistency audit passed for loomwright/agents/code-reviewer.md change.
```
...
<!-- thin-wrapper: canonical prompt lives in loomwright/agents/code-reviewer.md -->

## Agent Prompt

The canonical prompt lives in `loomwright/agents/code-reviewer.md`. This command file is intentionally a thin wrapper — all review policy (review modes, scope expansion, repo consistency audit, severity rules, output schema, decision matrix) is defined there. Do not re-embed `## Role:` or `## Quality Checklist` sections here; the sync check (`scripts/check-command-sync.sh`) will fail.
