...
3. **Then check for the WARNING block — it is an ADDITIONAL line, not one of the three above.** `apply: WARNING — readiness is '<verdict>', not 'configured'` is printed AFTER the `applied` / `no-op` headline (both paths run the same post-write verify), so it CO-OCCURS with it rather than replacing it. **Report BOTH lines** — reporting only the headline would silently drop the warning and present a dead negation as a success. It means the block WAS written but the negation did not take effect; the helper names each intended path still ignored and the rule that wins for it (from `git check-ignore -v`). Relay that verbatim too and do NOT present the apply as a success; the usual cause is a deeper or parent-`.gitignore` exclude the rewriter does not neutralise.
