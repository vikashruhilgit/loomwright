...
2. Read its status line and report it verbatim:
   - `apply: applied` — the managed block was written; a timestamped `.gitignore.backup.<ts>` sits beside it (pid-suffixed if that name was already taken, so a backup never overwrites another), and the allowlist was seeded into `.supervisor/config.json` as a JSON ARRAY.
   - `apply: no-op — already configured` — nothing was written (idempotent second run).
   - `apply: WARNING — readiness is '<verdict>', not 'configured'` — the block WAS written but the negation did not take effect. The helper names each intended path still ignored and the rule that wins for it (from `git check-ignore -v`). Report it verbatim and do NOT present the apply as a success; the usual cause is a deeper or parent-`.gitignore` exclude the rewriter does not neutralise.
