...
5. **Relay the `open:` URL whole — it carries `#token=<this run's token>` — and say to open that one, not a bare address.** The token is per-run, travels only in the fragment (which a browser never sends to a server), and is what the page's four buttons must present. Never invent, echo back or store it. Extra fail-safe branch: `serve: ABORTED — could not mint a per-run access token …` — nothing was started. Why the guard exists at all is in `docs/FLOOR_UI.md` §"Why the guard exists".
