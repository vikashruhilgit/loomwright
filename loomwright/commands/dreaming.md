...
1. **Pre-flight, all of which abort delivery cleanly rather than proceeding:** `git status --porcelain -- .agent/rules` must be empty (otherwise `/dreaming` would commit edits it did not author — report and stop); the current branch is recorded so it can be restored; `gh` must be authenticated. Nothing has changed yet at this point.
...
5. `git push -u origin dreaming/rules-<session_id>`, then `gh pr create --base "$(gh repo view --json defaultBranchRef -q .defaultBranchRef.name)" --head dreaming/rules-<session_id>` with a body naming the harvest numbers and the motivating finding ids. **Resolve the base branch, never assume `main`** — it is per-repo.
...
- **Git and the remote have exactly ONE reachable action, behind two gates.** `/dreaming` creates a branch, a commit, a push and a pull request **only** for a harvested `.agent/rules/` batch, and only for proposals that cleared **both** the per-item Accept **and** the separate per-item **Pre-push confirmation**. That commit contains `.agent/rules/*.json` and nothing else — staged by path, never `git add -A` — and delivery is refused outright if `.agent/rules/` already carries uncommitted changes. If the confirmed set is empty, **no git command runs at all**. `/dreaming` **never merges**, never force-pushes, and never resets or deletes a branch that carries commits.
...
- **Harvested `.agent/rules/` batch** — 4 proposed rules (coverage 74/107, dedupe 18.50 findings/rule, scope fidelity 98%).
