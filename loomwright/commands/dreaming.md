...
- **Harvested `.agent/rules/` batch** — 4 proposed rules (coverage 74/107, dedupe 18.50 findings/rule, scope fidelity 98% of the 55 CHECKABLE findings / 73% over all 74 motivating findings — both figures, never the flattering one alone). One of the four carries a `check_candidate: ! grep -rnE '19' src/a/*` line (≥3 of its findings share that literal) — DATA only, never executed here.
  Each rule takes **two** gates: content (**Accept**, or **Accept rule WITH check** when a candidate was shown), then a separate **Pre-push confirmation** (push). Only rules clearing both are committed to `dreaming/rules-<session_id>` and delivered by `gh pr create` — a rule accepted WITH check is committed with that exact string as its `check`, still `enforcement: advisory`, and it still does not RUN anywhere until a human separately runs `/rules check --confirm` on their own machine (see `skills/rules/SKILL.md` §8.1). Nothing confirmed ⇒ no branch, no PR. `/dreaming` never merges the PR it opens.
```
