...
**`check_candidate` — shown, never pre-selected.** When ≥3 of a proposal's supporting findings share one literal, grep-able token or path in their evidence, the harvester ALSO prints a `check_candidate: <string>` line beside `check: null` — DATA, exactly like the rest of the proposal: never executed by the harvester and never passed to `add-rule.sh` on the plain `Accept` path. Relay it to the user **verbatim, in a code span**, alongside the rest of the proposal. A candidate is only ever proposed for a SCOPED proposal (never for `applies_to: null` — a repo-wide `grep -r .` would scan `.git/`, the `.supervisor/` ledger the token was harvested from, and the rules store itself) and only when its pattern is a valid ERE over readable paths; the harvester then also prints an `invocation (Accept rule WITH check):` line, composed with `--enforcement must` (see delivery step 3). A proposal with no `check_candidate` line behaves exactly as before this feature existed — the option set below only grows a THIRD choice when a candidate is actually present, and picking it is never the default (see the Per-class option sets table and "The one exclusion that is load-bearing" below — neither gate on a harvested rule ever carries a `(Recommended)` marker, and that includes this third option).
...
1. **Accept / Accept rule WITH check / Reject / Edit (per item)** — the ordinary content gate: "is this rule's statement and scope right — and, if a `check_candidate` was shown, should it become the rule's `check`?". `Accept rule WITH check` **only ever appears when the proposal carried a `check_candidate` line**; a proposal with none offers the original three-option set, byte-unchanged. Both `Accept` variants **write nothing and create nothing** here — content-approval is still a marker, not a write; the composed call happens only at delivery (step 3 below). `Reject` and `Edit` write nothing and never reach the branch, preserving the same per-item guarantee every other destination in this command already has. The candidate is shown **verbatim, in a code span**, alongside the proposal — never paraphrased, never re-typed — so the human is confirming the exact string that will become `check`.
2. **Pre-push confirmation (per item, only for content-approved rules)** — a separate, explicit question: "**push this rule to the remote and open a PR?**". This is *not* implied by Accept, and Accept is not implied by it. Only rules that clear **both** enter the *confirmed set*. Answering no to the Pre-push confirmation leaves that rule unwritten and unpushed, with no other effect. **Default to NOT pushing. If the answer is ambiguous, treat it as no** — the item stays out of the confirmed set, exactly as `/setup rules` treats an ambiguous seed Offer as Cancel (`commands/setup.md`, the rules-module Offer). This gate is the safety property the whole feature is described by, and a gate that resolves an unclear answer *toward* the remote-affecting action is not a gate. Ambiguity is a reason to ask again or to skip the item, never a reason to push; only an unambiguous yes advances it. For the same reason **neither rules gate carries a `(Recommended)` marker at all** — not on the push-ward option, and not on `Do not push` either, which is a default rather than an observation and is expressed by option order instead — see "The one exclusion that is load-bearing" under "Approval Option Contract" below.
...
3. For each rule in the confirmed set, run the invocation the harvester already printed, **with `--confirm` appended and the stdin redirection kept** — the plain `invocation:` line for `Accept`, and, **only for a rule where the user picked `Accept rule WITH check`, the separate `invocation (Accept rule WITH check):` line instead**. That second line differs in exactly two places: it composes **`--enforcement must`** (not `advisory`) and ends in the literal `--check "$CANDIDATE"`. The `must` is load-bearing: `rules-check.sh` selects ONLY `enforcement == "must"` rules, so a rule accepted WITH its check but stored `advisory` could never run under `--confirm` or `--if-stamped`. `$CANDIDATE` holds the `check_candidate` string shown VERBATIM (never re-typed, never re-derived — the exact bytes the harvester printed and the exact bytes the user confirmed at gate 1). **Never splice the candidate into the command line inside quotes of your own** — every candidate contains single quotes, and the bound template also contains `"$(`, so `'<candidate>'` breaks out of its quoting and `"<candidate>"` would EXECUTE the embedded `$(grep …)` at delivery time. Load it through a **quoted** heredoc, which passes every byte literally (no expansion, no quote processing; a candidate never contains a newline, the harvester rejects one), and pass the variable double-quoted (the heredoc body line and its terminator start at column 0 — no list indentation, or the stored `check` gains leading spaces):

   ```bash
   CANDIDATE="$(cat <<'LOOMWRIGHT_CHECK_CANDIDATE'
   <the check_candidate string, pasted verbatim on this one line>
   ...
   ```bash
   # Accept (plain) — the `invocation:` line:
   ...
   # Accept rule WITH check — the `invocation (Accept rule WITH check):` line:
   bash "${CLAUDE_PLUGIN_ROOT}/scripts/add-rule.sh" --category '<cat>' --statement '<text>' \
     --enforcement must --applies-to '<glob>' [--applies-to '<glob>' …] \
     --source 'dreaming:<session_id>' --check "$CANDIDATE" --confirm < /dev/null
   ```

   `add-rule.sh` itself is **byte-unchanged** by this feature — it has always accepted `--check` (see `skills/rules/SKILL.md` §8.1 / `commands/rules.md` §"Stamp semantics" for why the resulting `must` rule's `check` still does not RUN anywhere until a human separately runs `/rules check --confirm` on their own machine — after which Phase 4.5's `--if-stamped` replay runs it). The `< /dev/null` stays even with `--confirm`: it costs nothing and it means **no writer call on this path can ever prompt at your terminal**. Pass every value as a **literal argv argument, never interpolated into a shell string** — the same argv discipline the other writers require, and `--check`'s value is no exception: it is the candidate string verbatim, never composed or concatenated. `add-rule.sh` is the store's confirm-gated sole writer, so its category containment, `applies_to` validation, array-only parse-gate, atomic write and read-back verify all apply — **including to a supplied `--check` value**, which is stored as an opaque string and never itself executed by the writer. **A writer refusal is not fatal to the batch**: report it, skip that rule, and continue with the rest.
...
- `header` — ≤12 chars naming the item class: `Memory` (§3 / §5 / agent-memory queue — same underlying store), `CLAUDE.md` (§4), `Lesson` (§6), `Orientation` (orientation queue), `Rule`, `Push`. Every row of the table below maps to one of these; a row with no header is the gap this list has already had twice.
...
| harvested rule — gate 1 (content), no `check_candidate` shown | `Accept` · `Reject` · `Edit` |
| harvested rule — gate 1 (content), a `check_candidate` WAS shown (executable-rule-candidates/01) | `Accept` · `Accept rule WITH check` · `Reject` · `Edit` (4 options — at, never over, the cap) |
...
| Item class | Recommend the named option iff | Otherwise |
|---|---|---|
...
| §5 collected candidate | evidence spans **≥2 distinct sessions**. (No dedupe clause: §5's own spec drops any candidate already present in project memory *before* listing it, so surviving the dedupe is a precondition of appearing at all — asserting it here would be a condition that can never be false) | abstain |
...
| harvested rule — **all three** gate-1/gate-2 choices (`Accept`; `Accept rule WITH check`; gate 2 `Do not push`) | **never** — including `Do not push`. There is no evidence axis to cite for any of the three: choosing to run a `check_candidate` at all is a judgment about *executable* trust distinct from content approval, not-pushing is a *default*, and this contract requires a **basis**, not a preference. The default is already expressed by `Do not push` holding first position in the option set (see the exclusion below); `Accept rule WITH check` never holds first position either — it is always ADDED after the plain `Accept`, never in front of it | abstain |
...
**Neither harvested-rule gate may ever carry a recommendation *toward* the remote-affecting action.** Gate 2's stated contract is *default to NOT pushing; an ambiguous answer is a no* (see "Two distinct gates, and the ordering is the safety property" above) — a `(Recommended)` on `Push and open a PR` would resolve a safety gate toward the action it exists to slow down. Gate 1 is excluded too, for a different reason: the whole point of the batch's two scope-fidelity figures is that the user reads **both**, and a recommendation substitutes for that reading.

**`Do not push` gets no marker either**, and the reason is worth stating because the opposite reading is tempting: it is the *safe* direction, so a marker on it would do no harm. But this contract's own rule is that a marker must cite a **basis**, and not-pushing has none to cite — it is a default, not an observation. The safe direction is already carried by **option order**: `Do not push` holds first position in gate 2's set, which is how a default is expressed here without borrowing the vocabulary of evidence. A marker that means "this is the default" and a marker that means "the evidence supports this" must not look identical, or the second one stops meaning anything.
...
- **Harvested `.agent/rules/` batch** — 4 proposed rules (coverage 74/107, dedupe 18.50 findings/rule, scope fidelity 98% of the 55 CHECKABLE findings / 73% over all 74 motivating findings — both figures, never the flattering one alone). One of the four carries a `check_candidate: grep -rnE -- '19' src/a/*; test $? -eq 1` line (≥3 of its findings share that literal) — DATA only, never executed here.
  Each rule takes **two** gates: content (**Accept**, or **Accept rule WITH check** when a candidate was shown), then a separate **Pre-push confirmation** (push). Only rules clearing both are committed to `dreaming/rules-<session_id>` and delivered by `gh pr create` — a rule accepted WITH check is committed with that exact string as its `check` and **`enforcement: must`** (the only enforcement `rules-check.sh` selects), and it still does not RUN anywhere until a human separately runs `/rules check --confirm` on their own machine (see `skills/rules/SKILL.md` §8.1). Nothing confirmed ⇒ no branch, no PR. `/dreaming` never merges the PR it opens.
```
