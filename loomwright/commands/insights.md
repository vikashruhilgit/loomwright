...
A **bounded** summary of the READ-ONLY funnel builder **`scripts/build-loop-evidence.sh`** — the per-run unattended-quality funnel **landed → clean → durable → cheap**, computed from existing `.supervisor/` data plus a read-only `git log` ("clean" applies the false-zero rule: drain-internal fix signals count against a PR even when it shows 0 GitHub review rounds). The dashboard embeds ONLY the **era-buckets table** (runs grouped by advisory-surface eras: rules seams ≥15.1.0, orientation memos ≥15.12.0) plus a one-line **funnel headline** (e.g. "N of M runs verifiably clean") — the full per-run table (dozens of rows) is deliberately **not inlined** so the dashboard stays skimmable; a pointer line tells you to run `bash "${CLAUDE_PLUGIN_ROOT}/scripts/build-loop-evidence.sh"` directly for the per-run detail (add `--jsonl` for machine-readable records).
