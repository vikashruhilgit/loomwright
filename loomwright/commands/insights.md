...
2. Writes **`.supervisor/insights/dashboard.md`** — Summary (sessions, completed/failed, completion rate, self-heal PASS count, avg heal iterations, total subtasks/files), an optional **System Twin hard-signal** section (contract-conformance + benchmark trend; see below), a **Per-version insights** section (runs / heal-PASS rate / avg heal iterations / avg rubric score, grouped by the additive `plugin_version` stamp; see below), a **Knowledge sources** section (which memory sources runs reported consulting; suppressed when none; see below), an **Eval fitness function** section, a **Heal-signal catch-rate (MEASURE)** section (the self-heal confusion-matrix trend; suppressed when no data; see below), a **Loop evidence (unattended-quality funnel)** section (bounded summary of `scripts/build-loop-evidence.sh` — era-buckets table + funnel headline, "no data" note when absent; see below), a **Missing-drain reconciliation** section (heal-outcome PRs with no review-drain dispatch marker; suppressed when no corpus), a **Brain Context Baseline** section (dormant in v1 — intentionally suppressed until activated), a **System Twin growth** section, a Cost note (ccusage $), an advisory **Token economics** section (`token_ledger` real vs proxy), a Recent-sessions table (with a Twin conformance/Δ column), a **Corpus health** section, and an Obsidian/Dataview snippet.
...
### Loop evidence (unattended-quality funnel)

A **bounded** summary of the READ-ONLY funnel builder **`scripts/build-loop-evidence.sh`** — the per-run unattended-quality funnel **landed → clean → durable → cheap**, computed from existing `.supervisor/` data plus a read-only `git log` ("clean" applies the false-zero rule: drain-internal fix signals count against a PR even when it shows 0 GitHub review rounds). The dashboard embeds ONLY the **era-buckets table** (runs grouped by advisory-surface eras: rules seams ≥15.1.0, orientation memos ≥15.12.0) plus a one-line **funnel headline** (e.g. "N of M runs verifiably clean (K of L measured runs — runs with review data)", where "measured" = runs whose clean stage isn't an `insufficient_data(...)` label) — the full per-run table (dozens of rows) is deliberately **not inlined** so the dashboard stays skimmable; a pointer line tells you to run `bash "${CLAUDE_PLUGIN_ROOT}/scripts/build-loop-evidence.sh"` directly for the per-run detail (add `--jsonl` for machine-readable records).

**Advisory only** — the builder never gates anything and always exits 0, and the dashboard section mirrors that: the invocation is guarded so the dashboard build can **never** fail because of it. It is **always rendered** (the Eval-fitness / Token-economics "degrade to a note" convention, not full suppression): when the builder is missing or emits no usable run records, the section shows a one-line "no data" note. Re-run `/insights` any time to refresh it (the summary is recomputed from the builder's live output — there is no separate ledger to maintain).
...
- `scripts/build-loop-evidence.sh` — the READ-ONLY unattended-quality funnel builder feeding the **Loop evidence** section (run it directly for the full per-run table the dashboard deliberately does not inline).
