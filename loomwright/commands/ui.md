...
Read-only. Reports **three sections in one report**: the module half (ui dir, bundle `present (3 files)` / `INCOMPLETE — …`, the `python3:` / `jq:` / `build-floor.sh:` availability lines, the `installed:` cell, and the `UI readiness:` verdict `configured` / `not configured` / `stale (run apply)` / `WITHHELD (…)`), then a `== server ==` section, then a `== projects ==` section with the registry path and its state.

**The `== server ==` section is how a reader gets the `open:` URL back**, and relaying it is the point of running this command. `serve` prints that URL exactly once and the token in it is otherwise unrecoverable, so a reader who scrolled past it could not reach the four buttons again. Two states, and they are different claims:

- `server: running (N process(es) …)` followed by an `open:` line — **relay the URL whole**, exactly as under `serve`. Liveness is proved from the process table using `stop`'s own rule (a pid whose command line names one of ours), never from the pidfile merely existing, because a URL whose server is gone is worse than none: the page accepts it and then reports a 403 the reader would debug as a token problem.
- `server: not running` — say so and offer `/ui serve --detach`. If it also prints a `note:` about a leftover `ui-serve.token`, relay that too: it names a server that did not stop cleanly, and the URL in it is dead. **`check` does not delete it** — a read verb that quietly repairs state is a read verb you can no longer trust; `serve` overwrites it and `stop` removes it.
- `open: UNKNOWN` — the server is up but its URL could not be recovered. Say plainly that the page will still READ at a bare address and that the four buttons stay refused until the server is restarted.
...
- **The `open:` URL also carries `#token=<this run's token>`. Relay it whole and say to open THAT url, not a bare one.** The token is per-run, lives only in the fragment (which a browser never sends to any server), and is what the page's four buttons — add, forget, scan, stop — must present to be accepted. A bare `http://127.0.0.1:<port>/` still renders everything; only the buttons are refused, and the page says so. Never invent or echo back the token. It dies with the server; `serve`'s output and `check`'s `== server ==` section are the only two places it is meant to appear — `serve` records that one line beside the ui directory (mode 0600, a SIBLING so the server itself can never hand it out over HTTP) precisely so `check` can give it back, and `stop` deletes it. The filename is derived from the ui directory's basename — `ui-serve.token` for the default `.../ui` — so two installs sharing a parent keep separate files. The extra fail-safe branch to relay verbatim is `serve: ABORTED — could not mint a per-run access token …` — nothing was started.
