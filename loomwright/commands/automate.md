...
/automate ... --trust-unprotected            # allow auto-merge onto a branch without enforceable protection (condition 4 ONLY — never overrides condition 6, the high-risk park)
...
/automate ... --max-tokens N                 # opt-in spend ceiling — parked BEFORE PICK when the run's summed ledger exceeds N (never a silent overspend)
...
| `--folder <dir>` | One source | — | **Folder source.** Enqueues every `.md` in `<dir>` (skipping ones already `## Status: done` or `## Status: proposed\|parked`, harness-port/02). §2. |
...
| `--auto-merge` | No | OFF (safe mode) | **Opt-in, default OFF.** At the GATE, executes `gh pr merge --squash` ONLY when the trusted-merge gate holds; otherwise fails **CLOSED** (park + notify). **Deep contract (the 6 conditions) is NOT restated here — see `skills/automate-loop/SKILL.md` §10.** **The gate is SELF-RESOLVING (red-team-hardening item 03): it computes conditions 2–6 itself from live `gh`/GraphQL/`classify-risk.sh` reads — a caller cannot hand it a verdict** (a `ctx.json` carrying a gate-owned key, e.g. `high_risk`, is refused outright: `PARK: ctx_carries_gate_owned_key`). Condition 6 re-classifies the PR head with `scripts/classify-risk.sh` and merges only on the JSON boolean `high_risk: false` — `true`, `null` (unclassifiable), a missing key, or any string (including `"false"`) parks; no override. |
| `--trust-unprotected` | No | off | The auto-merge gate escape hatch — allows merge onto a branch without enforceable branch protection. Meaningful only with `--auto-merge`. §10 condition 4 **only** — it does not override condition 6 (the `classify-risk.sh` high-risk park), and no flag, config key, or project file does. |
...
| `--max-tokens N` | No | unset (no ceiling) | **Opt-in token-spend ceiling (red-team-hardening/06).** Parsed at start and recorded in `## Run Config` (`max_tokens: N`) — unlike `--cheap`/`--notify`, this one IS persisted, so a bare `--resume` still enforces it. At the START of every PICK (§6 step 1, before RECONCILE picks a new item), the engine sums the run's ledger via `scripts/read-token-ledger.sh --run-id <runfile>` and compares TOTAL against N (`automate-helpers.sh ceiling-check`). A breach (`TOTAL > N`) OR a `LEDGER_UNREADABLE=1` reader answer ⇒ `## Status: paused`, `pause_reason: token_ceiling` — the run PARKS before picking, never a silent continue. Also acquires `.supervisor/run.lock` (`scripts/run-lock.sh acquire --owner automate:<run_id> --session-id <session_id>`) at the same PICK seam, released at item completion/park — see `--force-unlock` under `skills/automate-loop/SKILL.md` §11 for the human-only escape hatch on a held lock. **Honest limit:** the ledger counts only what a SubagentStop hook saw — never the main thread's own tokens, never CI-side (`claude-review`) spend. No dollar conversion (the plugin has no price table). §6 / §11. |

> **Run-file layout, the 6-condition trusted-merge gate, the suppress/restore config-toggle contract, and the single-open-PR invariant are defined ONCE in `skills/automate-loop/SKILL.md`** (and the run-file layout is documented as a markdown state-file contract in `docs/RESULT_SCHEMAS.md` §"AUTOMATE_RUN"). They are deliberately **not** restated in this table — reference the skill for the deep contract.
...
The scriptable steps — the config toggle, run-file atomic writes / append-only Progress / check-off, resume reconcile, and the 6-condition trusted-merge gate — are implemented + self-tested ONCE in `scripts/automate-helpers.sh`, which the loop **shells out to** (so the tested code is the executed code); the SKILL prose is the spec each subcommand conforms to (see `skills/automate-loop/SKILL.md` §1.5).
...
4. **Per-item loop.** For each `- [ ]` item: RECONCILE (+ brief-repair on a found merge) → suppress the default drain then run `/autonomous --single-iteration --requirement <path>` → own exactly ONE inline `/review-pr --until-mergeable` (restore the config toggle finally-style) → GATE (safe-mode park `awaiting_merge`, or `--auto-merge` 6-condition trusted gate) → SYNC `main` after merge (+ brief-repair) → check the item off + append `## Progress` via one atomic write. The single-open-PR invariant blocks PICK while any PR is open. (§6–§10)
5. **TERMINATE.** Queue fully resolved ⇒ `## Status: done` / `remaining: 0` (the `/loop` driver stops); `limit` reached or a park (`awaiting_merge` / `escalated` / `rate_limit`) ⇒ `## Status: paused` with the matching `pause_reason`. (§6)
...
- `skills/automate-loop/SKILL.md` — **the authoritative loop contract** (intake, single run file, RESUME, per-item loop, single drain + config-toggle, single-open-PR invariant, two modes, the 6-condition trusted auto-merge gate, termination).
