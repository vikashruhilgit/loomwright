...
| `--limit <N>` | No (queue mode only) | `5` | Caps **PROCESSED** items this invocation, **NOT** Queue size — the queue file's `## Queue` always holds the FULL resolved folder list. After N processed with items unchecked ⇒ `## Status: paused`, `pause_reason: limit_reached`. |
