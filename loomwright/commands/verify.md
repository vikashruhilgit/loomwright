...
| `--no-replay` | No | Passthrough flag, never persisted (same convention as `--notify`), forwarded to `verify-run.sh spec-replay <run_dir> --no-replay` (token-economy 07). Disables spec replay for THIS run — every AC is authored fresh, as before this item; a pure no-op with no `spec_replay` evidence line. See `skills/verify-walkthrough/SKILL.md` §2. |
