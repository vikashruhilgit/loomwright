...
7. **CI trust probe → `ci_untrusted` (ci-trust-probe-01).** After each round's settled-checks read, EVERY red required check is probed (bounded, `--max-probes`) via `scripts/ci-run-probe.sh` for narrow, evidence-based signs it never actually ran (zero steps / no runner ever assigned / a matched generic GitHub-native string — never a timestamp). A check classified `untrusted_infra` is recorded in the additive `checks_untrusted` result field, is **never** dispatched to a fix worker, and still blocks READY (its true state is UNKNOWN, not green — a `ran`/`unknown` verdict leaves the round unchanged). When the ONLY remaining READY-blockers are `untrusted_infra` checks, the loop exits `decision: ESCALATED` / `termination_reason: ci_untrusted`, naming each check + its reason and suggesting (never executing) `gh run rerun <id> --failed`. Authority: `review-heal` skill §U4 / §"READY redefinition" / §"Terminal states" — not restated here.
