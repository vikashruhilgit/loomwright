...
>
> You must be at the terminal to answer these unless you pass `--non-interactive-fallback` (required for CI / non-TTY environments — gates fail closed instead of prompting). Pass `--notify` to also POST a gate-event payload to the webhook configured via `LOOMWRIGHT_WEBHOOK_URL` (or the USER-SCOPE `~/.claude/loomwright/egress.json` entry for this repo's slug — v15.87.0, red-team-hardening item 02; a repo-relative `.supervisor/config.json`/`.supervisor/notify-config.json` `.webhook_url` is read only as an informational request, never a grant) so an out-of-band notifier (Slack relay, ntfy/push, etc.) can ping you when a gate fires; if no URL is resolvable, `--notify` warns once at INIT rather than silently doing nothing (v14.2.2).
>
...
/autonomous "<...>" --cheap                                              # forward the Sonnet cost profile to every inlined /supervisor run (v15.2.0+)
/autonomous "<...>" --max-tokens N                                       # opt-in spend ceiling — checked before each EVALUATE-time iteration, aborts rather than overspend (red-team-hardening/06)
...
> **Cost-profile note:** `--cheap` **is forwarded** (v15.2.0+) — the loop appends it to every inlined `/supervisor job:` invocation, in every iteration (see Parameters → `--cheap` and the `--cheap interaction note` below). The autonomous loop is genuinely expensive — each iteration is a full Launch Pad + full Supervisor run — so the Sonnet cost profile is a natural fit for multi-iteration runs. For profile semantics and the Haiku-session caveat, see `docs/ARCHITECTURE_CONTRACTS.md` §"Cost Profiles".

> **Terminology note:** an **iteration** (this command) ≈ one `/launch-pad` → `/supervisor` pass on the **SAME** goal; `/automate`'s **"item"** = one independent goal from a queue. `/autonomous` deepens one goal across iterations; `/automate` walks many goals one item at a time.
...
| `--single-iteration` | No | Disable multi-iter mode; run Launch Pad + Supervisor once and exit (v13-compat behavior). Equivalent to v13's "no `--allow-multi-iteration` passed" default. Default: off — multi-iteration is the v14 default. |
| `--no-stacked-branches` | No | Iter N+1 branches from `main` instead of `iterations[N].branch`. Disables stacked-PR mode; each iteration produces an independent PR off `main`. Pair with the Signal-1 rubric gate's `merge-and-continue` option for the v13 cadence. Default: off (stacked). Only meaningful in multi-iteration mode — a single-iteration run has no iteration N+1. |
| `--notify` | No | Enable webhook gate-event notifications. Requires a resolvable webhook URL. Fire-and-forget POST via `${CLAUDE_PLUGIN_ROOT}/scripts/send-webhook.sh --event-type gate ...`; the AskUserQuestion fires immediately after backgrounding the POST (no wait on delivery). The webhook URL resolves from `LOOMWRIGHT_WEBHOOK_URL` **or** the USER-SCOPE `~/.claude/loomwright/egress.json` entry for this repo's slug (v15.87.0 — a repo-relative `.supervisor/config.json` `.webhook_url` is informational only); if neither resolves, `--notify` **fails loud** with one warning at INIT step 0 (it does NOT silently no-op) — desktop banners are unaffected (v14.2.2). Default: off. |
| `--non-interactive-fallback` | No | Permit multi-iter execution in non-interactive (CI / stdin-not-tty) environments. Gates fail closed (abort with a clear status_reason) instead of firing AskUserQuestion. Required when running unattended; the INIT-time non-interactive check refuses to start multi-iter without it. **Auto-forwards `--non-interactive` to the inlined `/supervisor`** so Supervisor's Phase 4 `gh` retry path and adjudication gates fail closed consistently — a single `--non-interactive-fallback` is sufficient for CI; no need to also pass `--non-interactive` to `/autonomous` (see `skills/autonomous-loop/SKILL.md` EXECUTE step 1, "Auto-forwarded flags"). Default: off (interactive gates). |
...
| `--cheap` | No | **Forward the Sonnet cost profile (v15.2.0+).** Parsed at INIT and appended to **every** inlined `/supervisor job:` invocation at EXECUTE step 1 — unconditionally-when-present, in every iteration (single- and multi-iteration; stacked and non-stacked), NOT gated on `--non-interactive-fallback`. Supervisor's existing cost-profile engine (Sonnet overrides for orchestrator / execute-manager / worker / code-reviewer / Phase 4.5 fix tasks, and — when `--multi-voter-heal` is ON — the multi-voter verification voters/refute spawn) is reused unchanged. Default: off — absent means every role keeps the default `inherit` profile. Profile table + Haiku-session caveat: `docs/ARCHITECTURE_CONTRACTS.md` §"Cost Profiles". |
| `--max-tokens N` | No | unset (no ceiling) | **Opt-in token-spend ceiling (red-team-hardening/06).** Parsed at INIT and persisted into `state.json` (so EVALUATE can re-read it without re-parsing argv) AND forwarded to every inlined `/supervisor job:` invocation at EXECUTE step 1 (unconditional-when-present, same forwarding rule as `--cheap`). Before EACH iteration, EVALUATE sums the run's own session ledger via `scripts/read-token-ledger.sh` and compares against N; a breach OR a `LEDGER_UNREADABLE=1` reader answer ⇒ `status: aborted`, `status_reason: "token_ceiling_reached"` — the loop stops rather than start another iteration over budget. Also acquires `.supervisor/run.lock` (`scripts/run-lock.sh acquire --owner autonomous:<session_id> --session-id <session_id>` — `<session_id>` is the Claude Code runtime session id, matching `/automate` PICK's acquire shape, so a crashed session's lock is reclaimable) at INIT, released at the loop's own completion/abort tail. A `run_lock_reentrant owner=<outer> …` line (exit 0) is NOT an abort: an enclosing `/automate` in the same session holds the lock — proceed; this loop's own release is then a no-op. **Honest limit:** the ledger counts only what a SubagentStop hook saw — never the main thread's own tokens, never CI-side spend. No dollar conversion (no price table). |

> **`--cheap` interaction note (and the complete forward set):** Since v15.2.0, `--cheap` **is forwarded** to the inlined `/supervisor` call. The loop forwards **exactly four** flags to that invocation, each under its own condition (mirrors `skills/autonomous-loop/SKILL.md` EXECUTE step 1 §"Auto-forwarded flags" — the authority; keep the two in sync): `--base-branch "$expected_base"` (only on a stacked multi-iter run when `iteration > 1` — `--no-stacked-branches` NOT active), `--non-interactive` (only when `--non-interactive-fallback` was passed at INIT), `--cheap` (unconditional-when-present, every iteration — single- and multi-iteration, stacked and non-stacked alike), and `--max-tokens N` (unconditional-when-present, red-team-hardening/06 — passthrough only, `/supervisor`'s own Phase 3 poll loop re-checks it per-iteration against its OWN ledger sum). **No other flag is forwarded.** Note that `--base-branch` and `--non-interactive` are `/supervisor` flags the loop derives or forwards internally — they are **not** `/autonomous` flags and are not passed to `/autonomous` directly. The `--cheap` forward keeps the cost profile consistent across a multi-iteration run's whole stack, and `/automate` also passes `--cheap`/`--max-tokens` through to its inner `/autonomous` call, completing the `/automate → /autonomous → /supervisor` chain for both. For the profile semantics and the Haiku-session caveat, defer to `docs/ARCHITECTURE_CONTRACTS.md` §"Cost Profiles"; for the ceiling semantics, see `docs/ARCHITECTURE_CONTRACTS.md` §"Token ceiling".

> **`--skip-preflight-sync` interaction note:** `--skip-preflight-sync` is **not forwarded** to the inlined `/supervisor` call — it is not in the loop's forward set (`--base-branch` / `--non-interactive` / `--cheap` / `--max-tokens`; see EXECUTE step 1 "Auto-forwarded flags"). The Supervisor's Phase 1.5 PRE-FLIGHT SYNC gate therefore runs in **every** autonomous iteration — normally invisible because the CLEAR path is silent; under `--non-interactive-fallback` an OVERLAP/SUPERSEDED classification fails the iteration closed with `status_reason: "preflight_overlap_detected"`. To deliberately skip the gate for a one-off, run `/supervisor --skip-preflight-sync` manually.
