...
| `--cheap` | No | **Forward the Sonnet cost profile (v15.2.0+).** Parsed at INIT and appended to **every** inlined `/supervisor job:` invocation at EXECUTE step 1 — unconditionally-when-present, in every iteration (single- and multi-iteration; stacked and non-stacked), NOT gated on `--non-interactive-fallback`. Supervisor's existing cost-profile engine (Sonnet overrides for orchestrator / execute-manager / worker / code-reviewer / Phase 4.5 fix tasks) is reused unchanged. Profile table + Haiku-session caveat: `docs/ARCHITECTURE_CONTRACTS.md` §"Cost Profiles". |

> **`--cheap` interaction note:** Since v15.2.0, `--cheap` **is forwarded** to the inlined `/supervisor` call — it is part of the loop's forward set alongside `--base-branch` and `--non-interactive` (see `skills/autonomous-loop/SKILL.md` EXECUTE step 1 §"Auto-forwarded flags"). The forward is unconditional-when-present and applies to every iteration, so a multi-iteration run keeps the cost profile consistent across its whole stack. `/automate` also passes `--cheap` through to its inner `/autonomous` call, completing the `/automate → /autonomous → /supervisor` chain. For the profile semantics and the Haiku-session caveat, defer to `docs/ARCHITECTURE_CONTRACTS.md` §"Cost Profiles".
