...
/verify <ticket-path> --notify                # POST gate-event webhooks + a desktop banner at needs_auth, first FAIL, and run_end
...
5. On a `completed`/`aborted` run with ≥1 recorded `FAIL` **or** ≥1 `issue` line, auto-dispatches `scripts/propose-from-verify.sh <run_dir>` (same as `/propose --from-verify <run_id>`) — one evidence-carrying draft per `FAIL` `ac` line classified `REAL_BUG` and per `issue` line, written to `.supervisor/requirements/proposed/`, never enqueued; never on a paused run

**Notify (`--notify`).** A passthrough flag, never persisted (re-pass it on `--resume` too) — fires `scripts/send-webhook.sh --event-type gate` at exactly THREE events (`needs_auth` pause, the first `FAIL`, `run_end`), each guarded to fire at most once per run, plus `scripts/notify-desktop.sh` at `needs_auth`. Dispatched from inside `verify-helpers.sh evidence-append` (the store's sole writer) via a filesystem marker (`<run_dir>/.notify-enabled`, created by `preflight --notify` / `verify-run.sh notify-enable`) rather than an env var, because the qa-executor's VERIFY MODE runs in a separate Task-spawned process with no shared shell state. `LOOMWRIGHT_WEBHOOK_URL` unset ⇒ silent no-op, run unaffected.
...
- Appends up to three **promotion queues** after the six sections, shown only when non-empty: pending **orientation** memos, pending **agent-memory** proposals (labeled **PROMOTE PENDING PROPOSAL** — surfaced automatically; you no longer list `.supervisor/agent-memory-proposals/` by hand), and the harvested **`.agent/rules/` PR delivery queue** (each rule with its `applies_to`, the finding ids that motivated it, and the batch's coverage / dedupe-rate / scope-fidelity numbers — fidelity always as BOTH figures, checkable and all-motivating-findings). A batch the harvester marked **DISTILLATION FAILURE** is **not offered at all**: the verdict is surfaced, the queue stops there, and no gate is asked
...
- **One remote-affecting action, behind two gates.** A harvested rule needs the per-item **Accept** (authorising its *content*) **and** a separate per-item **Pre-push confirmation** (authorising the *push*); neither implies the other. Only rules clearing both are authored through `add-rule.sh --confirm`, committed to a `dreaming/rules-<session_id>` branch (staged by path — never `git add -A`) and delivered by `gh pr create`. Nothing confirmed ⇒ no branch, no commit, no PR. Delivery also **aborts before branching** unless HEAD is already contained in `origin/<base>` (base resolved from the repo, never assumed `main`) — a PR is the branch-versus-base diff, not the commit, so a rules-only commit cut from an unrelated feature branch would still open a PR full of that branch's changes. **`/dreaming` never merges the PR it opens.**
...
### 🌱 /propose — Candidate Work Items from the Churn Ledger, a /verify Run, and the Domain (Read-Only, Propose-Only)

**Purpose:** Turn the recorded churn ledger — or, with `--from-verify`, a `/verify` run's evidence, or with `--domain`, this project's own domain — into candidate work. The learning loop records and never proposes — findings accumulate, but nothing converts a trend into a piece of work, so every item this system executes was human-typed. By default `/propose` reads the pre-aggregated `.supervisor/floor/floor.json` and writes evidence-carrying candidate requirement files to `.supervisor/requirements/proposed/` (gitignored) via `scripts/propose-work.sh`. **It proposes; it never queues** — `proposed/` is deliberately NOT an `/automate --folder` target, and promotion is a human moving a file out of it. Threshold, never ranking: a `(class, flow_stage)` pair emits at ≥10 entries, with no score, rank, priority or top-N. A candidate that cannot cite ≥3 distinct ledger entries is not written; a basis older than 24h proposes nothing and names the age. Advisory and fail-safe — `set -uo pipefail` with no `set -e`, a `jq` guard that skips, `exit 0` always. **Three bases, never more than one in one invocation:** the default reads inward (our own recorded mistakes, local `jq` over a local file, **zero** external calls); `--from-verify <run_id>` reads a single `/verify` run's `evidence.jsonl` and drafts one file per `FAIL` `ac` line classified `REAL_BUG` plus every `issue` line (`DISCOVERY_GAP`/`ENVIRONMENT_ISSUE`/`BLOCKED`/`NOT_VERIFIABLE` never draft), via `scripts/propose-from-verify.sh`, sharing its blast-radius guard with the other two bases through `scripts/propose-common.sh`; while `--domain` reads outward via `scripts/propose-domain.sh` — the committed product store `.agent/product.json` (through `read-product.sh`, never re-parsed) plus competitor sources fetched through one named seam — and asks what competent apps in this domain do that this one does not. `--domain` is never implicit precisely because it spends fetch budget and emits judgment-class output. Its files are namespaced `domain--<slug>.md` so no two bases can collide in `proposed/`; it emits nothing an actually-fetched source does not corroborate (its expectation catalogue is a list of things to look for, not evidence); a capability the inventory cannot confirm either way is reported **unverified**, never missing; every gap is classified `TABLE-STAKES` / `DIFFERENTIATOR` / `NOT-FOR-US` with the default action read verbatim from `read-product.sh`'s `stance_default_action`; and with no product store it names the absence, offers the `propose-product.sh` bootstrap, writes nothing and exits 0.

**Parameters:**
- `--domain` (default off — run the **domain** basis *instead of* the ledger basis; the two never run in one invocation and `--domain` is never implicit)

Each basis is tuned by env vars read by its own script, all optional. Ledger basis (`propose-work.sh`): `PROPOSE_FLOOR_JSON`, `PROPOSE_OUT_DIR`, `PROPOSE_REQUIREMENTS_DIR`, `PROPOSE_THRESHOLD` (10), `PROPOSE_MAX_AGE_SECONDS` (86400), `PROPOSE_SOURCE_DATE_EPOCH`. Verify basis (`propose-from-verify.sh`): `PROPOSE_FROM_VERIFY_OUT_DIR` (`.supervisor/requirements/proposed`). Domain basis (`propose-domain.sh`): `PROPOSE_DOMAIN_STORE` (`.agent/product.json`), `PROPOSE_DOMAIN_OUT_DIR` (`.supervisor/requirements/proposed`), `PROPOSE_DOMAIN_REQUIREMENTS_DIR` (`.supervisor/requirements`), `PROPOSE_DOMAIN_FETCH_CMD` (**unset ⇒ no fetcher and nothing fetched at all** — it is the only external transport, invoked as `<cmd> <url>`, and there is no implicit `curl` fallback), `PROPOSE_DOMAIN_MAX_FETCHES` (5, for the whole run; local reads never draw on it), `PROPOSE_DOMAIN_MAX_SOURCE_AGE_SECONDS` (7776000 = 90d, past which a source is named STALE rather than cited silently), `PROPOSE_DOMAIN_SURFACE_FILE_CAP` (4000), `PROPOSE_DOMAIN_SOURCE_DATE_EPOCH`.

```
/propose                                       # read floor.json, write candidates to .supervisor/requirements/proposed/
/propose --from-verify <run_id>                # verify basis: read a /verify run's evidence.jsonl, write verify-<run_id>-*.md candidates
/propose --domain                              # domain basis: read .agent/product.json + fetched sources, write domain--<slug>.md candidates
```

**Note — deleting a proposal is NOT a durable dismissal.** The cited coordinates are the earliest entries for a pair, so the `evidence-set:` token is stable and the candidate returns on the next run; a domain candidate's `domain-gap: <slug>@<classification>` token is stable for the same reason. To dismiss permanently, paste that token into a requirement file stamped `## Status: done` anywhere under `.supervisor/requirements/`, and the next run names the file that suppressed it.

**Learn More:** see `loomwright/commands/propose.md` for the three-bases contract, all three env tables, the promote/dismiss contract, and the emission + staleness thresholds
...
**Usage:** `/rules list` · `/rules suggest` · `/rules add` · `/rules retract` · `/rules audit` · `/rules check`
...
**Purpose:** Single entry point for checking and configuring every optional plugin capability across 10 modules — **observability** (local Langfuse v3 + bundled OTel collector; `init | status | remove`), **telemetry** (delegates to `/telemetry`), **notifications**, **webhook**, **Beads**, **MySQL MCP**, **rules** (`status | (no-arg → seed)`; `remove` is N/A — retracting a rule is `/rules` territory, not a module teardown — seeds a cold-start repo's `.agent/rules/` store with a small set of **portable** conventions, stamped `provenance.source=setup:rules-seed` so a shipped default is never mistaken for something the tool learned here; every seed is repo-wide with `check: null`, rules stay DATA and advisory, and the module adds **no gate**), **statusline** (`status | apply | remove` — wires the one-line run report (phase · branch · N/M subtasks · age of the last event) into the user-scope settings document; backup-first, parse-gated, byte-compare idempotent, and the **only** module that writes user-scope settings from a script. A `statusLine` this plugin did not write is **preserved and reported**, never overwritten — replacing it takes an explicit flag and the previous value is recorded so `remove` restores it verbatim. The line carries **no active-agent field**: there is no spawn event in any log to read, so inferring liveness from another record's recency would be a guess, and it is omitted rather than guessed), **ui** (`check | apply | serve | stop | remove` — the CONFIGURATION half; the operational and registry verbs live in the direct `/ui` command — installs and serves **The Floor**, a local, loopback-only page rendering `.supervisor/floor/floor.json`: five pipeline stages above one lane per agent in the newest session, where a lane whose last event is older than the stall threshold visibly stalls, lanes are grouped by derived lifecycle state (needs-you/working/done/quiet/unknown), and a Feed of recent events sits alongside them. Zero new runtime dependencies beyond `python3` for `serve`, which always binds 127.0.0.1 — nothing leaves the machine. Marker-gated: `apply` and `remove` both WITHHOLD on a directory this module did not create. Like the status line it carries **no liveness and no active-agent claim** — a lane reports events that were RECORDED, never that anything is running now, and an agent whose type never appeared on a log line is drawn `identity unknown` rather than guessed — unless the emitters positively identified it as the session's own thread from the transcript path in the payload, in which case it reads `main thread`, because "has no role" and "has no identity" are different facts), and **memory** (`status | apply | remove` — puts `.claude/agent-memory/` + `.supervisor/memory/` + the findings ledger `.supervisor/postmortem/results.jsonl` under version control IN PLACE via gitignore negation, plus the repo allowlist; consent-bearing, and `remove` states plainly that git history retains anything already pushed). **The ledger is the third store and it is GATED**: it is structurally cross-repo (a postmortem append lands in the CURRENT working `.supervisor/`, not the analysed repo's), so `apply` fails CLOSED — it withholds the ledger negation, names the offending repo slugs and reports the third verdict `gated` whenever any record's `.repo` is outside the repo allowlist, while still exiting 0. No-arg invocation prints a status dashboard (one real check per module) then offers configuration via multi-select. Every module follows the same contract: check → report → offer → apply → verify — idempotent, never blind-overwrite (settings.json changes are jq-deep-merged with a timestamped backup, aborting on parse failure; the memory module's `.gitignore` block is backup-first, atomic, and ABORTS without writing on an unparseable file).

**Usage:**
```
/setup                      # status dashboard + multi-select configuration
/setup observability        # observability module directly: init | status | remove
/setup rules                # seed a cold-start .agent/rules/ store (status = read-only report)
...
### 🖵️ /ui — The Floor, Directly (Operational Half + Project Registry)

**Purpose:** The short path to the local, loopback-only run view, and the registry that decides which projects it may show. `/setup ui` is *configuration* (install / uninstall the bundle, marker-gated behind an explicit confirm); `/ui` is the *daily operation* — start it, stop it, and manage the project list. Both drive the same engine (`loomwright/scripts/setup-ui.sh`), so there is no second implementation to drift. One server on one port serves **every registered project**, chosen in the page rather than by which directory a terminal was launched in; the selected project regenerates on `--interval` and the others one at a time on a slower cadence, because one projector run costs about a second and regenerating all of them every tick would starve the loop. A project appears on the page **only because a human ran `add`** (or confirmed a `scan`) — there is no discovery-on-serve. The registry lives *beside* the ui directory rather than inside it, so `/setup ui` → `remove` cannot take the project list with it; `forget` is a registry edit only and **never touches the directory it is forgetting** (a distinct verb on purpose — one word meaning both "tear down the module" and "drop a project" is a data-loss shape).
...
/ui scan <dir>            # PROPOSE unregistered git projects under <dir>; writes nothing without --confirm
```

**When NOT to Use:** installing or uninstalling the bundle — `apply` and `remove` stay with `/setup ui`, which is the only place that materialises or deletes a directory of files outside the repo.

**Learn More:** see `loomwright/commands/ui.md`; the reference companion is `loomwright/docs/FLOOR_UI.md`, and the module contract lives in `loomwright/skills/setup/SKILL.md`.
...
| **PostToolUse (Bash)** | A Bash tool call completes (e.g. `gh pr create`) | Backstops the until-mergeable review drain on PR creation (`hook-dispatch-on-pr-create.sh`); session-scope gated, fail-safe (v14.34.0). PLUS a second entry (`reproject-state-on-terminal.sh`, PR #116 review round) that mechanically re-invokes `build-state.sh` once `session_end` lands in the session log. PLUS a third leaf on the existing Bash matcher (v15.66.0): the worktree observer `worktree-audit.sh record` — records worktrees added through the Bash tool in a repo the plugin has run in, and its read-only `report` lists the ones never seen removed that git still lists |
...
│   ├── ui.md                         # The Floor, directly: serve/stop + the project registry (check|serve|stop|add|list|forget|scan)
│   └── agent-help.md
