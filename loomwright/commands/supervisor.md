...
/supervisor --skip-children-check              # Short-circuit Phase 4 FINALIZE's children-settled pre-merge check (escape hatch)
...
/supervisor --multi-voter-heal                 # Opt in (default OFF): Phase 4.5 review spawns a second independent red-team-reviewer verification vote + refute check before fixing
/supervisor --no-multi-voter-heal              # Suppress multi-voter heal (overrides config)
/supervisor --voter-provider cursor:gpt-5      # With --multi-voter-heal: cast the verification vote via lens-run.sh (isolated provider CLI) instead of the red-team-reviewer Task
/supervisor --sdk-runner                       # EXPERIMENTAL (opt-in, default OFF): Phase 3 shells out to the quarantined SDK spike runner; fail-closed if node/runner absent
/supervisor --max-tokens 50000                 # Opt-in token-spend ceiling (red-team-hardening/06): Phase 3 poll loop parks BEFORE spawning another worker/reviewer round once the session's own ledger exceeds N
```
...
| `--cheap` | No | Cost-optimized profile: spawns orchestrator, execute-manager, workers, code-reviewer, Phase 4.5 fix tasks, and — when `--multi-voter-heal` is ON — the multi-voter verification voters/refute spawn with `model: "sonnet"` override at spawn time. **N/A for a `--voter-provider` voter:** that path is a Bash shell-out to `lens-run.sh`, not a Task spawn, so there is no spawn-time `model:` override — the model comes from the `name:model` suffix. Default behavior (`inherit` for all) is unchanged when flag is absent. **Caution:** on Haiku sessions, listed roles upgrade to Sonnet (costs more). See `docs/ARCHITECTURE_CONTRACTS.md` §"Cost Profiles". |
| `--max-tokens N` | No | **Opt-in token-spend ceiling (red-team-hardening/06).** Parsed at Phase 0 INIT and recorded as a Phase Flag (same re-read-after-context-loss convention as `--non-interactive`). At Phase 0 INIT, also acquires `.supervisor/run.lock` (`scripts/run-lock.sh acquire --owner supervisor:<session_id> --session-id <session_id>` — `<session_id>` is the Claude Code runtime session id, matching `/automate` PICK's acquire shape, so a crashed session's lock is reclaimable) — a held lock PARKs the run BEFORE any work begins, printing `run_lock_held owner=<label> pid=<pid> age=<s>`, never proceeding silently (a `run_lock_reentrant owner=<outer> …` line, exit 0, is NOT a park: an enclosing `/automate`/`/autonomous` in the same session holds the lock — proceed; this run's own release is then a no-op); released at the completion tail / any Phase 4 abort. When `--max-tokens` is set, each Phase 3 poll-loop iteration entry sums this session's ledger via `scripts/read-token-ledger.sh --session <session_id>` and compares against N; a breach OR `LEDGER_UNREADABLE=1` ⇒ the run parks with `SUPERVISOR_RESULT.status: failed`, `error: "token_ceiling_reached"` before spawning another worker/reviewer round. Forwarded automatically by `/autonomous` and `/automate` when they were passed the same flag (see their own `--max-tokens` docs). Default: unset (no ceiling, byte-identical behavior). **Honest limit:** the ledger counts only what a SubagentStop hook saw — never this session's own main-thread tokens, never CI-side (`claude-review`) spend. No dollar conversion (no price table). See `docs/ARCHITECTURE_CONTRACTS.md` §"Token ceiling". |
...
| `--skip-children-check` | No | Short-circuit Phase 4 FINALIZE's pre-merge safety-gate "children settled" check (v15.80.0): every `agent_identity` row this session wrote must have a matching terminal lifecycle row (`subtask_complete` / `token_ledger` / `agent_lifecycle: failed`) for the same `agent_id`, or the gate fails closed with `error: "children_unsettled: {ids}"` under `--non-interactive` (interactively, `AskUserQuestion`). The skip is recorded as a deliberate choice (`record_decision(phase: FINALIZE, decision: "user_skipped_children_check")`) and the run summary carries `children_check: skipped`. Escape hatch for a known-benign unsettled child (already resolved out-of-band) that should not block the merge. |
...
| `--red-team` | No | Opt in to an advisory red-team review in Phase 4.5: after the Code Reviewer holistic pass, if the integrated diff is **high-risk** — per `scripts/classify-risk.sh --kind-table`, the ONE heuristic (committed once in `docs/RESULT_SCHEMAS.md` §SUPERVISOR_RESULT; not restated here) — spawn exactly ONE `red-team-reviewer` pass (outside the heal loop) and post its findings to the PR as a clearly-labelled non-gating comment. **OFF by default** and **high-risk-only** (enabled-but-low-risk skips silently). The classification itself runs **unconditionally** in Phase 4.5 and is recorded as `SUPERVISOR_RESULT.risk_classification` (`{high_risk, reasons}`, advisory); only the spawn is opt-in. **Advisory & strictly NON-GATING** — it NEVER changes `heal_decision`, never drives the fix task, never gates, and never blocks the PR; the Code Reviewer's `CODE_REVIEW_RESULT` remains the sole gating signal. **Fail-safe** — any spawn error/timeout or comment failure is a logged no-op and the run continues. Equivalent to setting `.red_team_high_risk: true` in `.supervisor/config.json`. |
...
**Phase 4 (FINALIZE):** The merge/commit/PR protocol authority is `skills/async-orchestration/SKILL.md` **Part 2 — Supervisor FINALIZE Protocol** (the pre-merge safety-gate checklist items, sequential merge, worktree cleanup, commit/push/`gh pr create` mechanics, and the step 6.5 PR-base self-verify procedure; the skill is no longer frontmatter-preloaded as of the 4f routing change (v15.19.0) — Phase 2 PLAN now performs the session's genuine first Read of this file, and this Phase 4 Read is a genuine second load acting as a compaction refresh, guarding against Phase 3 EXECUTE's long multi-Task span compacting Phase 2's context out of the window before FINALIZE needs the merge/PR mechanics) — `agents/supervisor.md` "Phase 4: FINALIZE" keeps the short phase stanza with the mandatory gates (pre-merge safety gate, merge-conflict STOP, salvage before remove — v15.67.0, PR-base self-verify; the agent's numbered gate list is authoritative if this parenthetical ever lags it) and entry/exit conditions.

**Phase 4.5 (v11.0.0):** After the PR is created, Supervisor runs a holistic Code Reviewer on the full feature-branch diff. If it finds BLOCKING/HIGH severity `new` issues, Supervisor spawns a fix task that addresses them and pushes updates to the PR. The loop retries up to `--heal-iterations` times (default 3). On PASS the task is marked `completed`; on NEEDS_HUMAN or max iterations the task is marked `completed_with_escalation` with findings posted as a PR comment. Eliminates the manual review-and-fix cycle. The phase always runs (unless `--skip-self-heal` short-circuits the loop); it owns the completion tail (job-file move, state marked completed) so the record captures the heal outcome. **Protocol authority:** the full Phase 4.5 procedure (on-entry advisory enrichments, base-mismatch cleanup, the review-and-fix loop, rubric grading, red-team lens, and the completion-tail procedure steps 1–6) lives in `skills/self-heal-advisory/SKILL.md` **Part 2** — `Read("${CLAUDE_PLUGIN_ROOT}/skills/self-heal-advisory/SKILL.md")` at Phase 4.5 entry on BOTH the inline and delegated paths (deliberately not preloaded); `agents/supervisor.md` keeps the short phase stanza with entry/exit conditions plus the verbatim completion-tail guard.
...
```
Record `until_mergeable_dispatched: {UM_DISPATCHED}` on the job `## Outcome` block + `session_end` JSONL (and optionally `SUPERVISOR_RESULT`), with `until_mergeable_log: {UM_LOG}` only when true and a log was found. A marker exists IFF the dispatcher ran for this PR (step 5.5 OR the hook), so `true` is truthful regardless of which path fired; `false` is truthful ONLY when no marker exists. This mirrors the Phase 4.5 protocol authority — `skills/self-heal-advisory/SKILL.md` Part 2, completion-tail step 5.5 "Observability (AC8b)" (the awk snippet's agent-side copy moved there from `agents/supervisor.md`; THIS commands-side copy stays in place because the inline path loads this file) — keep the two in sync. Same fail-safe posture as every inline-path write: best-effort, never gates, never blocks the PR.
...
### Phase 1.5: PRE-FLIGHT SYNC (abbreviated — full output adds "Open PRs scanned", "Recent commits scanned", "PRs file-inspected" lines; see `skills/preflight-sync/SKILL.md`)
...
- `agents/supervisor.md` - Full agent prompt (7-phase model; Phase 4.5 stanza + completion-tail guard)
- `skills/self-heal-advisory/SKILL.md` - Phase 4.5 protocol authority (Part 1 advisory machinery + Part 2 loop protocol; Read at Phase 4.5 entry)
