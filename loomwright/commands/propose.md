...
- **Promote:** move the file out of `proposed/` into a real queue folder, then run it (`/automate --folder <dir>`, `/autonomous --requirement <path>`, or `/launch-pad`). Nothing is enqueued until you do. A domain candidate deliberately stops short of user stories and acceptance criteria for the work itself — hand the decided gap to `/product-owner`. A file may also be kept un-runnable **in place** (outside `proposed/`) by giving it a `## Status: proposed` stamp — `/automate`'s folder and backlog-dir intake skip a stamped file the same way they skip a `## Status: done` one, so moving the file out of `proposed/` **and** removing or changing the stamp are **both** needed before it will run.
