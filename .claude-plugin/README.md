...
A Claude Code plugin with 13 agent roles (8 user-facing + 5 internal), 50 focused skills, and optional Beads issue tracker integration. Automates plan-first readiness, parallel workflow execution, requirements definition, code review, adversarial audits, and dual-agent QA testing. v13.1.0 adds a notification surface (OS-native desktop banners + optional webhook push on plugin pauses), the `LAUNCH_PAD_RESULT` v1 schema with deterministic python validation, and a SessionStart resume hook — all purely additive on top of v13.0.0's `/autonomous` orchestration shell.

## Overview

The AI Agent Manager Plugin v13.1.0 includes:

- **Notification surface + `LAUNCH_PAD_RESULT` schema + SessionStart resume (v13.1.0)** — Two purely additive hardening features layered on v13.0.x. (1) **Notification surface:** two new hooks (`PreToolUse[AskUserQuestion]` + `Notification`) fire OS-native macOS/Linux desktop banners via `scripts/notify-desktop.sh` whenever the plugin pauses for user input — Supervisor 4-option adjudication, rubric gate, Plan Reviewer NEEDS_HUMAN, Launch Pad Phase 6, `/autonomous` merge-and-continue. `scripts/send-webhook.sh` extended with a parallel paused-event payload (Shape B) so the same events also reach Slack/Discord/PagerDuty via `AI_AGENT_MANAGER_WEBHOOK_URL`. Scope-gated by default (`AI_AGENT_MANAGER_NOTIFY_SCOPE=plugin|all`, default `plugin`) so unrelated AskUserQuestion calls in the host session don't generate spam; `AI_AGENT_MANAGER_DESKTOP_NOTIFICATIONS=0` hard-disables banners. (2) **`LAUNCH_PAD_RESULT` v1 schema:** Launch Pad now emits a structured result block in Phase 7 (exactly 4 fields: `schema_version=1`, `status ∈ {saved, discarded, blocked, aborted}`, `saved_brief_path: string | null`, `summary`) — closes the v13.0.x `ls`-diff fragility in `/autonomous` brief-save detection. Deterministic python YAML validator (`scripts/validate-launch-pad-result.py`) catches all 5 YAML null spellings and correctly rejects the string `"null"`; called from both the SubagentStop hook (agent-owned `-runner` path) and inline from the autonomous-loop skill (slash-command path). `ls`-diff retained as fallback for pre-v13.1.0 plugins. (3) **SessionStart resume hook:** `scripts/session-resume.sh` emits a bounded ~1 KB structured summary as `additionalContext` on session resume / compact (in-progress briefs, recent failed briefs, tail of `state.md`, last 3 session-log entries, recovery hints). Silent no-op on fresh `startup`. v13.1.0 is **purely additive** on top of v13.0.1 — no new agent, no existing schema changed, no behavioral change to any pre-existing hook / script / skill / command. New artifacts only: `notify-desktop.sh`, `session-resume.sh`, `validate-launch-pad-result.py`, `LAUNCH_PAD_RESULT` schema entry in `RESULT_SCHEMAS.md`, Phase 7 emission in `agents/launch-pad.md`. Hook count: 14 → 19 (3 notification + 1 LAUNCH_PAD_RESULT validation + 1 SessionStart). Slash-command count: 12 (unchanged). Skill count: 50 (autonomous-loop bumped to v1.1.0, no new skill).

- **Foreground-assisted goal-completion loop (v13.0.0, preserved)** — `/autonomous` slash command (`ai-agent-manager-plugin/commands/autonomous.md`) governed by the `autonomous-loop` skill (`ai-agent-manager-plugin/skills/autonomous-loop/SKILL.md`). The command is an inline main-thread workflow that chains Launch Pad → Supervisor for a single requirement. **Default mode is single-iteration** — pure command chaining, just so the user does not have to remember `/launch-pad "..."` then `/supervisor job: <path>`. **Multi-iteration mode** (opt-in via `--allow-multi-iteration --max-iterations N`, default N=3) adds an EVALUATE phase that reads `SUPERVISOR_RESULT` and re-plans on exactly two signals: (1) `status: completed` + `rubric_score N/M` with N<M (loop pauses for a rubric-gate AskUserQuestion — merge-and-continue verified via `gh pr view` / `git merge-base --is-ancestor`, stop-here, or force-continue-anyway); (2) `status: failed` + `inter_subtask_gap` on this iteration's brief, detected by anchor-by-filename. The loop **never auto-picks** on adjudication — Supervisor's existing 4-option `AskUserQuestion` (per `FAILURE_ESCALATION.md`) surfaces in-session as it does today. v13.0.0 artifacts: `/autonomous` command, `autonomous-loop` skill, `AUTONOMOUS_RUN` entry in `RESULT_SCHEMAS.md` (autonomous-layer-only, no hook validation).
...
└── ai-agent-manager-plugin/                 # The nested plugin
    ├── .claude-plugin/
    │   └── plugin.json                      # Plugin manifest (v13.1.0)
...
19 hooks centralized in `hooks.json` validate agent output:
- **SubagentStop:** Worker, Execute Manager, Code Reviewer, Supervisor, QA Executor, Plan Reviewer, Launch Pad Runner (7 validators — 6 prompt + 1 `type: command` python YAML parser for LAUNCH_PAD_RESULT, v13.1.0) + 3 `type: command` telemetry hooks on Code Reviewer, QA Executor, Supervisor + 1 `type: command` opt-in webhook hook (v12.2.0)
- **Stop:** Code Reviewer (completeness gate)
- **TaskCompleted:** Verify task genuinely done
- **WorktreeCreate / StopFailure:** Logging
- **PreToolUse[AskUserQuestion]** (v13.1.0): notify-desktop + send-webhook paused-event branch — OS-native banner + optional Slack/Discord push on every plugin pause
- **Notification** (v13.1.0): notify-desktop — fires on Claude Code's own attention signals (permission_prompt, idle_prompt, elicitation_*)
- **SessionStart** (v13.1.0): session-resume — emits bounded ~1 KB structured summary as additionalContext on resume / clear / compact
