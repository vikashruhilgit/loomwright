...
- **`--non-interactive-fallback`** — engage the per-gate policy. Without this flag, an `AskUserQuestion` on a closed stdin will hang or error; with it, each gate has a defined non-interactive outcome:
  ...
  - **Adjudication gate** (Supervisor's 4-option scope-expansion question): inherits Supervisor's `--non-interactive` policy if `/autonomous` forwards that flag. Otherwise the gate raises an in-session `AskUserQuestion` and stalls. The recommended CI invocation passes both: `/autonomous "..." --non-interactive-fallback --non-interactive` (the second flag forwards into the inlined `/supervisor`).
...
- **Recommended CI shape:** `claude /autonomous "..." --non-interactive-fallback --non-interactive --notify --max-iterations 3` with `AI_AGENT_MANAGER_WEBHOOK_URL` set in the CI environment. Examine the JSON sidecar at `.supervisor/autonomous/{session_id}/AUTONOMOUS_RUN.json` after the run; the `status_reason` will tell you exactly which gate (if any) closed the loop.
