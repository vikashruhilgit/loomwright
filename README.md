...
>
> **NEW in v15.38.0 — vendor-neutrality becomes a mechanically enforced ratchet:** portability was asserted in docs and enforced by nothing, so the core kept absorbing harness references (prompt-layer refs **+31%** since v15.13.0; `loomwright/scripts` portability **82% → 70%**). New fail-CLOSED gate `scripts/check-vendor-coupling.sh` reads a committed `loomwright/docs/vendor-coupling-manifest.json` — every vendor token, class glob and per-path allowance lives there, none hard-coded — classifies each path CORE / ADAPTER / COUPLED, and fails when a count exceeds its allowance. Breach is `actual > declared`; paying debt down always passes, and raising an allowance is legal only in the PR that spends it. `loomwright/scripts/resolve-loomwright-root.sh` adds the vendor-neutral `LOOMWRIGHT_ROOT` seam (ADAPTER-exempt on purpose: it names the harness once so nothing else has to); migrating existing call sites is explicitly out of scope. Six review rounds found six defects, **five of them the gate failing its own standard** — a hand-typed count inside the count-drift gate, the allowance-regeneration flag proved only by reading, validation that skipped files with no references, and a config field nothing read. All reproduced before fixing and backed by executed assertions (51 → 71), every new case mutation-verified. Counts unchanged: 14 agents / 21 commands / 41 skills / 24 hooks.
