...
>
> **NEW in v15.37.0 — `selvedge` scaffolded; the loomwright-pinned CI gates are now plugin-aware:** a fourth plugin, **selvedge** (the QA companion), is registered and installable but deliberately **empty** — it ships no agents, commands, skills, or hooks yet, because creating those dirs empty would trip the very gates that guard them (an empty agents dir is a 0-agent false green; a `skills/` with no index fails loudly). Three gates that hard-pinned `loomwright/` — `check-token-budget.sh`, `check-shared-prefix.sh`, `check-contract-parity.sh` — now **discover** plugins from `.claude-plugin/marketplace.json` instead, and CI's hard-gate self-test loop moved out of `ci.yml` into a testable harness that runs **every** plugin's `scripts/test-*.sh`. Each gate keeps two deliberately different branches: **skip silently** when a plugin ships no relevant tree, **fail loudly** when a tree exists but is malformed. Every widened loop is **mutation-controlled** — deleting its new branch fails a named, bounded set of self-test cases, because a widened loop whose self-test still passes with the branch deleted is worse than no gate at all. Counts unchanged: 14 agents / 21 commands / 41 skills / 24 hooks.
>
...
2. Commands are in `loomwright/commands/` (19 commands)
3. Skills are in `loomwright/skills/` (36 skills, versioned with SKILLS_INDEX.md; 18 tech-stack skills live in the sibling `stackpack/` plugin)
...
3. Verify with `/skills` — should show all 36 skills under "Plugin skills". Use `/agent-help` to confirm all 19 slash commands are registered.
