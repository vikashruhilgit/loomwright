...
>
> 📜 **Release history** — every version, including the current one, lives in [`CHANGELOG.md`](CHANGELOG.md). This README describes the plugin as it is **now**; per-release narrative belongs in the changelog, not here.
...
| **Propose** (command) | `/propose` · `/propose --domain` | Evidence-carrying candidate work items under `.supervisor/requirements/proposed/` from **three bases**, never more than one in one invocation: the churn ledger (inward, local, zero external calls); `--from-verify <run_id>`, one `/verify` run's evidence drafted into per-FAIL requirement files; or, with `--domain`, the project's domain — what competent apps in it do that this one does not, from `.agent/product.json` plus fetched sources — proposes, never queues | Deciding what to work on next   |
...
- **Foundation — System Contracts.** A per-subsystem **System Contract** store
  ...
  `loomwright/scripts/write-system-contract.sh` — it refuses any git-worktree CWD,
  hash-chains provenance, and writes atomically. Contracts are read via
  `loomwright/scripts/read-system-contract.sh` and self-tested by
  `loomwright/scripts/test-system-contract.sh`. (Context-Keeper is deliberately not in this
  write path.)
...
- Run `bash scripts/worktree-salvage.sh <path>` first (plugin-relative) — it preserves uncommitted content under the primary checkout's `.supervisor/salvage/<name>-<ts>/` and prints that path, or prints nothing when there is nothing to keep — then remove with: `git worktree remove ../project-{subtask_id}`
