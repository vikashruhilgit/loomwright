...
>
> **NEW in v13.1.0:** Notification surface + `LAUNCH_PAD_RESULT` schema + SessionStart resume hook. (1) Two new hooks (`PreToolUse[AskUserQuestion]` + `Notification`) fire OS-native macOS/Linux desktop banners via `notify-desktop.sh` whenever the plugin pauses on user input — Supervisor adjudication, rubric gate, Plan Reviewer NEEDS_HUMAN, Launch Pad Phase 6, `/autonomous` merge-and-continue. `send-webhook.sh` extended with a parallel paused-event payload (Slack/Discord/PagerDuty). Scope-gated by default via `AI_AGENT_MANAGER_NOTIFY_SCOPE=plugin|all` (set `0` on `AI_AGENT_MANAGER_DESKTOP_NOTIFICATIONS` to hard-disable). (2) New `LAUNCH_PAD_RESULT` v1 schema (exactly 4 fields: `schema_version`, `status`, `saved_brief_path`, `summary`) closes the v13.0.x `ls`-diff fragility in `/autonomous` brief-save detection. Deterministic python YAML validator (`scripts/validate-launch-pad-result.py`) catches all 5 YAML null spellings, called from both the SubagentStop hook and inline from the autonomous-loop skill. (3) New SessionStart resume hook (`scripts/session-resume.sh`) emits a bounded ~1 KB structured summary of prior plugin state on session resume / compact. Hook count: 14 → 19. Purely additive on top of v13.0.x — no new agent, no existing schema changed, no behavioral change to any existing hook / script / skill / command.
>
> **v13.0.0 (preserved):** A `/autonomous` slash command — command chaining for Launch Pad → Supervisor with optional multi-iteration re-planning (opt-in via `--allow-multi-iteration`). This is **foreground-assisted automation, not fire-and-forget**: Launch Pad's Phase 6 save prompt, NO-GO override, Plan Review FAIL × 3 escalation, Supervisor's adjudication 4-option gate, and the loop's own rubric gate (only in multi-iteration mode, after a `rubric_score N/M` with N<M) all bubble `AskUserQuestion` in-session — you stay at the terminal to answer them, the loop handles the rest. Two specific `SUPERVISOR_RESULT` signals trigger re-iteration: rubric N<M (gated on user-merge verification via `gh pr view` / local `git merge-base --is-ancestor`) and `failed + inter_subtask_gap on this iteration's brief` (Option C re-plan trigger, no merge needed). New artifacts in v13.0.0: `/autonomous` command, `autonomous-loop` skill, and one new `AUTONOMOUS_RUN` entry in `RESULT_SCHEMAS.md`. 12 slash commands, 50 skills. All v12.2.0 capabilities preserved (Agent Teams graduation, Outcomes Rubric, `/dreaming`, opt-in webhook hook) and v12.1.0 documentation increments preserved.
>
...
**New in v11.2.0 (preserved in v13.1.0)** — an optional GitHub Issues telemetry pipeline. After
