...
>
> **NEW in v15.40.0 — the harvester now checks whether the store already said it:** `harvest-conventions.sh` read `.agent/rules/` for context but never compared its own PROPOSALS against it, so a theme whose convention was written down months ago was offered to a human for Accept again in different words — and `/dreaming`, the batch's only consumer, would have put two differently-worded copies of one rule in one file, exactly the duplication the store's own cross-surface rule prohibits. Caught by hand on a real run; nothing mechanical caught it. A **store dedupe pass** now compares every proposal against the live store using the SAME distinctive-term overlap the `pw=` project-wide signal uses (one shared implementation, so the two cannot drift) — exact-string matching is useless here, because the observed pair made one claim in two wordings with no shared sentence. A match is reported as an **already-covered DEFERRAL naming the covering rule's id and the measured overlap**, spends **no cap slot**, and every EMITTED proposal prints the nearest live rule it did *not* match, so the threshold is checkable from the report in both directions. **The deferred theme's findings are not dropped from any denominator:** coverage still measures against them (so a deferral LOWERS coverage rather than shrinking the number it is measured against), they are itemised in the unmapped remainder with 'already covered' as the stated reason, and a new `already covered by the live store` figure plus a **COMBINED convention coverage** make the difference legible. **Limits, stated in the script's header:** the check is lexical, not semantic — it can miss a duplicate written in wholly different vocabulary — and it is advisory, never a gate; an unparseable rule file is fail-safe but NAMED, with the pass reported UNVERIFIED against it rather than silently clean. 31 new assertions, all three mechanisms mutation-verified (removing the pass re-proposes the duplicate; crediting deferred findings to the batch flatters coverage to 100%; making the already-covered continuation line unreachable loses the second cause from a combined empty batch). Counts unchanged: 14 agents / 21 commands / 41 skills / 24 hooks.
