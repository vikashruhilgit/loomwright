...
| **Propose** (command) | `/propose` · `/propose --domain` | Evidence-carrying candidate work items under `.supervisor/requirements/proposed/` from **two bases**, never both in one invocation: the churn ledger (inward, local, zero external calls) or, with `--domain`, the project's domain — what competent apps in it do that this one does not, from `.agent/product.json` plus fetched sources — proposes, never queues | Deciding what to work on next   |
