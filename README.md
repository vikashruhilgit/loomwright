...
>
> **NEW in v15.39.0 — reconciling a lifecycle no prompt can guarantee:** the brief move `in-progress/` → `done/` and the requirement close-out stamp are prompt-instructed completion-tail steps, so an agent that dies before reaching them (a Phase 4.5 reviewer failing 4× with a server error, 2026-08-30) strands a brief whose PR already merged. New `loomwright/scripts/reconcile-jobs.sh` classifies in-progress briefs `stranded_merged` / `stranded_closed` / `unknown` and repairs the strand with `--repair`. It is **offline by construction** — never calls the forge CLI, because the SessionStart hook runs it on every resume — and it **never repairs `unknown`**, because a stranded brief carries no PR URL to verify against. `session-resume.sh` stops asserting "Supervisor was mid-run" over merged work: Stranded (with the repair command) and UNVERIFIED are now separate, and the `/supervisor --continue` hint is suppressed unless something is genuinely unverified. It is the second such reconciler, not the first: `stamp-requirement-status.sh` has reconciled the requirement-stamp half since 2026-08-04, but reads `jobs/done/` exclusively — so a stranded brief structurally blocks it from firing, and repairing the move unblocks it. Also fixes a latent mismatch on the prompt path, where the completion tail stamped a requirement in a format `is_done()` could not read, leaving *successfully* closed-out requirements re-enqueueable forever (never fired: zero requirements carried the sentinel). 31 assertions, all seven mechanisms mutation-verified. **Limits:** the tail is unchanged, so this repairs strands rather than preventing them, and `unknown` means unverified, not clean. Counts unchanged: 14 agents / 21 commands / 41 skills / 24 hooks.
