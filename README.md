...
>
> **NEW in v15.47.0 — The floor acts: guarded writes from the page:** v15.46.0 let the page *show* the project registry; it still could not touch it, and the blocker was structural — the module served the bundle with `python3 -m http.server`, a static file server that answers `501` to any write. The read-only view had also justified having **no authentication** with "there is nothing to authenticate against on loopback": sound for a page that only renders, and **unsound the moment a write exists**, which is the real content of this release. A loopback port is not a security boundary in a browser — any site open in another tab can `fetch` a `POST` at `127.0.0.1:<port>` and never read the reply, and the write has still landed; DNS rebinding extends that reach to a remote origin. A stdlib `ThreadingHTTPServer` subclass replaces the static server — **still no bundler, no Node, no package manager, no CDN** — preserving every `GET` byte-for-byte (diffed against a live old server in the same test run) and still answering `501` to unrouted writes. **Exactly four mutating routes exist — `add`, `forget`, `scan`, `stop` — and the set is read out of the engine rather than restated in the test;** `apply` and `remove` are excluded *by decision, not omission*, because a page able to uninstall itself buys nothing and risks something. **The guard is four parts and all four are required together**, since any one alone is defeatable: a per-run CSPRNG token compared with `hmac.compare_digest`, **carried in a custom header** so a preflight a hostile cross-origin page cannot satisfy is forced (closing the "simple request" hole where a write lands unread), plus `Origin` validation and `Host` validation — the latter being what actually defeats rebinding, which an `Origin` check provably cannot. **Each part is proven load-bearing by a control that disables that part alone and watches the previously-refused request succeed** (403 shipped → 200 against the mutant), with a further control that fails red if any of those four is ever silently skipped. The token rides the URL **fragment**, which no server ever receives, and the page strips it via `replaceState`; it is popped from the environment before any subprocess spawns, so no child inherits it. Paths arriving from the browser are untrusted input — confined, and refused by name for traversal, non-directories, symlink escape and the sibling-prefix trick (`/home/user-evil` against root `/home/user`) — while **the command line stays deliberately unconfined**, because the terminal is the trusted channel and the browser is not. **Eight existing gate groups had their subject replaced and every one was repaired rather than deleted or relaxed;** the enumeration was the hard part and took three review rounds, because a list assembled from memory found two and a sweep for *the tokens a scanner matches* found four — only a sweep for **scanner names and assertion subjects** found all eight. Two of them fail *silently* (their mutants anchor on an exact string in the file being changed, so once it is gone the control is skipped rather than red), so the repairs assert the control still executes. The kill-guard path that replacing the server breaks — `stop` matches on a command line — went from **zero coverage to seven**. `serve` writes one further file, `<ui dir>/serve.log`, one line per *mutating* request and one for a request that could not be answered at all, never a caller-supplied string; a completed read is not logged, so the page's own poll never grows it, and it is reachable at `GET /serve.log`. **This supersedes the v15.46.0 line below claiming the bundle contains no `POST`:** it now contains exactly one, which is the point of this release. Counts unchanged: 14 agents / 22 commands / 41 skills / 24 hooks.

> **NEW in v15.46.0 — `/ui`: the floor in one command, and one floor for every project:** The Floor was reachable only the long way — `serve` and `stop` lived behind `/setup ui`, a *configuration* command, so daily operations went through a configuration dashboard and in practice pushed people back to running the engine by hand in a terminal. **`/ui`** is that shorter path: `check` / `serve` / `stop` plus a project registry (`add` / `list` / `forget` / `scan`). Install and uninstall stay with `/setup ui` — the two verbs that materialise and delete a directory of files outside the repo — and the two files' documented verb sets are asserted to **union** to exactly the engine's own dispatch table, so a verb the engine gains and nobody documents fails. **One server on one port now shows any registered project, chosen in the page** rather than by which directory a terminal was launched in. A project appears **only because a human ran `add`** (or confirmed a `scan`); the registry lives *beside* the ui directory so a module `remove` cannot take the project list with it, and `forget` is a registry edit that never touches the directory it is forgetting — a distinct verb on purpose, since one word meaning both "tear down the module" and "drop a project" is a data-loss shape. The selected project regenerates on `--interval` and the others one at a time on a slower cadence, because one projector run costs about a second and regenerating all of them every tick starves a 2 s loop at two projects — measured against a stub projector of known cost, with a control proving the naive shape really is over budget. Per project the page distinguishes `ready` / `never regenerated` / `unavailable` / `unreadable`, and the registry's own *absent* and *unparseable* stay two different sentences. Selecting a project **sends nothing** — it changes which already-written file the page fetches, and the bundle still contains no `POST`, no `PUT`, no `DELETE` and no `fetch` with a `method` option, on loopback only.
>
> **NEW in v15.44.0 — The Floor: a run you can watch, and a page that never claims more than it recorded:** v15.43.0 made the run state legible (`build-floor.sh` → `.supervisor/floor/floor.json`) but nothing rendered it, so a stall was still discovered only when a human noticed nothing had happened for a while. **`/setup ui`** is the tenth `/setup` module: a three-file, framework-free bundle (no bundler, no Node, no package manager, no web font) copied into the ui directory and served with `python3 -m http.server --bind 127.0.0.1`, regenerating `floor.json` every 2 s. Five pipeline stages (Queue · Plan · Execute · Review · Shipped) sit above one lane per agent in the newest session. **Motion is evidence, never decoration:** exactly ONE timer on the page, and a lane's shuttle advances only when that lane's recorded `events` count changed — verified in a browser with a control (bumping one lane advanced only that lane). The one stated exemption is the pulse on a non-stalled lane, whose ABSENCE is the stall signal; the suite asserts that budget by occurrence count with a mutation control. **The page never claims more than the record supports:** no liveness and no "currently running agent" — no spawn event exists in any log, so it would be a guess, and the page says so permanently under the lanes; an untyped lane is drawn `identity unknown` rather than assigned a plausible role, and `read_only` is a genuine tri-state whose third value renders "read-only unknown" rather than silently "not read-only". **Absent, empty and stale are three distinct renders, never a blank page** — `no floor.json at this origin`, `no run in flight`, `floor.json is stale (<age>)` — all browser-verified against committed fixtures with the console read for errors, and `?stale=<seconds>` is documented beside `?stall=<seconds>` because a committed fixture's timestamp is always in the past. **Loopback is the whole security posture and is not negotiable at the command line:** `--bind 127.0.0.1` always, a `default-src 'self'` CSP, only the relative `floor.json` fetched, an egress scanner with a mutation control, and a real network log showing 127.0.0.1 only — which matters because `floor.json` carries branch names, session ids and agent ids. The engine is **marker-gated** (`apply` and `remove` both WITHHOLD on a directory this module did not create; `remove` also refuses `/`, `$HOME` and the plugin install dir), every branch exits 0, and residue is proven by hashing two whole trees before and after a full `apply` → `serve` → `stop` → `remove`. The projector gains an additive `agents` roster, newest-session agent rows and subtask rows — every field omitted when absent, never defaulted — with `schema_version` still 1. New reference doc `docs/FLOOR_UI.md`; a `(z)` parity group in `test-setup-ui.sh` now gates the module-registration surfaces that no CI check covered. Counts unchanged: 14 agents / 21 commands / 41 skills / 24 hooks; `/setup` modules 9 → 10.
>
> **NEW in v15.43.0 — nine surfaces, nine parsers, and a number that meant two different things:** the run state a view needs is already on disk, but scattered across **nine surfaces in four formats** — markdown tables (`.supervisor/state.md`), folder membership (the four `jobs/` lifecycle dirs), JSONL (`logs/*.jsonl`, `postmortem/results.jsonl`) and JSON (`drain-rounds/`, `.agent/rules/`), plus `automate/`, `insights/runs/` and `worker-summaries/`. With no single reader, every future view re-implements the same nine parsers and drifts independently. **`build-floor.sh`** is that reader and `.supervisor/floor/floor.json` is the contract — read-only on every input, `command -v jq` skips rather than fails, **exit 0 always**. Three properties carry it. **Every count states its own counting basis,** because a bare number was already ambiguous: `.supervisor/logs/` holds 99 directory entries but 65 `*.jsonl` session logs, both readings defensible and differing by 34, so every number ships with the glob or predicate that produced it. **Absent evidence is an omitted key, never a plausible default** — a surface that cannot be counted carries no `count` key at all, never `0`, and names the reason; an input that will not parse reports `unverified`, never `counted`. **Sessions segment by the `cc_session_id` FIELD, never by filename,** because a log file is named for a `state.md` `status:` that can read `running` for weeks. **The hardest defect was a proven-looking zero.** Under `nullglob` an UNREADABLE directory expands to zero arguments exactly like an empty one, so the JSON-document surfaces (`drain_rounds`, `rules`) fell into the counted-0 arm and published a fabricated zero carrying an `mtime_epoch` that implied a reading which never happened; the same blindness in the session reader's bulk `cat … 2>/dev/null` dropped an unreadable log silently, reporting a `counted` UNDERCOUNT together with `lines_malformed: 0` — asserting cleanliness about bytes it never saw. Both now guard readability the way `state.md`, `postmortem` and `count_glob` already did, and the reader normalises per file so a newline-less final record is no longer spliced onto the next file's first one and blamed on the input. **The suite gap was exact and is the root cause:** `grep -n chmod` over the tests returned a single hit — the `chmod +x` on a `stat` stub — so no test made any input unreadable, all three *existing* `! -r` branches were uncovered and the two missing ones stayed invisible behind 105 green assertions. There is now a case per surface family, each asserting `unverified`, the `count` key **absent** rather than zero, the offending path named, and exit 0, each gated on a measured premise that `chmod 000` actually denies a read on this machine. **Tests are hermetic by construction:** `.gitignore`'s `.supervisor/*` means eight of the nine inputs are absent from every fresh clone and worktree, exactly where CI runs, so a real-tree assertion would compare 0 to 0 and go silently green. Containment is hashed with `find` — never git, which reports neither an illegal write into an ignored tree nor the legitimate output — and directories and symlinks are in scope, not just regular files. Determinism diffs two RAW unfiltered runs under two different injected timestamps, and the single-wall-clock pin now recognises `printf '%(%s)T'`, `$EPOCHSECONDS`, `$SECONDS` and `git log --format=%ct` as clock reads too, with one mutant per idiom. Counts unchanged: 14 agents / 21 commands / 41 skills / 24 hooks.
>
> **NEW in v15.41.0 — the sole writer ate the blank line between sections:** `write-lessons.sh` rebuilt a `## <category>` section from its `- [id]` entry lines alone, so the blank separator sitting immediately before the NEXT heading matched no loop and was silently dropped — on every write into a category that is not last in the file. The sibling branch that creates a brand-new section prints that separator explicitly, so the two halves of one rebuild disagreed on the format. Deterministic and compounding: once a separator is gone it never comes back, so the committed `.supervisor/memory/LESSONS.md` eroded a little more every `/dreaming` session. The rebuild now replays the section **in place** — surviving entries keep their positions, the new entry lands after the last pre-existing one, and non-entry lines are carried through — with eviction, supersede, and genuinely-last sections all unchanged. **The suite gap was precise:** the existing two-categories case creates two FRESH categories, so both of its writes took the *other* branch and the buggy one was never executed; no test mentioned "blank" or "separator". New case 4b asserts the separator survives a second entry into a non-last category, with the control that one existed first and the counter-case that a last section gains no spurious trailing blank, plus an in-file mutation control (gated on non-empty + differs + `bash -n`) proving the assertion goes red without the fix. 168 → 182 assertions, six of them added across review rounds. Four cover an empty-section arm the fix also introduced (a heading whose sole entry was retracted, then added into again — reachable in production via `retract`, and never reached by the section-9 retract fixtures, which always retract one of TWO entries); reverting the fix flips that arm's separator assertion too, so it was a SECOND instance of the same defect rather than a new edge case. Two more pin eviction and the separator **in combination** — the `<=3` cap was covered only in a single-category store, where the evicting section is always the last one and has no following heading to lose. Reverting the fix flips the separator half of that pair and leaves the eviction half green, which is what makes it a control rather than a restatement. The already-damaged store is repaired **whitespace-only** — LESSONS.md is a sole-writer store whose provenance keys on a content hash, so a hand-edited entry line would go invisible to `read-lessons.sh`; three sections had lost their separator, and all 13 entries verifiably still read back. Counts unchanged: 14 agents / 21 commands / 41 skills / 24 hooks.

> **NEW in v15.37.0 — the adjudication emit template, and a gate that checks templates PARSE:** `EXECUTE_CHECKPOINT`'s adjudication shape had no emit template, only prose — an agent emitting one from the base template was rejected by its own SubagentStop hook. A third labelled template is added in both kinds (`requires_gap` / `lane_collision`), and the templates are written in a shape `result_block_parser.py` can actually read: a wrapped flow sequence and a multi-line quoted scalar both fail to parse, so `adjudication_options` is a block sequence and the lane-collision `reason` is one line. The root cause was the gate — `parity-emit-block` checked that field NAMES appear, never that a template PARSES. New eval-corpus task `emit-block-parses` closes that, with its mutation table executed by `test-emit-block-parses.sh` rather than described in prose.
>
> **NEW in v15.36.0 — graphify/bridge tier retired; `/setup` twin module removed:** the code-graph + bridge enrichment tier is removed at every seam, on measurement — 0 recorded uses across all session logs, a 255-commit-stale bridge orphan its reader emitted nothing against, and no control arm on the spike's 71% gate score. The `brain-context` ladder now reads memos → repo-map → nothing (every removed consult was already a self-gating no-op with the graph absent, so callers' success paths are byte-identical). **User-facing removals:** the `/setup` **twin** cold-start bootstrap module (graph + bridge + starter CLAUDE.md) and the external-brain `LOOMWRIGHT_BRAIN_ROOT` path. `twin-graph.sh` is RETAINED untouched — it serves the live System Twin contract store, not the retired tier. D9 is split by regenerability (Derived → auto-delete freely; Distilled → never), and the reversal harness is committed verbatim under `docs/SPIKES/code-graph-harness/` — any revival must be a cheap incremental refresh that passes it. Counts unchanged: 14 agents / 21 commands / 41 skills / 24 hooks.
>
> **NEW in v15.20.0 — workers start warm and stay in their lane (D6):** Launch Pad's file-impact analysis is no longer thrown away after the brief is assembled. A new `scripts/build-context-digest.sh` writes a **bounded** per-job **context digest** (File Impact Map, interfaces touched, conventions, sibling-subtask summary, cross-lane contracts; contract at `docs/RESULT_SCHEMAS.md` §CONTEXT_DIGEST), and every worker receives it as a **pointer** — path + ≤200-char summary + "Read only the sections you need", never a pasted body. Worktree-resident workers get the **main-checkout absolute path** and are told so, because gitignored `.supervisor/` artifacts do not exist inside linked worktrees. Both carriers agree: the Task-spawn contracts and the SDK runner (`contextDigestPointer` / `laneGlobs`) compose the same pointer. Each subtask now also declares **`lanes:`** beside `provides`/`requires`; out-of-lane writes are recorded in a new **optional, additive, report-only `out_of_lane`** field — **no `WORKER_RESULT` schema bump** (it stays at 2, per the `memory_candidates` precedent), and lane data never touches `outputs_gap`, so the `status`⇔`outputs_gap` invariant and its brief-unreadable carve-out are untouched. A lane collision is flagged **only** between subtasks that are mutually UNREACHABLE in the `requires` DAG (same wave) — sequentially-ordered subtasks may legally share a file. New Plan Reviewer **Criterion 16** validates lane declarations. Also fixes a real silent-corruption bug: the SDK brief parser entered its contract block on only one of the two heading spellings in live use here, so on the other one the whole dependency graph vanished and every subtask went LAUNCHABLE in wave 1 — it now accepts both, **fails closed** on a zero-contract parse, and stops `lanes:` leaking into `requires`. Counts unchanged: 14 agents / 21 commands / 41 skills / 24 hooks.

> **NEW in v15.19.0 — Supervisor's frontmatter is skill-routed, not preloaded (4f), and briefs carry a base-commit freshness anchor (4g):** `loomwright:supervisor-runner`'s frontmatter `skills:` list drops from seven preloaded skills to **zero** (the key is omitted entirely), matching the inline `/supervisor` path's long-standing zero-preload pattern. Three skills are replaced by phase-entry Reads at their actual point of use (`async-orchestration` at Phase 2 PLAN + a Phase 4 compaction refresh, `commit` at Phase 4, `state-management` at Phase 4.5); the other four are removed outright after a per-skill audit found no point in the agent body that consumes them — each disposition is recorded with its evidence in `agents/supervisor.md` §"Preloaded Skill Routing (4f)". Spawn-time weight drops 51814 → 21029 proxy tokens; **note that is frontmatter weight, not a session total** — the four removals are an unconditional saving, the three routed skills are a redistribution, and `async-orchestration` is now paid roughly twice on a run that reaches FINALIZE (the deliberate price of the compaction-refresh guarantee). **4g:** Launch Pad now stamps `- **Base commit:** {sha}` on every brief, and Phase 1.5 PRE-FLIGHT SYNC gains a **third**, ADVISORY-ONLY signal (c) computing churn over the brief's anticipated paths — it never changes the CLEAR/OVERLAP/SUPERSEDED classification, never fires the soft-gate, and degrades silently on a missing or unparseable stamp (every pre-existing brief). No new gates. Counts unchanged: 14 agents / 21 commands / 41 skills / 24 hooks.
...
> **NEW in v15.11.0 — sdk-spike token levers (quarantined, opt-in) + per-subtask token accounting:** the quarantined `loomwright/sdk-spike/` runner gains a per-role effort config table (worker `medium` / reviewer `high`; `--effort` / `--worker-effort` / `--reviewer-effort` overrides validated fail-closed), an opt-in `--task-budget` per worker query (runner-enforced 20k minimum; omitted entirely when unset), and first-class per-subtask `token_usage` accounting (real usage from SDK result messages incl. failing-query fold-back; dry-run emits zeros labeled `proxy: true` — never invents tokens). Context editing is honestly recorded as NOT exposed by the pinned SDK 0.3.202 (capability-matrix gap; pin unchanged). Self-test 25/25 → 41/41; `FABLE_PARITY_EVAL.md` gains a datestamped additive recorded-observable amendment (decision rule byte-unchanged). Quarantine posture and default path unchanged. Counts unchanged: 14 agents / 21 commands / 41 skills / 22 hooks.
...
Optional next: run `/setup` for a status dashboard and guided configuration of optional capabilities (observability, telemetry, notifications).
...
| **Orchestrator**      | `/orchestrator goal: "..."`     | Plan work → default one task (Decomposition Threshold); no per-subtask review gate at any threshold — Phase 4.5 is the sole review | Starting implementation         |
