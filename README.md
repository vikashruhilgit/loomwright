...
>
> **NEW in v14.11.0 — `/obsidian`: a Full Linked Obsidian Vault of `.supervisor/`:** A new read-only, opt-in `/obsidian` command (backed by `scripts/build-vault.sh`, self-tested) projects the local `.supervisor/{twin,logs,memory}` trees into a configurable shared destination as a **Full Linked Obsidian Vault** at `<vault>/<slug>/` — cross-linked markdown you can browse, search, and graph in Obsidian. The projection is **read-only** (never mutates any source of truth), **content-hash idempotent** (unchanged inputs produce no rewrites), **sparse-tolerant** (absent twin/logs/memory subtrees degrade gracefully), and **decoupled** — no agent reads the vault back, so it can never feed stale state into a run. This is **+1 command (14 → 15)** — no new agent/skill/hook (still 13 / 15 / 50 / 19).
>
