...
- `preflight_sync` is optional (additive in v14.8.0, schema version unchanged at 1). When present, it MUST be either `null` or one of `"clear"`, `"overlap_proceed"`, `"superseded_proceed"`, `"skipped"`, `"unverified"`. Absent OR `null` means the Phase 1.5 PRE-FLIGHT SYNC gate did not run (legacy emission, or a pre-v14.8.0 `--continue` resume that lands after the gate). The Supervisor SubagentStop hook MUST NOT reject a SUPERVISOR_RESULT solely for the presence or absence of `preflight_sync` — pre-v14.8.0 blocks without this field remain valid (the hook does not enumerate it, mirroring the `branch_base` / `pr_state` additive precedent). The fail-closed abort path does NOT use a `preflight_sync` value; it emits `status: failed` with `error: "preflight_overlap_detected"` (see the `preflight_overlap_detected` AUTONOMOUS_RUN `status_reason` below).
