...
SUPERVISOR_RESULT:
  ...
  knowledge_sources_used: string[]     # optional (v14.28.0+) — advisory, non-gating telemetry: which memory sources the run actually consulted (lessons / project memory / per-agent memory / twin / brain-context). Open-set lowercase source tags: `project_memory`, `lessons:<category>`, `agent_memory:<agent>`, `twin:<path>`, `brain_context`. Absent ⇒ "none used". NEVER gates / changes `heal_decision` / blocks the PR; NOT enumerated by the Supervisor SubagentStop hook. Same data appears as a FLAT array on the `session_end` JSONL line (`build-insights.sh` reads the flat field) — the dual-shape precedent `contract_conformance` uses. Schema_version stays 1 — field is additive and optional.
```
...
CODE_REVIEW_RESULT:
  ...
  pattern_proposals: object[]          # optional — patterns for CLAUDE.md
    - pattern: string
      file: string
      description: string
  knowledge_sources_used: string[]     # optional (v14.28.0+) — advisory, non-gating telemetry: which memory sources the reviewer consulted (its own per-agent memory + project memory). Open-set lowercase tags: `project_memory`, `agent_memory:code-reviewer`, `lessons:<category>`, `twin:<path>`, `brain_context`. Absent ⇒ "none used". NEVER changes the `decision`, NEVER blocks the PR; NOT enumerated by the Code Reviewer SubagentStop hook. Additive — does NOT bump `schema_version` (stays 3).
...
- `knowledge_sources_used` is optional (additive in v14.28.0, schema version unchanged at 3). When present it MUST be an array of short lowercase source-tag strings (open set: `project_memory`, `agent_memory:code-reviewer`, `lessons:<category>`, `twin:<path>`, `brain_context`). Absent ⇒ "none used". The field is **advisory and non-gating**: it NEVER changes the `decision`, NEVER blocks the PR, and is NOT enumerated by the Code Reviewer SubagentStop hook, so blocks with or without it validate unchanged. Pre-v14.28.0 blocks without this field remain valid.
- `summary` must be present
...
- `knowledge_sources_used` (flat array, added v14.28.0) ⇔ `SUPERVISOR_RESULT.knowledge_sources_used` — the advisory, non-gating memory-usage telemetry array; absent ⇒ "none used"; the flat `session_end` array is the surface `build-insights.sh` reads (the nested SUPERVISOR_RESULT object is the same data in the other shape). Additive — no `schema_version` change.
