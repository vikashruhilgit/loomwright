...
SUPERVISOR_RESULT:
  ...
  knowledge_sources_used: string[]     # optional (v14.28.0+) — advisory, non-gating telemetry: which memory sources the run actually consulted (lessons / project memory / per-agent memory / twin / brain-context). Self-reported — records *claimed* usage, not machine-verified reads. Open-set lowercase source tags: `project_memory`, `lessons:<category>`, `agent_memory:<agent>`, `twin:<path>`, `brain_context`. Absent ⇒ "none used". NEVER gates / changes `heal_decision` / blocks the PR; NOT enumerated by the Supervisor SubagentStop hook. Same data appears as a FLAT array on the `session_end` JSONL line — the dual-shape precedent `contract_conformance` uses. (`build-insights.sh` reads the `session_end` line today but does NOT yet aggregate this field — `/insights` surfacing of it is a deferred follow-up.) Schema_version stays 1 — field is additive and optional.
  until_mergeable_dispatched: bool | null  # optional (v14.31.0+) — observability: `true` when the completion tail dispatched the `--until-mergeable` review-and-heal drain (Supervisor set `AI_AGENT_MANAGER_UNTIL_MERGEABLE=1` on the detached `review-pr-runner` process). `false`/`null`/absent ⇒ not dispatched (default-OFF / opted out / no PR). Records the dispatch DECISION, not the drain's outcome — the drain's `REVIEW_HEAL_RESULT` is the authority for readiness (see `skills/review-heal/SKILL.md`). NEVER gates / changes `heal_decision` / blocks the PR. Schema_version stays 1 — field is additive and optional.
  until_mergeable_log: string | null   # optional (v14.31.0+) — observability: path to the dispatched drain's run log (e.g. `.supervisor/logs/until-mergeable-<...>.log`) when `until_mergeable_dispatched` is true; `null`/absent otherwise. Informational only. Schema_version stays 1 — field is additive and optional.
```
...
- `until_mergeable_dispatched` and `until_mergeable_log` are optional (additive in v14.31.0, schema version unchanged at 1). When present, `until_mergeable_dispatched` MUST be `true`, `false`, or `null`; `until_mergeable_log` MUST be a non-empty string path or `null`. Absent OR `null`/`false` means the `--until-mergeable` drain was not dispatched (the default-OFF / opted-out / no-PR case). Both are **advisory and non-gating**: they record the dispatch DECISION for observability and NEVER change `heal_decision`, NEVER block the PR, and are NOT enumerated by the Supervisor SubagentStop hook, so blocks with or without them validate unchanged (mirroring the `branch_base` / `pr_state` / `preflight_sync` / `knowledge_sources_used` additive precedent). Pre-v14.31.0 blocks without these fields remain valid. The drain's own `REVIEW_HEAL_RESULT` (schema v2) remains the authority for the drain's readiness outcome — these two SUPERVISOR_RESULT fields only record that Supervisor dispatched it and where its log lives.
...
**v14.31.0 additive fields (backwards-compat):** `until_mergeable_dispatched` and `until_mergeable_log` are purely additive optional observability fields following the same precedent. They record whether the completion tail dispatched the opt-in `--until-mergeable` review-and-heal drain (and where its log lives) — advisory only, never gating. The Supervisor SubagentStop hook does NOT enumerate them, so blocks with or without them validate unchanged, and pre-v14.31.0 consumers ignore them. Schema_version remains `1`.
...
- `knowledge_sources_used` (flat array, added v14.28.0) ⇔ `SUPERVISOR_RESULT.knowledge_sources_used` — the advisory, non-gating memory-usage telemetry array; absent ⇒ "none used"; the flat `session_end` array is the surface `build-insights.sh` *will* read once aggregation lands (the field is emitted today but `build-insights.sh` does not yet extract/aggregate it — `/insights` surfacing is a deferred follow-up). The nested SUPERVISOR_RESULT object is the same data in the other shape. Additive — no `schema_version` change.
...
REVIEW_HEAL_RESULT:
  ...
  rounds: int                          # drain rounds actually run (the --max-rounds-bounded outer loop)
  max_rounds: int                      # the drain hard ceiling (default 5; --max-rounds N overrides)
...
- `READY` (**v2 only, `--until-mergeable`**) → all **required** checks green AND no unresolved **bot-authored** review threads remain; the "ready to merge" notification fired (`notified: true`). Human approval, `reviewDecision: REVIEW_REQUIRED`, and human-authored unresolved threads are explicitly **NOT** readiness blockers — the loop never waits on a human (AC3). `READY` is terminal-stop-and-notify, **merge-identical to `PASS`/`ESCALATED`** — the PR is left open for a human (no `gh pr merge` is ever invoked, AC6).
- `ESCALATED` (v1/v2) → reviewer escalated (`NEEDS_HUMAN`) **or** the loop exhausted its bound (`max_heal_iterations` in v1; `max_rounds` in v2) with issues remaining, **or** (v2) a fail-closed condition tripped (GraphQL review-thread query errored → thread-state unknown; required-check metadata unavailable without `--required-checks all-non-neutral`). Findings are posted to the PR via `gh pr comment`, notifications fired best-effort, PR left open. Never auto-fixes past escalation, never merges.
