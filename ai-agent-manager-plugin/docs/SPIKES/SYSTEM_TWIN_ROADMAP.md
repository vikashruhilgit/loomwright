...
| **2 Prove** | Phase 4.5 conformance check (advisory, never blocks) + a **canary** benchmark (`selftest_pass_count`, fixed corpus) | **Ground-truth execution** of the real app/tests (the QA Executor Playwright muscle is NOT wired into the gate); executable acceptance contracts |
...
**Net (as of v14.16.0):** the *spine* is in and advisory; **M0 + M1 shipped**. The Twin is accumulating (7 contracts) with incident-history + a derived blast-radius graph. We have predict + prove + compound — none of it gating yet. **Next rung: M2.**
