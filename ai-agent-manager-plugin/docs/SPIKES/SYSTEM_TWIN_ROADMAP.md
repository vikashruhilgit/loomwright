...
| **2 Prove** | Phase 4.5 conformance check (advisory, never blocks) + a **canary** benchmark (`selftest_pass_count`, fixed corpus) + the **eval instrument** (`run-eval.sh` over a task corpus, `pass_rate` fitness signal — M2a ✅, v14.17.0) + a **generic ground-truth execution** step (`run-ground-truth.sh` runs a brief's declared `## Executable Acceptance` `cmd:`/`corpus-task:` checks in Phase 4.5, advisory — M2b slice 1a ✅, v14.19.0) | The QA Executor Playwright muscle auto-dispatched for web-app repos (M2b **slice 1b**, deferred); auto-running the agent loop in CI against the eval corpus (M2b **part-2**, deferred) |
...
**Net (as of v14.19.0):** the *spine* is in and advisory; **M0 + M1 shipped, M2a (the eval instrument) shipped**, and **M2b slice 1a shipped** — a generic executable-acceptance **ground-truth execution** step (`run-ground-truth.sh`) is now wired into Phase 4.5 (advisory), so the gate can run a brief's declared `cmd:`/`corpus-task:` checks and report a hard pass/fail. The Twin is accumulating (7 contracts) with incident-history + a derived blast-radius graph. We have predict + prove + compound — none of it gating yet. **Next rung: M2b slice 1b** (auto-dispatch the QA Executor's Playwright muscle for web-app repos) **+ M2b part-2** (auto-run the full agent loop in CI against the corpus).
...
3. **M2b slice 1b + part-2 (the big remaining build):** M2a (eval instrument) and M2b slice 1a (the generic `run-ground-truth.sh` executable-acceptance path wired into Phase 4.5, advisory) have shipped (v14.17.0 / v14.19.0). Next: **slice 1b** — auto-dispatch the QA Executor's Playwright/app-execution muscle for web-app repos (today `qa-executor:` checks are recognized-but-skipped) — and **part-2** — auto-run the full Launch Pad→Supervisor agent loop in CI against the corpus. This is the structural unlock for "provable-done" and the prerequisite for every milestone above it.

---

## 7. Open decisions / risks to resolve as we climb

- **`cmd:` trust mitigation has no enforcement home yet (owner: M2b slice 1b, before any gating).** The M2b slice 1a ground-truth runner executes arbitrary `bash -c` from a brief's `## Executable Acceptance` `cmd:` bullets, automatically and unattended in Phase 4.5 (incl. under `/autonomous`, where the brief is machine-authored). The documented mitigation — "review `cmd:` bullets at Plan Review" — is currently **prose-only**: `plan-reviewer.md` and `launch-pad.md` do not yet inspect that section. This is acceptable while ground-truth is advisory (it can never block/gate), but the control MUST be wired into a prompt (a Plan Reviewer check on `## Executable Acceptance` + a Launch Pad authoring convention), and/or an opt-in `--no-cmd` mode for the autonomous path, **before** slice 1b expands execution or M3 flips advisory → gating. Until then a malicious/buggy `cmd:` bullet runs with no review step asserting on it.
