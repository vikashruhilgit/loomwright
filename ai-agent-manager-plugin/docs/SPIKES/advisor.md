...
Can the Anthropic **Advisor tool** (executor model + higher-intelligence advisor sub-inference, e.g. Sonnet/Haiku executor + Opus advisor) be invoked from plugin subagents that ai-agent-manager spawns via Claude Code's Task tool — or is it currently reachable only by calling `/v1/messages` directly with a beta header?
