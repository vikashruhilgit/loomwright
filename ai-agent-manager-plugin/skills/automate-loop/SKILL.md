...
- **Inputs the engine already holds:** from the step-3 `REVIEW_HEAL_RESULT` read `fix_cycles`, `repeat_check_failure`, `unresolved_bot_feedback`, and `decision` (the drain result `READY|ESCALATED`); from the step-2 `SUPERVISOR_RESULT` read `repo`, `number`, `pr_url`, `branch` (and `heal_decision`/`rubric_score` for the summary).
