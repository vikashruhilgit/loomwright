...
**Plugin-self authoring convention (v14.21.0):** a machine-authored brief generated by Launch Pad **on this plugin's own repo** whose change touches the **plugin doc surface** SHOULD declare both doc-surface invariants. This convention is the single canonical home for the plugin doc-surface set (authority: the surfaces `scripts/check-doc-currency.sh` validates, plus the count sources) — `agents/`, `commands/`, `skills/`, `docs/`, `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `.claude-plugin/README.md`, `hooks/hooks.json`, `CLAUDE.md`, `README.md` (each directory subsumes its index/help files, e.g. `skills/SKILLS_INDEX.md`, `commands/agent-help.md`). This is the **doc-currency** surface — distinct from `code-reviewer.md`'s review-trigger taxonomy (which additionally covers `.supervisor/jobs/**`). Declare:
