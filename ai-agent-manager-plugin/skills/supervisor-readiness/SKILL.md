...
**Plugin-self authoring convention (v14.21.0):** a machine-authored brief generated by Launch Pad **on this plugin's own repo** whose change touches the doc surface (`agents/`, `commands/`, `skills/`, `docs/`, `plugin.json`, `marketplace.json`, `hooks.json`, `CLAUDE.md`, `README.md`, `SKILLS_INDEX.md`) SHOULD declare both doc-surface invariants:
