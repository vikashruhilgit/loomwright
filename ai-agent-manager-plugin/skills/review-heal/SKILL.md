...
| Scoped check-wait bound | **`--check-wait-timeout N`** (seconds) | Bounded wait for the **scoped set** (required + review-producing checks) to settle (§"Wait-For-Settled-Checks"). Only applies under `--until-mergeable`; defaults to a conservative bound. Forwarded from the dispatcher via `AI_AGENT_MANAGER_CHECK_WAIT_TIMEOUT`. |
...
- **check-run output/annotations** → the annotation `message`/`title` and check `output.{title,summary,text}` **text**. Because a check-run has no `.user.login`/`.body` shape, **synthesize a comparable object** per check-output finding: `{user:{login:<check.app.slug or check.name>}, body:<output/annotation text>}` so it can flow through the shared classifier unchanged.

> Reading `reviews[].state` / a comment's author / a check's `conclusion` **alone is insufficient**: those are metadata. The actionable finding — like #64's MEDIUM — lives in the **body/output text**. Classify on `(login, body)`; never on state/conclusion in isolation.

**Classify via the shared helper — do NOT re-implement the regexes.** Pipe each channel's items (the real `{user.login, body}` objects, or the synthesized check-output objects) through **`ai-agent-manager-plugin/scripts/classify-bot-review.sh`** (stdin = JSON array of comment-like objects, stdout = only the bot-authored review findings, original objects passed through; empty/invalid → `[]`, exit 0). The classifier owns `bot_author_re` / `review_marker_re` as its single source of truth — this skill **never** redefines them. Concatenate the per-channel classifier outputs into one finding set; the readiness decision is computed over that **UNION**.

```
bot_findings = (
    classify(reviews ∪ latestReviews bodies)
  ∪ classify(reviewThreads first-comment bodies)        # only unresolved threads feed the blocker set
  ∪ classify(issue_comments)
  ∪ classify(synthesized check-output objects)
...
```
deadline = now + check_wait_timeout          # --check-wait-timeout N (seconds); conservative default
