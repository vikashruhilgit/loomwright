...
2. **Resume-thrash guard (if this is a `--continue` run landing in SELF_HEAL):**
   - `Context-Keeper(operation: record_self_heal_resume, increment: true)` → reads the returned count
   - If count ≥ 3: abort the loop, mark task `completed_with_escalation` with reason `"self_heal_resume_thrash"`, skip to completion tail with `heal_loop_ran=true, heal_decision=ESCALATED`
3. Check `--skip-self-heal` flag: if set, record `record_decision(phase: SELF_HEAL, decision: "loop_skipped", rationale: "--skip-self-heal flag")` and jump to completion tail with `heal_loop_ran=false`.
