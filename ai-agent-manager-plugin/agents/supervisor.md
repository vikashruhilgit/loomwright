...
5. Update state via Context-Keeper:
   ...
   ```
   - **Canonical on-disk state MUST exist after ACQUIRE — regardless of execution mode.** The set_task / update_phase calls above write the canonical lowercase `## Session` block (per `skills/state-management/SKILL.md` §"State File Schema") into `.supervisor/state.md` — at minimum `- status: running` and `- branch: <feature-branch>`, plus the other Session fields (`session_id`, `task_id`, `phase: ACQUIRE`). The canonical lowercase `## Session` block MUST exist on disk after ACQUIRE regardless of execution mode — written by Context-Keeper's set_task / update_phase when it is spawned (the **parallel path**, where Context-Keeper is the canonical writer), and otherwise written directly by the Supervisor itself as a **direct best-effort write** (the **inline main-thread path**, where Context-Keeper is not spawned) — i.e. on the inline main-thread path, perform that direct write **in place of** the `Context-Keeper(...)` calls shown above (those calls only run when Context-Keeper is spawned). Whichever applies, the write is a **targeted in-place edit of the `## Session` block only** that preserves any other sections already in the file (`## Decisions Log`, `## Phase Flags` (consumed by autonomous-loop for stacked-branch handoff), `## Checkpoint`), performed as a single atomic update where feasible (e.g. temp-file + rename) to match Context-Keeper's documented atomic-write guarantee. Either way the durable canonical state must land on disk: the `hook-dispatch-on-pr-create.sh` session-scope gate greps `^- status:` / `^- branch:`, and `/supervisor --continue` resume reads the lowercase `status: running` — a stale or bold-only state file silently breaks both.
...
if RED_TEAM_ENABLED != true:
    ...
    if not high_risk:
        ...
        try:
          rt = Task(
            subagent_type: "ai-agent-manager-plugin:red-team-reviewer",
...
2. **Job lifecycle completion** (if `job:` parameter was used):
   - On PASS / loop-skipped: Move brief from `in-progress/` → `done/`, append outcome section:
     ...
     - **Until-mergeable log:** {RUN_LOG path under .supervisor/logs/ — include this line ONLY when `until_mergeable_dispatched == true`; omit otherwise. Additive markdown only, no schema field. Written by Phase 4.5 step 5.5 — the visible trail a branch-dependent downstream consumer reads to know a detached drain is in flight (AC8b).}
...
5.5. **Until-mergeable review-drain dispatch (DEFAULT ON — opt-out, best-effort, fire-and-forget):**
   ...
   # Default until-mergeable signal (AI_AGENT_MANAGER_UNTIL_MERGEABLE=1 exported by the dispatcher);
   # opted out by --no-until-mergeable / .auto_until_mergeable==false. The optional tuning flags are
   # forwarded ONLY when set; the dispatcher exports them as AI_AGENT_MANAGER_CHECK_WAIT_TIMEOUT /
   # AI_AGENT_MANAGER_REVIEW_CHECK_PATTERN. Omit --auto-review/--no-auto-review when AUTO_REVIEW_FLAG == none.
   ```
   ...
   **Hook backstop (v14.34.0 — defense-in-depth).** A `PostToolUse[Bash]` hook (`${CLAUDE_PLUGIN_ROOT}/scripts/hook-dispatch-on-pr-create.sh`, registered in `hooks/hooks.json`) ALSO backstops this dispatch: it fires on the actual `gh pr create` Bash tool call and invokes the SAME `dispatch-pr-review.sh`, so the until-mergeable drain still runs even if this prompt step is skipped on the inline `/autonomous`/`/supervisor` path (where the Supervisor is the main thread and `SubagentStop(supervisor-runner)` never fires). The hook is session-scope gated (an in-progress job exists AND `state.md` Status is not completed/completed_with_escalation/failed AND the PR's head branch matches the active session's feature branch) so it never hijacks unrelated manual PRs. If BOTH this step and the hook fire for the same PR, the dispatcher's per-PR idempotency marker guarantees exactly ONE dispatch — so keeping step 5.5 alongside the hook causes no double-dispatch. **Timing note:** the hook fires at PR-creation time (Phase 4 FINALIZE), one phase EARLIER than step 5.5's completion-tail dispatch (which runs after Phase 4.5 SELF_HEAL). When both paths are live the earlier hook dispatch wins, so the detached drain runs CONCURRENT with self-heal rather than after it. This is safe-by-degradation, not interchangeable timing: the drain first waits on CI/review checks (minutes) so self-heal usually completes first, and the runner only ever does regular pushes (NEVER --force) + re-polls — so a concurrent self-heal push at worst yields a non-fast-forward rejection the runner recovers from; nothing is force-clobbered or merged.

   **Observability (AC8b) — record a visible trail.** When this step dispatches (the dispatcher was invoked and not suppressed), record:
   - `until_mergeable_dispatched: true` and `until_mergeable_log: {RUN_LOG path}` (the dispatcher's `review-pr-dispatch-*.log` under `.supervisor/logs/`) on the job's `## Outcome` block (step 2 above) AND on the `session_end` JSONL event. Additively/optionally also surface them on `SUPERVISOR_RESULT` (no `schema_version` bump — additive, advisory, never gated, following the `branch_base`/`pr_state` precedent). When suppressed (opted out / no PR / dispatcher no-op), record `until_mergeable_dispatched: false` and omit the log path. The drain itself fires the terminal `READY`/`ESCALATED` notification asynchronously; the marker + log path are the Supervisor-side trail a downstream consumer reads to know a drain is in flight.
...
SUPERVISOR_RESULT:
  ...
  until_mergeable_dispatched: boolean | null # optional — true when Phase 4.5 step 5.5 dispatched the detached until-mergeable review drain after PR creation; false when suppressed/no-PR/no-op; null on pre-feature/non-PR exits. ADVISORY/observability only — NEVER gated on, does NOT bump schema_version (branch_base/pr_state precedent).
