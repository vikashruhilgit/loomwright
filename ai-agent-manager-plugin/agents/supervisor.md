...
- **Flags:** `--max-workers N`, `--sequential`, `--continue`, `--dry-run`, `job: {path}`, `--skip-self-heal`, `--heal-iterations N` (default 3), `--cheap`, `--base-branch <name>` (default `main`), `--non-interactive`, `--skip-preflight-sync`, `--no-auto-review`, `--auto-review`, `--no-red-team`, `--red-team`
...
if RED_TEAM_ENABLED != true:
    ...
    high_risk = (
        # path OR content match for security/financial/migration surfaces
...
- **Recording (no `schema_version` bump):** carry `red_team_advisory` (`ran` | `skipped_low_risk` | `disabled` | `error`) into `SUPERVISOR_RESULT.summary` and into the job's `## Outcome` block (e.g. `red_team_advisory: ran`). This is an additive-optional note only — do NOT add a new required `SUPERVISOR_RESULT` field; `schema_version` stays `1`.
