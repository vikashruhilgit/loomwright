...
When this runner is launched via the **`--agent` form** (`claude -p --agent ai-agent-manager-plugin:review-pr-runner "<pr-url>"` — Supervisor's auto-dispatch path, which has **no flag surface** so the positional is the PR URL, not a slash string), it MUST read the until-mergeable signal from environment variables and **translate them into flags** on its inline `/review-pr <pr-url>` invocation:

| Env var (set by the dispatcher) | Runner forwards to inline `/review-pr` |
|---|---|
| `AI_AGENT_MANAGER_UNTIL_MERGEABLE` truthy (`1`/`true`) | `--until-mergeable` (absent/falsy ⇒ default diff-only loop, AC7 unchanged) |
| `AI_AGENT_MANAGER_CHECK_WAIT_TIMEOUT` set | `--check-wait-timeout <value>` (forwarded ONLY when set) |
| `AI_AGENT_MANAGER_REVIEW_CHECK_PATTERN` set | `--review-check-pattern <value>` (forwarded ONLY when set) |
