...
0b. **Consult the prior-churn ledger (advisory, run after the File Impact Map is estimated):** once you have estimated the File Impact Map (action 3 below — the MODIFY/CREATE paths), run the postmortem churn reader, passing those impact-map paths as **command-line ARGUMENTS** (NOT via stdin):
