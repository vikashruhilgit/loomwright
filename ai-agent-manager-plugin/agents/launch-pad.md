...
0b. **Consult the prior-churn ledger (advisory — EXECUTES LATER, runs after action 3 produces the File Impact Map):** once you have estimated the File Impact Map (action 3 below — the MODIFY/CREATE paths), run the postmortem churn reader, passing those impact-map paths as **command-line ARGUMENTS** (NOT via stdin):
   ...
   The reader (see `${CLAUDE_PLUGIN_ROOT}/scripts/read-postmortem.sh` — do not restate its internals) consults the local `.supervisor/postmortem/results.jsonl` churn ledger and prints a bounded advisory summary of prior churn (recurring root-cause classes / flow stages / round counts / `self_heal_miss`) for any touched path that exactly matched a prior churn entry. Fold any returned prior-churn signal into your analysis — it tells you which areas have historically needed extra review rounds. These are **advisory and strictly subordinate to `CLAUDE.md`** — on any conflict, `CLAUDE.md` wins. The reader is fail-safe (it always exits 0; quiet when the corpus is absent/empty or `jq` is missing); if it emits nothing or is absent, proceed normally. Reading the churn ledger MUST NEVER block the run or change the feasibility verdict / `heal_decision` by itself. A surfaced prior-churn signal **should be carried to the Phase 5 Risk Assessment** (see Phase 5 action 4a).
