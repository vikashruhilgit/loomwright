...
| `--check-wait-timeout N` | No | conservative bound | Seconds to **bounded-wait** for the *scoped* set (required + review-producing checks) to settle before each READY test, so a late-posting review check is seen rather than missed; it NEVER waits on unrelated optional checks. Only meaningful with `--until-mergeable`. **Authoritative definition:** `review-heal` skill §"Wait-For-Settled-Checks". |
| `--review-check-pattern <glob>` | No | `*review*` / `claude*` | Glob(s) that mark a check as **review-producing** (in addition to required checks), widening the scoped wait/scan set; combinable with the `notify-config` include/exclude list (exclude wins). Only meaningful with `--until-mergeable`. **Authoritative definition:** `review-heal` skill §"All-Channel Read" / §"Wait-For-Settled-Checks". |
