...
The worker-proposed durable facts harvested from the gathered sources. `/dreaming` scans the session logs, the `.supervisor/worker-summaries/*.md` files, and the done/failed briefs for worker `WORKER_RESULT.memory_candidates[]` strings — the optional field workers have emitted since v14.4.0 (they also echo the same strings into their `.worker-summary.md`, which is why both are scanned). The collected strings are **deduped** against the existing project memory — `/dreaming` reads the verified facts via `bash "${CLAUDE_PLUGIN_ROOT}/scripts/read-project-memory.sh"` first and drops any candidate already present (and dedups duplicates among the candidates themselves). Each surviving unique candidate is listed:
