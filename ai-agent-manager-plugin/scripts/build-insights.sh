...

{
  ...
  #
  # This block is a DORMANT placeholder: the heading/anchor exists for a future iteration (a
  # brain-lift panel), but the guard below is permanently false in v1, so nothing is consumed and the
  # existing results.jsonl section's behavior is unchanged. To activate later, flip the guard and
  # render a trend from $brain_baseline — but only once we've decided to surface a brain-lift panel.
  ...
  if false; then
    # DEAD in v1 — do not consume brain-baseline.jsonl into the active trend. Left as the wiring
    # point for a future brain-lift panel; flipping `if false` to a real check is a deliberate,
