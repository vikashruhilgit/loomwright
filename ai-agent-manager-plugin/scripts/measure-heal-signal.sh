...
#   2. $AI_AGENT_MANAGER_HEAL_SIGNAL_REPOS   (newline- or colon-separated)
...
if [ "${#REPOS[@]}" -eq 0 ] && [ -n "${AI_AGENT_MANAGER_HEAL_SIGNAL_REPOS:-}" ]; then
  # split on newline OR colon (trailing \n so a single un-terminated path is not dropped)
  while IFS= read -r _r; do
    [ -n "$_r" ] && REPOS+=("$_r")
  done < <(printf '%s\n' "$AI_AGENT_MANAGER_HEAL_SIGNAL_REPOS" | tr ':\n' '\n\n')
fi
