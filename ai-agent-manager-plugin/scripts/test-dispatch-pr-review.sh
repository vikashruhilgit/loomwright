...
# pointed at via AI_AGENT_MANAGER_CLAUDE_BIN. All under temp dirs.
# ----------------------------------------------------------------------------
...
# repo, with the stub bin on PATH and AI_AGENT_MANAGER_CLAUDE_BIN -> stub-claude.
# Captures rc in RUN_RC. Stub git fetch failures are tolerated (no origin remote).
run_real() {
  local repo="$1"; shift
  ( cd "$repo" && PATH="$FX_BIN:$PATH" \
      AI_AGENT_MANAGER_CLAUDE_BIN="$FX_BIN/stub-claude" \
...
( cd "$FX_REPO" && PATH="$FX_BIN:$PATH" AI_AGENT_MANAGER_CLAUDE_BIN="definitely-not-a-real-claude-xyz" \
