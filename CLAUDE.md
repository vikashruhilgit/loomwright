...
**v15.2.0 — `--cheap` cost-profile passthrough through `/automate → /autonomous → /supervisor` (flag-plumbing + docs, additive):** `--cheap` is now a real, forwarded flag on both outer loops. `/autonomous` parses it at INIT and appends it to **every** inlined `/supervisor job:` invocation at EXECUTE step 1 — unconditional-when-present across single/multi-iteration and stacked/non-stacked, NOT gated on `--non-interactive-fallback`; the previously-dangling "Auto-forwarded flags" subsection is now defined there, enumerating the complete forward set (`--base-branch` / `--non-interactive` / `--cheap`). `/automate` passes `--cheap` through to its inner `/autonomous --single-iteration` RUN invocation as a **pure passthrough** — never interpreted by the engine, NOT persisted in the run file's `## Run Config` (re-pass on `--resume` / `/loop` ticks). The Supervisor cost-profile engine is reused **unchanged**; Haiku-session caveat stays single-sourced in `docs/ARCHITECTURE_CONTRACTS.md` §"Cost Profiles". Strictly additive/opt-in — without the flag both loops are byte-for-byte unchanged. **Counts UNCHANGED: 14 agents / 21 commands / 57 skills / 21 hooks; no `schema_version` bump.** Additive on top of v15.1.0.

**v15.1.0 — advisory house-rules ENFORCEMENT wired at 3 seams + `/rules add`/`check` mechanized (slice #3b-ii, additive):** Completes the `.agent/rules/` House Rules slice. **(1) Advisory enforcement at 3 seams (never-gating, mirroring `prior_churn`):** the fail-safe reader `read-rules.sh` is now consumed at the worker DO-side (`agents/supervisor.md` fast-path spawn + `agents/execute-manager.md` parallel spawn inject applicable rules into worker prompts), at the Phase 4.5 self-heal REVIEW lens (`agents/supervisor.md` + `skills/self-heal-advisory/SKILL.md` thread a HOUSE-RULES ADVISORY line into the `code-reviewer` prompt), and as a SessionStart nudge folded into `scripts/session-resume.sh` (hook-neutral, gated on the reader emitting NON-EMPTY output). All three are strictly advisory / fail-safe — they NEVER change a `heal_decision`, a review `decision`, or gate/block a PR, and are subordinate to CLAUDE.md. **(2) `/rules add` mechanized** into the sole-writer `scripts/add-rule.sh` (path-containment slug + `/`/`..`/leading-dot/metachar/empty REJECTION + array-only parse-gate + atomic temp-`mv` + read-back verify), proven by `scripts/test-add-rule.sh` (asserts `../escape`/`a/b`/`.hidden`/metachar/empty categories are rejected with no traversal write). **(3) `/rules check` mechanized** into `scripts/rules-check.sh` — the sole execution path — with a default-off `--no-cmd` unattended trust valve (mirroring `run-ground-truth.sh --no-cmd`; `--no-cmd` WINS over `--confirm`); no advisory seam ever enables execution and the reader still never executes a `check`. New static self-test `scripts/test-rules-seams.sh` asserts each of the 4 seams references `read-rules.sh`, never references `rules-check.sh`, and never pipes reader output into a shell executor. `applies_to` path/scope filtering remains inert (a later slice). **Counts UNCHANGED: 14 agents / 21 commands / 57 skills / 21 hooks; no `schema_version` bump.** Additive on top of v15.0.0.

> 📜 **Full release history** (v15.0.0 → v14.0.0 and earlier) lives in [`CHANGELOG.md`](CHANGELOG.md). CLAUDE.md keeps only the two most recent release notes.
...
- Plugin manifest: `loomwright/.claude-plugin/plugin.json` (v15.2.0)
