...
**v14.18.0 — `/insights` becomes the unified System-Twin scoreboard:** Turns the local `/insights` dashboard into the deterministic run-scoreboard for the System Twin's fitness + growth signals — **advisory, additive, no counted artifacts.** **(1) `scripts/run-eval.sh`** now best-effort appends each `EVAL_RESULT` (plus a `recorded_at` UTC field) as one JSON line to the gitignored, git-root-relative `.supervisor/eval/results.jsonl` (atomic, fail-safe, still always exits 0), with an `EVAL_RESULTS_FILE` override and a `--no-record` flag; `scripts/test-run-eval.sh` extended (now 9 assertions). **(2) `scripts/build-insights.sh`** gains two always-rendered dashboard sections — **Eval fitness function** (latest pass-rate + ` → ` trend from `results.jsonl`) and **System Twin growth** (current contract count from `.supervisor/twin/contracts/*.md` + cumulative `add`-event growth from `.supervisor/twin/.provenance.jsonl`) — both sparse-tolerant ("no data yet"), jq-deterministic, exit 0; `scripts/test-insights.sh` extended (now 39 assertions). **(3) `commands/insights.md`** gains the distinct-from-Claude-Code-Insights / `ccusage` "deterministic run scoreboard" framing + docs for the two sections. **No new agent/command/hook/skill (still 14 / 16 / 51 / 19); new scripts uncounted; additive (EVAL_RESULT stays schema_version 1).** Additive on top of v14.17.0.

**v14.17.0 — System Twin M2a: a deterministic eval harness (the real fitness function):** Lands the **eval instrument** slice of M2 (Pillar 2 "provable-done") — **advisory, additive, no counted artifacts.** **(1) `scripts/run-eval.sh`** — a deterministic runner/scorer that measures plugin *output quality* against a fixed task corpus: each task is a self-contained `eval-corpus/<task-id>/` dir carrying an executable `check.sh` (exit 0 = pass), and the script reports the **`pass_rate` M/N** as the fitness signal tracked release-over-release (the determinism invariant covers the tallies/`per_task`, NOT the contextual `commit`/`date`). It is **fail-safe** (missing corpus or no `jq` → `status:"unverified"`, `0/0`, always exits 0). **(2) Corpus format + a 4-task seed corpus** under `scripts/eval-corpus/` (`doc-currency-green`, `eval-selftest-green`, `fixture-unit-test`, `version-consistent`). **(3) New `EVAL_RESULT` schema** (`schema_version 1`) in `docs/RESULT_SCHEMAS.md`. **(4) `scripts/test-run-eval.sh`** self-test. The eval harness is **distinct from the canary benchmark** (`run-benchmark.sh` / `BENCHMARK_JSON`) — "eval" ≠ "benchmark", kept separate by name and dir. **M2b is explicitly deferred:** auto-running the full Launch Pad→Supervisor agent loop in CI against the corpus, and wiring ground-truth execution into Phase 4.5. **No new agent / command / hook / skill (still 14 / 16 / 51 / 19); new scripts are uncounted; additive schema.** Additive on top of v14.16.0.
...
- Plugin manifest: `ai-agent-manager-plugin/.claude-plugin/plugin.json` (v14.18.0)
