...
**v15.3.0 — fail-closed resume validation gate for `.supervisor/state.md` (`resume_state_invalid`; prompt-surface, additive):** `/supervisor --continue` now refuses to act on a loaded state file whose `## Session` block doesn't parse against the known schema — the same fail-closed philosophy as the Phase 1.5 preflight and rubric gates, closing the one remaining trust-unvalidated-local-state hole (every result block is hook-validated; resume state was not). The gate lives ONCE at the earliest consumption point (`agents/supervisor.md` Phase 0 INIT step 2, BEFORE any loaded state — including `cost_profile` hydration — is consumed); the Phase 1 ACQUIRE resume read references that gate instead of duplicating the enums (mirror-drift avoidance). Contract (authoritative in `skills/state-management/SKILL.md` §"Resume validation gate"; skill 1.1.0 → 1.2.0 + SKILLS_INDEX row in the same change): `## Session` block must exist; `phase` ∈ `INIT | ACQUIRE | PLAN | EXECUTE | FINALIZE | SELF_HEAL | LOOP` and `status` ∈ `running | paused | completed | completed_with_escalation | failed` (verbatim from §"State File Schema"; `PRE_FLIGHT_SYNC` is a `record_decision`-only Decisions Log label, NOT a valid state phase); an asserted `branch:` must `git rev-parse --verify`. ANY violation ⇒ refuse the resume with `SUPERVISOR_RESULT.status: failed` + the closed additive error `resume_state_invalid` (documented in RESULT_SCHEMAS.md prose per the `preflight_overlap_detected` precedent; NO `schema_version` bump; deliberately NOT an AUTONOMOUS_RUN `status_reason` — the autonomous loop never invokes `--continue`, so `supervisor_failed_other` case (a) is the catch-all) and a user instruction to inspect/delete `.supervisor/state.md` — NEVER a silent fresh-start fallback, and NO escape-hatch flag in v1 (deleting the file IS the escape hatch). READ-side gate only: the Context-Keeper sole-writer contract and inline-path write responsibility are untouched; a MISSING state file still starts fresh unchanged; a VALID file resumes identically. Mirrored in `commands/supervisor.md` (`--continue` Parameters row + Troubleshooting entry) and a CLAUDE.md Common-Pitfalls line. **Counts UNCHANGED: 14 agents / 21 commands / 57 skills / 21 hooks.** Additive on top of v15.2.3.

> 📜 **Full release history** (v15.2.3 → v14.0.0 and earlier) lives in [`CHANGELOG.md`](CHANGELOG.md). CLAUDE.md keeps only the two most recent release notes.
...
- Plugin manifest: `loomwright/.claude-plugin/plugin.json` (v15.4.0)
