...
**v15.0.0 — Loomwright / atelier naming baseline (mechanical):** Establishes the plugin id `loomwright` (directory + `plugin.json` `name`), the marketplace id `atelier` (`.claude-plugin/marketplace.json` `name`), the persistent-memory directory convention (`.claude/agent-memory/loomwright:<agent>/`), the `-runner` registered-agent IDs, and the `LOOMWRIGHT_*` environment-variable prefix (25 vars). **No functional/behavioral change** — mechanical release only. **Counts unchanged: 14 agents / 21 commands / 57 skills / 21 hooks; no `schema_version` bump.**

> 📜 **Full release history** (v14.51.0 → v14.0.0 and earlier) lives in [`CHANGELOG.md`](CHANGELOG.md). CLAUDE.md keeps only the two most recent release notes.
...
- Plugin manifest: `loomwright/.claude-plugin/plugin.json` (v15.1.0)
