...
State writes are confined to `.supervisor/autonomous/{session_id}/` (the loop's own state) and `.supervisor/requirements/` (the requirement files). Supervisor remains the sole writer of `.supervisor/jobs/` and `.supervisor/state.md` per existing contracts. `AUTONOMOUS_RUN` uses an autonomous-layer-only status enum (`done | paused_max_iterations | aborted | failed`) intentionally distinct from `SUPERVISOR_RESULT.status` to avoid schema collision; the summary is plain markdown plus a JSON sidecar (no hook validation, no resume contract in v1 — those are future work).
