...
**v14.35.0 — back-compatible rename of `.supervisor/notify-config.json` → `.supervisor/config.json` (additive):** The run-behavior config file is renamed back-compatibly. Readers now prefer `.supervisor/config.json` and fall back to the legacy `.supervisor/notify-config.json` (the new path wins when both exist), so existing installs keep working with no action — migrate by `mv`-ing the file or simply rely on the fallback. Ships the scripts back-compat read, a full doc sweep (README current-state references + a migration note; dated historical banners/CHANGELOG entries left frozen), and the version bump. **No new agent/command/skill/hook; counts unchanged at 14 agents / 18 commands / 55 skills / 20 hooks; no `schema_version` bump.** Additive on top of v14.34.0.
...
- Plugin manifest: `ai-agent-manager-plugin/.claude-plugin/plugin.json` (v14.35.0)
