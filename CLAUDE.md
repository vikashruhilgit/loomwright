...
**v14.41.0 — `/automate` generic automation engine (single run file → per-item loop, additive):** A new `/automate` command + `automate-loop` authority skill that walks **arbitrary work from any starting point** to a reviewed PR (and, opt-in, a trusted merge). It converts **any source** — a prompt (via `/product-owner`), a requirements folder, or a backlog/plan doc — into a **full Queue with a per-run processing cap** inside **one markdown run file** `.supervisor/automate/<run_id>.md` (the contract, dashboard, and resume state — **no manifest / registry / progress.jsonl ledger**; "find prior runs" = glob `.supervisor/automate/*.md` not `## Status: done`), then drives each Queue item through the per-item loop: `/autonomous --single-iteration --requirement <path>` → **one owned inline `/review-pr --until-mergeable` drain** → trusted-merge-or-park → sync `main` → check the item off + append the **append-only `## Progress`** (atomic temp+rename writes). **Layering:** `/autonomous` (one requirement) ⊂ per-item loop ⊂ `/automate` (source → Queue → loop); **`/backlog` is NOT a separate command** — a folder/backlog-doc is just one kind of source. **Smart resume** globs the run files for incomplete runs and **reconciles each in-flight item against ground truth** (`gh`/`git`/`## Status: done`) before trusting a checkbox; ambiguous resume fails **closed** under `--non-interactive-fallback` (`resume_ambiguous_non_interactive`). **Single-drain ownership:** the loop sets `.supervisor/config.json {"auto_review": false}` around the inner `/autonomous` RUN phase (byte-for-byte backup to a transient `<run_id>.config-backup.json` + absent-delete + malformed-abort + RECONCILE crash-restore) and owns exactly ONE inline drain — no double-dispatch. **Two modes:** safe (default — a `READY` drain parks `awaiting_merge` for a human; nothing merges automatically) and opt-in `--auto-merge` (default OFF) behind a **5-condition fail-closed trusted-merge gate** (drain `READY` + head-SHA-unchanged & base `main` + `reviewDecision` ∉ {`CHANGES_REQUESTED`,`REVIEW_REQUIRED`} with GraphQL-only unresolved-human-thread check + enforceable branch protection or `--trust-unprotected` + required-checks-green-AND-rubric-satisfied from `SUPERVISOR_RESULT.rubric_score`) — the **only place in the plugin that executes `gh pr merge --squash`**; `ESCALATED` never merges and **parks** the run (single-open-PR invariant holds in BOTH modes). `--limit N` (default 5) caps **PROCESSED** items, not Queue size — the full resolved Queue is materialized and **human-confirmed before processing**. `AUTOMATE_RUN` documented at `schema_version: 1` as a **markdown state-file contract, NOT a hook-validated emitted result block** (nothing validates it at a SubagentStop). Helpers + **35 stubbed-`gh` self-tests** (`scripts/automate-helpers.sh` + `test-automate-helpers.sh`: config suppress/restore, atomic run-file write + append-only Progress, resume reconcile, auto-merge fail-closed on each blocker). **New: 1 command (`/automate`), 1 skill (`automate-loop`); no new agent / hook / runner (`/automate` is inline-only) — counts move to 14 agents / 19 commands / 56 skills / 20 hooks; no `schema_version` bump.** Additive on top of v14.40.0.

**v14.40.0 — graduate the Local Twin self-heal catch-rate measurement from a scratch spike into a shipped, re-runnable instrument + wire it into `/insights` (additive):** Step 2b of the Local Twin path (`docs/SPIKES/LOCAL_TWIN_PATH.md` §1 Step 2) turns the throwaway Step-1 measurement into a permanent dial — "what is the self-heal catch-rate?" is now re-runnable anytime, across repos. **(1) The tool (`scripts/measure-heal-signal.{sh,py}`, uncounted + `test-measure-heal-signal.sh`):** a READ-ONLY instrument that harvests the heal signal from done-brief `## Outcome` blocks, joins it to the `/pr-postmortem` churn labels, and emits the same confusion matrix (recall / FP / FN) + JSONL shape as the Step-1 spike — PRESERVING its logic, not reinventing it. The repo list is parameterized (default: the current repo — machine-agnostic and reproduces the Step-1 baseline since the join only ever resolves within the self repo; overridable via `--repo` / `$AI_AGENT_MANAGER_HEAL_SIGNAL_REPOS` / `.supervisor/config.json .measure_heal_signal.repos`). It writes only under its `--out` dir (default `.supervisor/heal-signal/`, gitignored) and never mutates the repos it measures. **(2) Less of a floor:** `review_rounds` / `self_heal_misses` are documented FLOORS (CI-check-channel churn invisible to the counter; repo-shape regex gaps). The tool's label dedup now takes the per-field MAX across re-gathers (so a corrected-upward re-gather like #47's 0→6 wins) instead of latest-`ts`-only, and surfaces label-quality diagnostics (model-guessed / zero-churn-signal / floor-raised) so the floor is VISIBLE rather than silently trusted. Fully closing the gather's regex gaps stays the gather script's domain. **(3) `/insights` MEASURE leg:** `build-insights.sh` gains a `## Heal-signal catch-rate (MEASURE)` section sourced from `.supervisor/heal-signal/results.jsonl` (one append-only trend line per run) — latest catch-rate, FN/N, coverage, self-heal-lane churn share, and a recall trend oldest→newest. Advisory / DIRECTIONAL (churn labels), never gating; SUPPRESSED entirely when no scored matrix exists (the System-Twin-hard-signal / knowledge-sources precedent — no fabricated zeros). **(4) Bounded backfill as a DECISION, not an action:** coverage is the known gap (the matrix sees only PRs carrying BOTH a heal signal AND a model-classified label — ~10–24% today). `--backfill N` PLANS the work — lists up to N unlabeled heal-signal PRs + the estimated cost and STOPS; it NEVER dispatches `/pr-postmortem` (the label needs full model classification, not just gather), so a ~3-hour bulk backfill can never run by accident. **(5) Self-tests:** `test-measure-heal-signal.sh` (13 cases — matrix correctness, floor-raising flips TN→FN, READ-ONLY invariant, backfill-is-a-plan, trend ledger, wrapper repo-resolution incl. an env-split last-line bug it caught) + `test-insights.sh` (grows to 68 — heal-signal section render, suppression, and a scored recall-null run rendered as `recall n/a`). Re-running the tool on `ai-agent-manager` reproduces the trusted Step-1 baseline at its N=8 coverage (recall 0% / FN 6/8 / self_heal-stage share 54%); the **durable signal is recall 0%**, which held when a subsequent human-approved 10-PR `--backfill` round widened self-repo coverage 24%→55% (N 8→18, FN 15/18, self_heal share 53%) — confirming the rubber-stamp is not a small-N artifact. All fail-safe invariants preserved: READ-ONLY toward measured repos, advisory only, never blocks a run or changes a `heal_decision`. **No new agent / command / skill / hook (the tool + tests are uncounted scripts; counts unchanged at 14 agents / 18 commands / 55 skills / 20 hooks); no `schema_version` bump.** Additive on top of v14.39.0.

> 📜 **Full release history** (v14.39.0 → v14.0.0 and earlier) lives in [`CHANGELOG.md`](CHANGELOG.md). CLAUDE.md keeps only the two most recent release notes.
...
- Plugin manifest: `ai-agent-manager-plugin/.claude-plugin/plugin.json` (v14.41.0)
...
**`gh pr merge --squash` has exactly ONE sanctioned executor (invariant — do not break):** the **only** place in the plugin that EXECUTES `gh pr merge --squash` is the `automate-loop` `--auto-merge` gate (`skills/automate-loop/SKILL.md` §10, implemented by `scripts/automate-helpers.sh`'s `gate-eval`). It is **opt-in, default-OFF**, and fires only when ALL FIVE trusted-merge conditions hold; any failed / null / unreadable condition fails **CLOSED** (park + notify). `review-heal` (`/review-pr`) and Supervisor Phase 4.5 **NEVER merge** — they review-and-heal only and leave the PR open for a human (`READY`/`PASS`/`ESCALATED` are all terminal-stop, merge-identical). The positive-form invariant check is `grep -rn "gh pr merge --squash" ai-agent-manager-plugin/ | grep -viE "no |never |not "` — it must resolve only under `skills/automate-loop/`, `scripts/automate-helpers.sh` (+ its test), and the `/automate` command/help docs that describe the sanctioned gate; review-heal / review-pr / RESULT_SCHEMAS keep only their negative-assertion ("never merges") mentions. Adding a second executor anywhere else is a regression.
