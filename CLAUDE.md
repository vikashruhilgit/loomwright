...
**v14.37.0 — fix the post-PR review-drain dispatch on the inline `/supervisor` & `/autonomous` paths (additive):** Closes two diagnosed root causes that left the until-mergeable review drain silently un-dispatched on the inline main-thread paths. **(1) Inline canonical `state.md` write:** Phase 1 ACQUIRE and the Phase 4.5 completion tail now write/flip the **canonical lowercase `## Session` block** (`- status:`, `- branch:`) to `.supervisor/state.md` on the **inline** path (not only via a Context-Keeper spawn) — best-effort/non-fatal — so the durable state the hook's session-scope gate and `/supervisor --continue` resume consume actually exists and is truthful. **(2) Hook format-tolerance:** `hook-dispatch-on-pr-create.sh` now parses **both** the canonical lowercase (`- status:`/`- branch:`) **and** the inline-Supervisor **bold** (`- **Status:**`/`- **Branch:**`) `state.md` forms for term (ii) status + term (iii) branch; the anti-hijack branch-MATCH requirement and fail-closed-on-unresolvable posture are unchanged, the hook still ALWAYS exits 0, and its self-test gains bold-format MATCH / branch-MISMATCH / stale-status cases (16 green). **(3) `/autonomous` single-iteration drain parity:** the autonomous EXECUTE enumeration no longer drops Supervisor **Phase 4.5 step 5.5** — single-iteration `/autonomous` now reaches the post-PR until-mergeable drain through the inline Supervisor workflow (parity with direct `/supervisor`); EVALUATE's chained review-heal is documented as an **additional, multi-iteration-only** stacking gate and the R9 downstream-ordering note is unchanged (doc/prompt-level only). All fail-safe invariants preserved (the hook + state write never gate, never change `heal_decision`, never block the PR/run). **No new agent/command/skill/hook; counts unchanged at 14 agents / 18 commands / 55 skills / 20 hooks; no `schema_version` bump.** Additive on top of v14.36.0.

**v14.36.0 — Learning Loop Phase 4 — churn ledger: postmortem provenance + read-back into Launch Pad & Supervisor Phase 4.5 (additive):** Closes the read-back half of the PR-churn learning loop. `POSTMORTEM_RESULT` is enriched with additive provenance fields (`pr_url`, `brief_path`, `job_path`, `branch`, `changed_paths`) so a churn finding can be correlated back to the brief/job/branch and the files it touched — **additive, `schema_version` stays 1**. A new **fail-safe** `read-postmortem.sh` read-back helper surfaces prior churn ledger findings, wired **advisory-only** into **Launch Pad Phase 3 / Risk Assessment** (prior-churn hints inform file-impact + risk analysis) and **Supervisor Phase 4.5 pre-review enrichment** (prior-churn context primes the self-heal review). The read-back is **strictly advisory / non-gating**: it never blocks a PR, never changes a `heal_decision`, and any read miss / missing source / parse failure degrades to a logged no-op (per the CLAUDE.md bimodal "side-effect emitters fail SAFE" invariant). **No new agent/command/skill/hook; counts unchanged at 14 agents / 18 commands / 55 skills / 20 hooks; no `schema_version` bump** (the new `.sh` + test/fixture are uncounted scripts). Additive on top of v14.35.0.

> 📜 **Full release history** (v14.32.0 → v14.0.0 and earlier) lives in [`CHANGELOG.md`](CHANGELOG.md). CLAUDE.md keeps only the two most recent release notes.
...
- Plugin manifest: `ai-agent-manager-plugin/.claude-plugin/plugin.json` (v14.37.0)
...
| PostToolUse (Bash) — v14.34.0 | A Bash tool call completes (e.g. `gh pr create`) | hooks.json | type:command — `hook-dispatch-on-pr-create.sh`; backstops the until-mergeable review drain on PR creation. Session-scope gated (in-progress job + state.md Status≠completed/failed + head-branch match); fail-safe, always exits 0 |
