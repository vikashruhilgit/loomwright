...
**Latest change:** **vendor-neutrality becomes a ratchet — and the ratchet kept failing its own standard.** Portability was asserted in a memory and a requirement doc and no check enforced it, so the core kept absorbing harness references (prompt-layer refs +31% since the v15.13.0 measurement; `loomwright/scripts` portability 82% → 70%). The new fail-CLOSED gate `scripts/check-vendor-coupling.sh` reads a committed `loomwright/docs/vendor-coupling-manifest.json` — every vendor token, class glob and per-path allowance lives there and the gate hard-codes none of them, so each policy change is a reviewable diff. Paths are classified CORE / ADAPTER / COUPLED; breach is `actual > declared`, while `actual < declared` **passes**, because debt paid down must never be a CI failure. `loomwright/scripts/resolve-loomwright-root.sh` introduces the vendor-neutral `LOOMWRIGHT_ROOT` seam and is ADAPTER-exempt deliberately: its whole job is to name the harness once so nothing else has to. Migrating the existing call sites is explicitly NOT in scope — this stops the growth. **A stated assumption was refuted rather than planned around:** the requirement assumed the references were dominated by legitimately-adapter surfaces and flagged that unverified; measured, adapter-ish is 47% and core-ish 50%. **The instructive part is the review, not the feature** — six rounds found six defects and five were the gate failing its own standard: an adapter glob whose justification did not match what it swept in, a hand-typed count inside the count-drift gate, the allowance-regeneration flag proved only by reading, validation that ran only on files which already had references, and a config field nothing read. Each was reproduced before being fixed and is now backed by an executed assertion, every new case mutation-verified — including the controls that stop a case being satisfiable by a gate which simply rejects everything. **Honest limits are stated in the gate's own header, not just the PR:** it counts LITERAL occurrences, so a runtime-assembled reference is invisible; root prose, the sibling plugins and `.github/` are unscanned rather than certified clean; and the resolver's exhausted-resolution branch is untested by design, with the note carrying its own deletion condition so it cannot become permanent cover for untested code. Counts unchanged. Full narrative in `CHANGELOG.md`.
