...
**v14.25.0 — Auto-author Outcomes Rubric in `/autonomous` multi-iteration mode:** The `/autonomous` loop gains a rubric **producer** (it was rubric-consumer-only before). In multi-iteration mode, when the requirement lacks `## Outcomes Rubric`, the inlined Launch Pad auto-authors one at a guarded Phase 5 step (3–7 diff-checkable bullets distilled from Acceptance Criteria + Phase 3 analysis); the human approves/edits it at Launch Pad Phase 6, and the loop persists the approved rubric back into the requirement body and freezes it for later iterations. Degenerate fallback: fewer than 3 diff-checkable bullets → no rubric authored, and the existing no-rubric gate applies unchanged. Documentation/prompt-only: `skills/supervisor-readiness/SKILL.md` (new §"Auto-Authoring (multi-iteration)", 1.1.0→1.1.1), `agents/launch-pad.md` (guarded Phase 5 step), `skills/autonomous-loop/SKILL.md` (PLAN steps, 1.3.0→1.3.1), `agents/plan-reviewer.md` (Criterion 3 extended — still 14 criteria), `commands/autonomous.md` + `docs/RESULT_SCHEMAS.md`. **No new agent/command/skill/hook; no schema_version bumps; counts unchanged at 14 agents / 18 commands / 54 skills / 19 hooks.** Additive on top of v14.24.0.
...
> 📜 **Full release history** (v14.23.3 → v14.0.0 and earlier) lives in [`CHANGELOG.md`](CHANGELOG.md). CLAUDE.md keeps only the two most recent release notes.
...
- Plugin manifest: `ai-agent-manager-plugin/.claude-plugin/plugin.json` (v14.25.0)
