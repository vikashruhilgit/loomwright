...
**Version + counts:** authoritative in `plugin.json` (see `Plugin Layout` below for the per-directory breakdown) — not restated here. **Latest change:** the findings ledger (`.supervisor/postmortem/results.jsonl`) becomes a **committed, GATED third store** under `/setup memory`, so the churn corpus finally travels with the repo instead of every fresh clone/worktree/CI run starting blind. It could not simply be un-ignored: the ledger is **structurally cross-repo** — a `/pr-postmortem` append lands in the CURRENT working `.supervisor/`, never in the analysed repo's — so this plugin's own held **7 `vendsy/hub` records among 88**, and a naive addition would have generalised that contamination into every user's repo. `setup-memory.sh` gains `ledger_gate_blocks_foreign_records()`: `apply` emits the ledger negation **only** while every record's `.repo` is inside the resolved allowlist, and otherwise **refuses to write it**, names the offending slugs and **still exits 0** — fail-closed in the WRITE dimension, never in the exit status (the helper's `FAIL-SAFE CONTRACT` makes status lines, not exit codes, carry the outcome). **The conditional negation forced a third verdict class, and the two are one fix:** `render_report` marks any INTENDED path probing `ignored` as `not configured`, whose remediation copy says to comment out "the surviving exclude" — which for a withheld ledger is the module's OWN `.supervisor/*` line, and `commands/setup.md` mandates relaying that copy verbatim, so an unconditional add would have made a **correct refusal permanently self-report as failure with destructive guidance**. Hence `REPORT_GATED` + a `gated*)` arm inserted **ahead of** the under-inclusion fall-through, and `LEDGER_INTENDED_PATH` held separately from `INTENDED_PATHS` so the ledger is probed only when the gate passes. Two further consequences are closed rather than hidden: the byte-compare no-op branch no longer prints the exact `apply: no-op — already configured` headline the class exists to prevent, and the clean→contaminated transition (which legitimately rewrites `.gitignore` to WITHDRAW the negation) is **announced** with the slugs plus the honest limit that withdrawal does **not** un-track an already-committed ledger. Degenerate inputs are decided, not guessed: **absent ledger ⇒ PASS**; **absent OR BROKEN `jq` ⇒ gate UNEVALUATED** (negation not emitted on a fresh repo, exit 0), failing neither toward `gated` nor toward publishing something unchecked; and **an UNREADABLE ledger REFUSES** — "could not examine" is never "examined and clean". **EMITTING and WITHDRAWING are ASYMMETRIC and must not share one gate outcome:** emitting a NEW negation requires an affirmative examined `pass`, but WITHDRAWING one already in the block requires an affirmative `refuse` backed by REAL, EXAMINED foreign slugs — every could-not-examine outcome (`jq` absent **or non-functional** — `command -v` tests presence, not function — ledger unreadable, every record unparseable) **PRESERVES** what is already there, because a withdrawal is an unrequested state change justified by a diagnosis never made, and it would print a contamination message naming slugs that do not exist. Observed slugs and could-not-examine reasons therefore travel in separate channels with **opposite** remedies (`filter-ledger` is right for a real foreign record and truncates an unreadable ledger to zero bytes). **The gate is jq-only and that is load-bearing:** the ledger mixes compact and spaced JSON, so `grep -c '"repo":"…/loomwright"'` returned **38** where jq returned **39**, and a foreign record in spaced form would be **invisible** to a compact grep — a guard that cannot fire. Every ledger assertion in `test-setup-memory.sh` is now jq (including the pre-existing group (d) greps), the spaced form is a named case (`test_gate_blocks_foreign_spaced_form`) that proves the evasion *inside* the fixture, and both wrong emission shapes are negative controls (`test_negation_wrong_position_still_ignored`). This repo's ledger was filtered (re-derived with jq: 42 + 39 = **81** retained, **7** `vendsy/hub` removed) and committed. **A latent defect surfaced and is fixed:** `test-committed-twin-scrub.sh`'s `ignore_is_in()` probed with `git check-ignore --verbose -n`, whose exit status is 0 whenever a pattern MATCHED — **including a negated `!` pattern**, i.e. exactly when the path is committable; invisible while the stores were re-included by DIRECTORY patterns, it fired the moment one was re-included by a FILE pattern. The verdict now comes from `-q`. Suites **198 → 295** and **32 → 37**; mutation control against the pre-fix form: **237 passed / 22 failed**, with each fix reverted in turn failing only its own assertions (243/16, 244/15, 251/8, 257/2, 248/11, 258/1, 257/2, and for the emit/withdraw asymmetry 278/8 — 281/5 and 285/1 for its two halves). **One remaining unpinned branch is closed by test group (o):** `do_remove()` gained a THIRD `tracked_count` and a three-path `git rm -r --cached` hint when the ledger became a managed store, but every ledger group drives `apply`/`check` only, so the remove half was untested. Group (o) applies on a clean-ledger fixture, really `git add`s the ledger (a count of 0 would pass whether or not the code looks at it), then asserts remove's own warning reports the ledger count, that the ledger is IGNORED again by a real `git check-ignore` probe, and that the remediation names all three stores. Two idioms are load-bearing: the text assertions are scoped to the half of the output BEFORE `== verify ==`, because `do_remove` tails into `render_report`, whose own "tracked today" line names all three stores and kept an unscoped assertion green under mutation; and the ignore probe uses `--no-index`, since plain `check-ignore` reports a TRACKED path as not-ignored regardless of the rules — the very already-tracked-stays-tracked fact this output warns about. Suites **286 → 295**; mutation control, one break per assertion: dropping the ledger from remove's tracked-count line ⇒ 294/1 ((o1) only), from the `git rm` hint ⇒ 294/1 ((o3) only), and leaving an effective ledger negation behind after remove ⇒ 290/5 ((o2) plus the four pre-existing byte-exact round-trip assertions, as expected). Counts unchanged. See `CHANGELOG.md` v15.28.0.
