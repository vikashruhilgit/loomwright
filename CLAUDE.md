...
**Version + counts:** authoritative in `plugin.json` (see `Plugin Layout` below for the per-directory breakdown) — not restated here. **Latest change:** the curation cadence's three numbers now speak **one unit**. v15.29.0 shipped a readiness count (*files newer than a wall-clock stamp*), a consumption window (*the N most-recent files*, `--sessions` default 5) and a record (*wall-clock now*) in three files with nothing asserting a relationship between them — so `pending=61 ready=yes` described a run that reads 5 and permanently retired the other 56. Three consequences, all in `scripts/curation-status.sh`: **(1)** `record dreaming` now takes the session ids actually consumed and `pending` is the set complement — a **watermark provably cannot** express "the newest 5 are done, the older 57 are not", since /dreaming leaves behind the *older* tail and no value of T counts an older tail; **(2)** the count names the `window=` that will drain it, pinned to `commands/dreaming.md`'s Parameters table by a cross-file test; **(3)** `pending` counts logs carrying reflection **signal** (measured: 29 of 62 logs were pure `token_ledger`/`subtask_complete`, ~96% of corpus bytes, 0% of signal), with **different predicates for /dreaming and /insights** because the two consumers genuinely differ, both failing OPEN. `/insights` keeps its watermark — exact for it, since it re-reads the whole corpus every run. Counts unchanged at 14/21/41/24. Full narrative — including the zsh `producer | grep -q` census inversion and the mutant that exposed dead code rather than a missing test — in `CHANGELOG.md` v15.30.0. The new uncounted `scripts/curation-status.sh` (`status`/`record dreaming`/`nudge`, every exit `exit 0`) stores `/dreaming`'s last run in `.supervisor/curation-state.json` and DERIVES the other two (`/insights` ⇐ dashboard mtime; `/pr-postmortem` ⇐ ledger max `.ts` with **`select(.source != "automate_drain")`** — the v15.28.0 corpus has two producers and 26 of 81 records are engine appends no human ran). Three findings shape it: the seam is **`SessionStart`, not `Stop`** (that hook is `type: "prompt"`, answering the result validator, never shown as text); the nudge needs a **dedicated `startup)` branch** because the shared gate bails on `source=startup` — and widening that gate was tried and REJECTED, since it would put `observability_probe`'s `curl` and desktop notification on every fresh session start; and because that branch exits from inside the `case`, it must do its own `.supervisor/` check, its own `jq -Rs` envelope, and be defined above the case. `/dreaming` and `/insights` gain `--force` and an advisory decline; **`/pr-postmortem` never declines** (both its forms target one named PR). `rules_nudge`'s firing surface is unchanged and pinned. Counts unchanged at 14/21/41/24. Full narrative — including the `stat -c %Y`-first trap, the `unknown`-never-`0` rule, and the root-skip on the unreadable-ledger fixture — in `CHANGELOG.md` v15.29.0.
