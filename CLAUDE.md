...
**Latest change:** **a writer may not vouch for itself, and a committed store may not be written without a human.** The previous release wired five shared checks into six sole writers; two of them were not load-bearing, and both are now closed. **`add-orientation.sh` was validating the memo it stamps, not the memo it was given** — the entry handed to the validator was the COMPOSED file, whose header the writer writes itself carrying `head_sha: <short sha>`, and a labelled hex token IS a citation to `validate_provenance`, so **every memo vouched for itself**; measured, a memo citing nothing at all was written with rc 0. The validated entry is now **body + summary only**, a new `--source` supplies provenance properly, and `/dreaming` passes `dreaming:<session_id>`. The fix is **writer-side only** — `validate-entry.sh` is untouched, because it is shared by six writers and must not learn one writer's file format. This **knowingly reverses the reasoning of the commit three before it**, which made a labelled `head_sha:` count as a citation after an earlier fix refused real memos on all-digit shas (2.6% of commits); that commit's own objection to `--source "head_sha:$sha"` was that the writer would vouch for itself outside the artifact a reader can see, and keeping the header inside the validated entry is the same self-vouching one layer in. The labelled-sha acceptance is unchanged and still serves every other writer, and a prose-cited memo still writes with no `--source` — pinned in **both** directions, since a refusal that fires for everything is not a working check. Separately, **two writers could append to a committed store with no human in the loop**: `.supervisor/memory/` looks gitignored and is not (`.gitignore` carries a `!.supervisor/memory/` negation, so `LESSONS.md` and `PROJECT_MEMORY.md` are tracked), and one review invocation proved it by accident. Both are now gated. The rule is **stated, not inferred**: a sole writer whose store is COMMITTED requires `--confirm`; one whose store is gitignored does not — decided by `git ls-files <store>`. `write-system-contract.sh` correctly keeps no gate and says so in its own header. The lesson, in `AGENT_GUIDELINES.md`: **validation is not a substitute for a confirm gate.** Three things the gate work found that no plan contained: `write-lessons.sh` has a **third** mutating action (`--attest-existing`, appending to the committed chain); **a correctly-placed gate was still not sufficient**, because both writers bootstrap their store *before* the validator, so a dry-run still created two committed files until the bootstrap became conditional; and **`supersede` cannot be gated per-half** — its retract half commits before its add half, so a naive per-site gate would let a "no" exit 0 having already tombstoned the target. Finally, **half the source requirement was already fixed before it ran**: its duplicate scope, its AC2 and its "not deferrable past 05" argument all rest on a ~90-body-token threshold that **never existed** — a prior commit had already replaced the comparison store with a whole-store corpus, and a **12-token** duplicate is refused on the live writer. That is recorded in the requirement file, and **item 05 was never blocked by this**. Counts unchanged. Full narrative in `CHANGELOG.md`.
