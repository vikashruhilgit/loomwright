...
14 hooks centralized in `hooks.json` since v10.0.0 (the v12.2.0 webhook hook brought the count from 13 → 14; v13.0.0 and v14.0.0 add no new hooks — v14's `--notify` gate-event webhooks are emitted inline by the autonomous-loop, not by a SubagentStop hook). Prompt-based validation uses fast haiku model with 30s timeout. WorktreeCreate / StopFailure / telemetry / webhook hooks use `type: command` for zero-latency.
