...
**v15.2.1 — "Which command?" decision table + flag-table completeness audit (docs-only, additive):** README.md, `.claude-plugin/README.md`, and `commands/agent-help.md` now carry a consistent "Which command?" decision table (5 intent rows with per-row "does NOT do" merge notes — none merge except `/automate --auto-merge`, opt-in). The Parameters tables in `commands/supervisor.md` / `autonomous.md` / `automate.md` were completeness-audited: every real flag has a row with default + "only meaningful when" precondition; `autonomous.md` gains the exact 3-flag forwarded-set enumeration (mirroring `skills/autonomous-loop/SKILL.md` §"Auto-forwarded flags"), an `--until-mergeable` ownership note landed in `supervisor.md`, and an iteration-vs-item terminology note connects `/autonomous` and `/automate`. No behavior change; merge-executor invariant grep still resolves to exactly 4 surfaces. **Counts UNCHANGED: 14 agents / 21 commands / 57 skills / 21 hooks; no `schema_version` bump.** Additive on top of v15.2.0.
...
> 📜 **Full release history** (v15.1.0 → v14.0.0 and earlier) lives in [`CHANGELOG.md`](CHANGELOG.md). CLAUDE.md keeps only the two most recent release notes.
...
- Plugin manifest: `loomwright/.claude-plugin/plugin.json` (v15.2.1)
