...
**v14.32.0 — `/review-pr --until-mergeable` all-channel drain + Supervisor auto-run-by-default (additive):** Extends the v14.30.0 `--until-mergeable` heal loop from bot-only signals to **all external review channels** and wires it on by default in the Supervisor completion tail. **(1) All-channel read:** the `review-heal` skill's until-mergeable drain now reads **every** external PR review channel each round — required CI check-runs, automated/bot reviews, **and human-authored review threads** — via a new shared `scripts/classify-bot-review.sh` classifier (single source of truth for bot-vs-human author classification, replacing the inline `gh api graphql` typename/login logic; uncounted script + self-test `test-classify-bot-review.sh`). **(2) Scoped wait:** the loop now distinguishes *actionable-now* signals (drainable: failing required checks, bot threads, machine-fixable findings) from *await-scoped* signals (human approval / human threads) and surfaces — never silently swallows — the latter, while still NEVER merging and NEVER blocking on a human. **(3) Validate-then-fix:** each round validates the freshly-read external state **before** dispatching the `Task(general-purpose)` fixer (no fix on stale signal), and the **READY** definition is refined to "required checks green AND no unresolved actionable (bot/machine) threads remain — human-scoped items surfaced, not gated." **(4) Supervisor auto-run-by-default:** the Supervisor completion tail now auto-runs the until-mergeable drain by default via a new fail-safe dispatcher `scripts/dispatch-pr-review.sh` (uncounted; mirrors `dispatch-pr-postmortem.sh`, always exits 0, never alters `SUPERVISOR_RESULT`), opt-out via `--no-until-mergeable` / `auto_until_mergeable: false` / `AI_AGENT_MANAGER_UNTIL_MERGEABLE`. **(5) Schema:** additive `REVIEW_HEAL_RESULT` (stays `schema_version: 2`) + `SUPERVISOR_RESULT` (stays `schema_version: 1`) fields only. **No new agent / command / skill / hook (counts unchanged at 14 agents / 18 commands / 55 skills / 19 hooks; `classify-bot-review.sh` + `dispatch-pr-review.sh` are uncounted scripts); no `schema_version` bump.** Additive on top of v14.31.0.
