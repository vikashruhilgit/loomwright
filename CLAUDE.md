...
**v14.30.0 — `/review-pr --until-mergeable` external-review-aware heal loop + churn-gated auto-postmortem (additive):** Extends `/review-pr` + the `review-heal` skill with an **opt-in, default-OFF** `--until-mergeable` mode that drains *machine-speed* external PR signals — **required** CI check-runs (discovered from branch-protection metadata; only required checks gate readiness, fail-CLOSED to `ESCALATED` when that metadata is unreadable unless `--required-checks all-non-neutral` is set), automated/**bot** reviews, and unresolved **bot-authored** review threads (classified via `gh api graphql` `reviewThreads`, since `gh pr view --json` does not expose them) — auto-fixes the BLOCKING/HIGH items via a `Task(general-purpose)` worker, pushes (**never `--force`**), and loops until **required checks are green AND no unresolved bot-authored threads remain**, then stops with `decision: READY` and fires a best-effort "ready to merge" desktop/webhook notification. **It NEVER merges and NEVER waits on human approval / `REVIEW_REQUIRED` / human-authored threads** (those are surfaced, not gated). Bounded by `--max-rounds` (default 5) with an anti-churn fingerprint guardrail that escalates to one deep "fix-the-class" self-review on oscillation. On loop exit it conditionally fires the existing read-only `/pr-postmortem <pr-url>` via a new fail-safe dispatcher (`scripts/dispatch-pr-postmortem.sh`, uncounted) — **churn-gated** (`postmortem_churn_threshold`, default 2; no-op on clean PRs), opt-out via `--no-auto-postmortem` / `auto_postmortem: false`, decision computed **before** dispatch so the dispatcher can NEVER alter `REVIEW_HEAL_RESULT.decision` (always exits 0, append-only to `.supervisor/postmortem/results.jsonl`). `REVIEW_HEAL_RESULT` goes to **`schema_version: 2`** (adds `READY` + new fields; v1 still accepted) and the `/autonomous` consumer docs treat unrecognized/`READY` as terminal non-re-iterate. Default `/review-pr` (no flag) is byte-for-byte unchanged. **No new agent / command / skill / hook (counts unchanged at 14 agents / 18 commands / 55 skills / 19 hooks); no `SUPERVISOR_RESULT` schema_version bump.** Additive on top of v14.29.0.

> 📜 **Full release history** (v14.29.0 → v14.0.0 and earlier) lives in [`CHANGELOG.md`](CHANGELOG.md). CLAUDE.md keeps only the two most recent release notes.
...
- Plugin manifest: `ai-agent-manager-plugin/.claude-plugin/plugin.json` (v14.31.0)
