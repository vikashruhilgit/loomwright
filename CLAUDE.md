...
**v14.32.0 — `/review-pr --until-mergeable` all-channel drain + Supervisor auto-run-by-default (additive):** Extends the v14.30.0 `--until-mergeable` heal loop to drain **all machine-speed external review channels** and wires it on by default in the Supervisor completion tail. **(1) All-channel read:** the `review-heal` skill's until-mergeable drain now computes readiness over the **UNION** of every external PR review channel each round — formal reviews (`reviews`/`latestReviews`), inline review threads, **PR issue comments** (the #64 channel gap — `gh api .../issues/{n}/comments`, which `gh pr view --json reviews` never returns), and **bot check-run output/annotations** — classifying each channel on its **body/content text** (not metadata) via a new shared `scripts/classify-bot-review.sh` (single source of truth for **bot-authored review-finding** classification on `(login, body)`, extracted from `pr-postmortem-gather.sh`; uncounted script + self-test `test-classify-bot-review.sh`). The §U3 GraphQL thread bot-vs-human (`__typename`/`login`) classification is unchanged. Human approval, `REVIEW_REQUIRED`, and human-authored threads/comments are **surfaced, never drained or gated**. **(2) Scoped wait:** a bounded wait scoped to **required + review-producing checks only** (configurable via `--review-check-pattern`, default `*review*`/`claude*`; unrelated optional checks are NEVER waited on or escalated by themselves), which re-scans **all** channels after the scoped set settles (so a late-posting review comment is seen), and fails CLOSED to `ESCALATED` if a required/review-producing check is still in flight at the bound. **(3) Validate-then-fix:** **replaces** the blind BLOCKING/HIGH severity floor — every detected bot finding (any stated severity) is evidence-validated against the current branch, fixed at **any** severity if confirmed (the #64 finding was MEDIUM), surfaced/escalated if confirmed-but-not-auto-fixable, and dismissed if stale/invalid; the refined **READY** ⇔ required checks green AND scoped review-producing checks settled AND no unresolved validated bot findings across ALL channels (human approval/threads surfaced, never gated). **(4) Supervisor auto-run-by-default:** the Supervisor completion tail now auto-runs the until-mergeable drain by default via the **existing** fail-safe dispatcher `scripts/dispatch-pr-review.sh` (uncounted; pre-v14.30.0 — **repurposed** here from the opt-in plain-`/review-pr` dispatch to a default-ON until-mergeable drain; still always exits 0, never alters `SUPERVISOR_RESULT`), opt-out via `--no-until-mergeable` / `auto_until_mergeable: false`, signal threaded by the `AI_AGENT_MANAGER_UNTIL_MERGEABLE` env var. **(5) Schema:** additive `REVIEW_HEAL_RESULT` (stays `schema_version: 2`) + `SUPERVISOR_RESULT` (stays `schema_version: 1`) fields only. **It NEVER merges and NEVER waits on a human.** **No new agent / command / skill / hook (counts unchanged at 14 agents / 18 commands / 55 skills / 19 hooks; `classify-bot-review.sh` + `dispatch-pr-review.sh` are uncounted scripts); no `schema_version` bump.** Additive on top of v14.31.0.

**v14.31.0 — Generic Launch Pad + Plan Reviewer prevention guardrails (additive):** Three additive, repo-agnostic, **trigger-gated** prevention guardrails that catch a class of planning gaps before they surface as review churn — no behavior change on ordinary briefs. **(1)** Launch Pad Phase 3 ANALYZE gains a generic **validator-owned-surfaces** rule: when a task's acceptance depends on a generated/mechanical validation script (doc-currency / lint / codegen / consistency checker — stated generically, **never a hard-coded filename**), Launch Pad folds the validator-scanned surfaces into the File Impact Map (or notes that they may need updates). **(2)** Launch Pad Phase 1 VALIDATE gains a **dirty-source-doc warning**: when the specific source requirement/doc a brief is planned from is uncommitted, it emits a WARNING (provenance/staleness unverified) — scoped to that one file, never the whole tree. **(3)** Plan Reviewer gains conditional **Criterion 15 (Canonical-List / Source-of-Truth Verification)**: when a brief asserts a canonical-source claim ("single source of truth" / "canonical list" / "exact names" / "authoritative") **and** cites a source, it verifies the brief's restated values against that source and flags mismatches under a new `canonical_source` PLAN_REVIEW_RESULT issue category; it skips silently otherwise (the Criteria 11/13/14 conditional precedent). Note the **gating asymmetry**: guardrails (1)+(2) are advisory-only (warn / annotate, never block), whereas Criterion 15 — like Plan Reviewer's other numbered criteria — is **gating**, so a verified gate-breaking mismatch on a *triggering* brief can reach BLOCKING→FAIL and block the save (the "no behavior change" above refers to *non-triggering* briefs). **No new agent / command / skill / hook (counts unchanged at 14 agents / 18 commands / 55 skills / 19 hooks); no schema_version bump** (`canonical_source` is an additive free-form category, the `executable_acceptance` precedent). Additive on top of v14.30.0.

> 📜 **Full release history** (v14.30.0 → v14.0.0 and earlier) lives in [`CHANGELOG.md`](CHANGELOG.md). CLAUDE.md keeps only the two most recent release notes.
...
- Plugin manifest: `ai-agent-manager-plugin/.claude-plugin/plugin.json` (v14.32.0)
