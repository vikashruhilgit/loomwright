...
**Version + counts:** authoritative in `plugin.json` (see `Plugin Layout` below for the per-directory breakdown) — not restated here. **Latest change:** an **8th `/setup` module, `memory`**, puts a repo's Twin memory stores (`.claude/agent-memory/`, `.supervisor/memory/`) under version control **IN PLACE** via gitignore negation — no move, no symlink, no generated copy (a `git mv` would silently sever every agent from its memory while leaving the test suite green: the harness path is fixed and a committed copy is never injected). New `loomwright/scripts/setup-memory.sh` (`check`/`apply`/`remove`/`allowlist`/`filter-ledger`, fail-safe, write-contained to `.gitignore` + `.supervisor/config.json`, and it NEVER runs `git add`/`rm`/`commit`) plus `test-setup-memory.sh`, which **asserts the naive `.claude/` + `!` negation FAILS** and the `/*` form works, checks every intended/unintended path individually via `git check-ignore`, and covers the dotfile provenance sidecars. The repo allowlist is a **LIST, never a string and never the live remote** (a rename leaves records under the old slug), defaulting to the current remote — never a hardcoded owner. Consent is load-bearing: `apply` discloses what becomes version-controlled BEFORE writing, and `remove` states plainly that **git history retains anything already pushed**. `.supervisor/postmortem/` is deliberately NOT negated, and the tracked-write risk (`git add -A` sweeping memory writes into unrelated commits) is stated and handed to the memory-writer item — no proposal queue here. Counts unchanged (a module, not a command). See `CHANGELOG.md` v15.25.0.
