...
**Version + counts:** authoritative in `plugin.json` (see `Plugin Layout` below for the per-directory breakdown) — not restated here. **Latest change:** the `--until-mergeable` drain's declared `--max-rounds` ceiling is now **mechanized** (`loomwright/scripts/drain-rounds.sh`, a shared per-PR ledger both `/review-pr` entry paths call — previously the same prose-only bound, never a real inline-vs-runner asymmetry) plus a new **termination-only severity floor** (`--severity-floor`, default `HIGH`): a round whose entire fixed yield is sub-floor can terminate `READY`/`sub_floor_converged` without a further all-channel re-scan, gated on a SHA-bound confirming required-check pass (never a false READY on red/unknown CI) and explicitly excluded from `--auto-merge` eligibility. See `loomwright/skills/review-heal/SKILL.md` §"Termination-only severity floor (`sub_floor_converged`)".
