...
**v14.22.0 — `/pr-postmortem`: on-demand PR review-churn root-cause analysis:** A new read-only, advisory-only diagnostic that turns post-PR review churn into a measurable trend. After an agent-generated PR has absorbed multiple rounds of review-and-fix, `/pr-postmortem <pr-url>` gathers the PR's metadata + review threads + feature-branch diff via the new `scripts/pr-postmortem-gather.sh`, buckets each review round into one of six reproducible root-cause classes, attributes it to the agent-flow stage that should have caught it (`launch_pad` / `worker` / `self_heal` / `unknowable`), prints a human-readable root-cause report, and appends **exactly one** jq-built, fail-safe `POSTMORTEM_RESULT` trend line to `.supervisor/postmortem/results.jsonl`. It **never writes code, never gates, never blocks the PR** — the append is best-effort (any failure prints one warning and still exits 0). The trend file is the **seed corpus for the deferred M2b part-2b synthetic eval harness**. Ships as the `pr-postmortem` skill (the analysis protocol + miss-class taxonomy, authority for the command), the `commands/pr-postmortem.md` entry point, the `pr-postmortem-gather.sh` read-only gather (+ its self-test), a `POSTMORTEM_RESULT` schema section in `docs/RESULT_SCHEMAS.md` (`schema_version: 1`, **no hook validator** — the command is the main agent of its own session), and a SKILLS_INDEX row. **Current counts are now 14 / 17 / 52 / 19 — +1 command +1 skill (the `pr-postmortem` analyzer); no new agent, no new hook.** Additive on top of v14.21.0.
...
- Plugin manifest: `ai-agent-manager-plugin/.claude-plugin/plugin.json` (v14.22.0)
- Agents: `ai-agent-manager-plugin/agents/` (14 markdown prompts)
- Commands: `ai-agent-manager-plugin/commands/` (17 entry points)
- Skills: `ai-agent-manager-plugin/skills/` (52 skills, see `SKILLS_INDEX.md`)
...
│   ├── skills/                                # 52 skills + SKILLS_INDEX.md
