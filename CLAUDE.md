...
**Version + counts:** authoritative in `plugin.json` (see `Plugin Layout` below for the per-directory breakdown) — not restated here. **Latest change:** all 14 `loomwright/agents/*.md` files now declare an identical, one-canonical-order superset `tools:` line (consistency-only — see `loomwright/docs/shared-agent-prefix.md` and `loomwright/docs/POINTER_AUDIT.md` §"HONEST CACHE EXPECTATION"; the 512-token cross-agent cache floor is **not** cleared and reuse remains structurally zero), with every agent's *observed effective* capability set preserved bit-for-bit via a per-agent `disallowedTools` denylist derived from disk (`superset − effective`, not `superset − declared`). Four agents (`launch-pad`, `product-owner`, `qa-strategist`, `red-team-reviewer`) deliberately keep `Write`/`Edit` unblocked because the harness's `memory: project` grant gives them that capability effectively today regardless of their prior declared `tools:`. This is an explicit, accepted enforcement-model downgrade — moving the allowlist half of restriction to uniform-by-construction and leaving the denylist to carry the whole restriction for every agent — recorded in `loomwright/docs/ARCHITECTURE_CONTRACTS.md` §"disallowedTools (Defense-in-Depth)", not shipped silently. See `CHANGELOG.md` v15.24.0 for the full per-agent derivation and the `code-reviewer` open question left unresolved by design.
