...
- Commands: `loomwright/commands/` (24 entry points)
...
**Plugin `description` is a summary, not a changelog (anti-rebloat):** the `description` field in `plugin.json` and `.claude-plugin/marketplace.json` is the crisp card shown in the plugin-manager UI. It deliberately carries **no `vX.Y.Z` string at all** (the `version` field already carries that) — on a count bump, update the four counts **in place** and keep it short; put the per-release narrative in `CHANGELOG.md` — **never append another version clause to the description.** Length is gated by `scripts/check-doc-currency.sh` (fails when either description exceeds 600 chars or carries more than one `vX.Y.Z`-shaped token — red-team-hardening item 08).
...
Full hook table (trigger, location, validation, one row per hook) relocated to `loomwright/docs/HOOKS.md` §"Hook Table" in the v15.21.0 diet — now the authoritative, always-current source. Only three hooks still use prompt-based validation (`SubagentStop[loomwright:code-reviewer]`, `Stop`, `TaskCompleted`); every other hook is `type: command` for zero-latency. **Two decision dialects, do not mix them:** a `type: prompt` hook's model answers `{"ok": true|false, "reason": …}`; a `type: command` validator must print the documented Stop/SubagentStop shape — `{}` to allow, `{"decision": "block", "reason": "…"}` to block (`result_block_parser.emit`). A command hook that prints the `ok` shape is a silent no-op — the runtime records `hook_success` and the subagent stops (probed 2026-09-21; `loomwright/docs/HOOKS.md` §"Command-validator decision shape", fixture `scripts/fixtures/subagentstop-decision-shape-probe.json`). **`|| true` convention:** every `type: command` hook string carries `|| true` as belt-and-suspenders — valid ONLY because all of them are always-exit-0 fail-safe emitters/validators. A future blocking gate (non-zero-exit) added as a `type: command` hook must NOT carry `|| true` — that would silently neuter it (see §"Failure-Mode Invariants" below). **That future gate has now arrived:** the test-integrity guard's `PreToolUse[Bash]` and `PreToolUse[Write|Edit]` leaves (both invoking `scripts/guard-test-integrity.sh`, six-phase-loop-gaps/02) are the plugin's first fail-CLOSED blocking `type: command` hooks and are the ONLY two command-hook leaves in `hooks.json` that carry NO `|| true`, by design — see `docs/HOOKS.md`'s guard row for the full arm/disarm/opt-out contract.
...
**`gh pr merge --squash` has exactly ONE sanctioned executor (invariant — do not break):** the **only** place in the plugin that EXECUTES `gh pr merge --squash` is the `automate-loop` `--auto-merge` gate (`skills/automate-loop/SKILL.md` §10, implemented by `loomwright/scripts/automate-helpers.sh`'s `gate-eval`). It is **opt-in, default-OFF**, and fires only when ALL SIX trusted-merge conditions hold (the sixth — `scripts/classify-risk.sh` says the diff is not high-risk — has NO override of any kind: not `--trust-unprotected`, not a config key, not an exclude list); any failed / null / unreadable condition fails **CLOSED** (park + notify). **The gate is SELF-RESOLVING (red-team-hardening item 03): it computes its own conditions 2–6 from live `gh`/GraphQL/`classify-risk.sh` reads — a caller cannot hand it a verdict** (a `ctx.json` carrying a gate-owned key — e.g. `high_risk`, `head_sha`, `review_decision` — is refused outright, `PARK: ctx_carries_gate_owned_key`, before any condition is even evaluated). `review-heal` (`/review-pr`) and Supervisor Phase 4.5 **NEVER merge** — they review-and-heal only and leave the PR open for a human (`READY`/`PASS`/`ESCALATED` are all terminal-stop, merge-identical). The positive-form invariant check is `grep -rn "gh pr merge --squash" loomwright/ | grep -viE "no |never |not "` — it resolves to exactly five surfaces: `skills/automate-loop/SKILL.md`, `loomwright/scripts/automate-helpers.sh` (the executor), `loomwright/scripts/test-automate-helpers.sh`, `commands/agent-help.md` (which describes the sanctioned gate), and `loomwright/scripts/result_block_parser.py` (a comment at `result_block_parser.py:1379` [pins: `convention CLAUDE.md already uses`] citing this grep's negation-filter convention as precedent for the worker destructive-command tripwire — a prose mention in a parser, it executes nothing). (`commands/automate.md`'s own two mentions are correctly EXCLUDED by the `no|never|not` filter; review-heal / review-pr / RESULT_SCHEMAS likewise keep only their negative-assertion "never merges" mentions.) See `skills/automate-loop/SKILL.md` §11 for the authoritative enumeration. Adding a second executor anywhere else is a regression.
