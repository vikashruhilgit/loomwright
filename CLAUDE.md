...
**v14.42.0 — worktree-isolate the detached until-mergeable review drain + retire the suppress-for-the-self-heal-race kill-switch (additive):** Ends the four-PR review-drain dispatch saga (#67/#70/#71/#72) by removing the **root hazard** instead of adding a fifth dispatch guarantee: the detached `review-pr-runner` drain used to run in the SAME working tree/index as the inline Phase 4.5 self-heal (`git checkout "$HEAD_REF"` in the shared checkout), so each reliability fix re-exposed the collision → an operator suppressed it via `.supervisor/config.json {"auto_review": false}` → the kill-switch had no safe restore → a genuine drain (PR #74) was silently dropped. **(1) Isolated worktree (`scripts/dispatch-pr-review.sh`):** the detached dispatched drain now runs in its OWN git worktree — a plugin-owned **sibling** of the primary checkout (`../{project}-review-{pr_hash}`, OUTSIDE the tracked tree so an inline `git add -A` can't sweep it), via a **detached-HEAD checkout at the PR head SHA** (`git worktree add --detach <path> <sha>`) so a same-branch drain never hits "branch already checked out"; it never checks-out/stages/commits in the inline session's tree. Scope: isolation targets only the *detached dispatched* path (the one concurrent with self-heal); an inline `/review-pr` session stays worktree-free. **(2) Atomic claim + stale-lock reclaim + marker-after-success (pinned AC4a order):** existing-marker-wins `exit 0` → verify `claude` launchable else `exit 0` with no marker → atomic `mkdir "<pr_hash>.lock"` (loser exits 0; lock records `pr_url`/`pid`/`ts`/paths) → create worktree → write RUN_LOG header → write the durable marker → launch the detached wrapper. The marker is written **only after** worktree+header succeed, so it can never claim a dispatch that did not start; stale-lock reclaim fires ONLY when no-marker + pid-dead + past-TTL (closing the crash-between-`mkdir`-and-marker wedge). **(3) Executable trap-owned cleanup + fork-aware push:** a `trap cleanup EXIT` wrapper (not a skippable prompt instruction) removes ONLY the dispatcher-created hash-keyed worktree+lock (no broad `*-review-*` sweep); same-repo pushes use an explicit refspec (`git push origin HEAD:<head_ref>`, never `--force`); cross-repo/fork PRs degrade to review-only `ESCALATED`. **(4) Retire the race rationale:** the three "no worktree fan-out" surfaces (`skills/review-heal/SKILL.md`, `commands/review-pr.md`, `docs/RESULT_SCHEMAS.md`) are reconciled to scoped wording, the `agents/supervisor.md` step-5.5 timing note drops "concurrent-with-self-heal is safe-by-degradation", and **no repo guidance recommends `auto_review:false` to suppress the drain for the self-heal race** (the flag REMAINS as a general opt-out — only the suppress-for-the-race rationale is retired). **(5) Missing-drain reconciliation in `/insights` (`scripts/build-insights.sh`):** a new advisory `## Missing-drain reconciliation` section surfaces heal-outcome PRs with NO dispatch marker, classified `opted_out` (only on durable run-time evidence, never inferred from the mutable current `config.json`) vs `unknown_or_opted_out` (default; a genuine silent drop like #74 surfaces here); read-only, SUPPRESSED when no corpus exists. **(6) Invariants preserved:** all dispatch paths still ALWAYS `exit 0` / fail-safe; the drain still NEVER merges and NEVER `--force`-pushes; the audit is advisory-only; the two existing dispatch paths + idempotency marker are unchanged (this REMOVES a failure mode, it does not add a fifth dispatch path). **No new agent / command / skill / hook (counts unchanged at 14 agents / 19 commands / 56 skills / 20 hooks); no `schema_version` bump.** Additive on top of v14.41.0.

**v14.41.0 — `/automate` generic automation engine (single run file → per-item loop, additive):** A new `/automate` command + `automate-loop` authority skill that walks **arbitrary work from any starting point** to a reviewed PR (and, opt-in, a trusted merge). It converts **any source** — a prompt (via `/product-owner`), a requirements folder, or a backlog/plan doc — into a **full Queue with a per-run processing cap** inside **one markdown run file** `.supervisor/automate/<run_id>.md` (the contract, dashboard, and resume state — **no manifest / registry / progress.jsonl ledger**; "find prior runs" = glob `.supervisor/automate/*.md` not `## Status: done`), then drives each Queue item through the per-item loop: `/autonomous --single-iteration --requirement <path>` → **one owned inline `/review-pr --until-mergeable` drain** → trusted-merge-or-park → sync `main` → check the item off + append the **append-only `## Progress`** (atomic temp+rename writes). **Layering:** `/autonomous` (one requirement) ⊂ per-item loop ⊂ `/automate` (source → Queue → loop); **`/backlog` is NOT a separate command** — a folder/backlog-doc is just one kind of source. **Smart resume** globs the run files for incomplete runs and **reconciles each in-flight item against ground truth** (`gh`/`git`/`## Status: done`) before trusting a checkbox; ambiguous resume fails **closed** under `--non-interactive-fallback` (`resume_ambiguous_non_interactive`). **Single-drain ownership:** the loop sets `.supervisor/config.json {"auto_review": false}` around the inner `/autonomous` RUN phase (byte-for-byte backup to a transient `<run_id>.config-backup.json` + absent-delete + malformed-abort + RECONCILE crash-restore) and owns exactly ONE inline drain — no double-dispatch. **Two modes:** safe (default — a `READY` drain parks `awaiting_merge` for a human; nothing merges automatically) and opt-in `--auto-merge` (default OFF) behind a **5-condition fail-closed trusted-merge gate** (drain `READY` + head-SHA-unchanged & base `main` + `reviewDecision` ∉ {`CHANGES_REQUESTED`,`REVIEW_REQUIRED`} with GraphQL-only unresolved-human-thread check + enforceable branch protection or `--trust-unprotected` + required-checks-green-AND-rubric-satisfied from `SUPERVISOR_RESULT.rubric_score`) — the **only place in the plugin that executes `gh pr merge --squash`**; `ESCALATED` never merges and **parks** the run (single-open-PR invariant holds in BOTH modes). `--limit N` (default 5) caps **PROCESSED** items, not Queue size — the full resolved Queue is materialized and **human-confirmed before processing**. `AUTOMATE_RUN` documented at `schema_version: 1` as a **markdown state-file contract, NOT a hook-validated emitted result block** (nothing validates it at a SubagentStop). Helpers + **50 stubbed-`gh` self-tests** (`scripts/automate-helpers.sh` + `test-automate-helpers.sh`: config suppress/restore, `config-orig` true/false/absent/malformed-abort, atomic run-file write + append-only Progress (incl. create-section), queue check-off idempotency + `abandoned` mark, resume reconcile, auto-merge fail-closed on each blocker incl. null/absent-rubric, the reviews-not-required reachability paths, and the fail-CLOSED unresolved-human-thread path). **New: 1 command (`/automate`), 1 skill (`automate-loop`); no new agent / hook / runner (`/automate` is inline-only) — counts move to 14 agents / 19 commands / 56 skills / 20 hooks; no `schema_version` bump.** Additive on top of v14.40.0.
...
- Plugin manifest: `ai-agent-manager-plugin/.claude-plugin/plugin.json` (v14.42.0)
...
`git worktree list`; `git worktree remove ../project-BD-XXa`; `git branch -d feature/BD-XXa`. The detached until-mergeable review drain also creates a sibling worktree (`../{project}-review-{pr_hash}`, detached-HEAD at the PR head SHA) owned by `dispatch-pr-review.sh`'s `trap cleanup EXIT` wrapper; on a hard crash it self-prunes only its own hash-keyed path on the next bounded `git worktree prune` — remove a stray one the same way if needed.
