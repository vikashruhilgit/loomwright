...
**v15.11.0 — sdk-spike token levers: per-role effort + opt-in per-subtask task budget + first-class token accounting (quarantine unchanged, additive):** All inside the quarantined `loomwright/sdk-spike/` runner (still UNCOUNTED, opt-in `--sdk-runner`, default path byte-identical). (1) **Runner config layer** — `ROLE_CONFIG` table (worker effort `medium`, reviewer `high`; precedence `--worker-effort`/`--reviewer-effort` > `--effort` > table) with fail-closed validation (invalid effort/budget ⇒ non-zero exit, no query issued); opt-in `--task-budget N` per WORKER query (documented 20k minimum enforced runner-side — < 20000 fails closed; omitted entirely from `Options` when unset). (2) **Per-subtask token accounting** — every EXECUTE_RESULT-equivalent subtask entry carries an additive `token_usage` object (real `usage`/`total_cost_usd`/`num_turns` in live mode, incl. `QueryFailedError` fold-back of a failing query's real spend; `proxy: true`-labeled zeros on `--dry-run` — token counts never invented). (3) **Context editing / history pruning: NOT exposed by pinned SDK 0.3.202** — recorded as a capability-matrix gap (PreCompact/PostCompact hooks + `getContextUsage()` only); pin unchanged. (4) **Self-test 25/25 → 41/41** (config parse valid/invalid, budget floor/omission, accounting fields in dry-run output, positive-path effort-precedence + worker-only taskBudget trace assertions via opt-in `SDK_SPIKE_TRACE_OPTS=1`, worker-arm QueryFailedError fold-back via `throw-usage-worker`). (5) **Docs:** `SDK_RUNNER_SPIKE.md` capability matrix +3 doc-verified rows (sdk.d.ts line refs; live verification of taskBudget/effort/QueryFailedError paths PENDING — the first live eval run is their first exercise); `FABLE_PARITY_EVAL.md` additive datestamped amendment (amended 2026-07-18, before any run) adding token-cost-per-subtask as a RECORDED observable — the 1.5× decision rule byte-unchanged. **Counts UNCHANGED: 14 agents / 21 commands / 41 skills / 22 hooks; no `schema_version` bump.** Additive on top of v15.10.0.

> 📜 **Full release history** (v15.10.0 → v14.0.0 and earlier) lives in [`CHANGELOG.md`](CHANGELOG.md). CLAUDE.md keeps only the two most recent release notes.
...
- Plugin manifest: `loomwright/.claude-plugin/plugin.json` (v15.12.0)
...
├── scripts/                                   # CI validators (version, doc-currency, skills-index, command-sync, contract-parity, token-budget)
