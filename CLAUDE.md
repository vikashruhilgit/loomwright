...
**Latest change:** **nine surfaces, nine parsers, and a number that meant two different things.** The run state a view needs is already real and already on disk, but scattered across **nine surfaces in four formats** — markdown tables, folder membership, JSONL and JSON. With no single reader, every future view re-implements the same nine parsers and drifts independently. **`build-floor.sh`** is that reader and `.supervisor/floor/floor.json` is the contract: read-only, deterministic, mirroring `build-insights.sh` exactly — `set -uo pipefail` WITHOUT `set -e`, a `command -v jq` guard that skips rather than fails, exit 0 always. **Every count states its own counting basis, because a bare number was already ambiguous:** `.supervisor/logs/` holds 99 directory entries but 65 `*.jsonl` session logs — the rest are plain `.log` dispatch transcripts. Both readings are defensible, they differ by 34, and the source requirement picked the other one; the glob now ships beside the number. **Absent evidence is an omitted key, never a plausible default:** an uncountable surface carries no `count` key at all — never `0` — and names the reason, and an input that will not parse reports `unverified`, never `counted`. The mtime helper copies `build-handoff.sh`'s GNU-then-BSD **order** (load-bearing: on GNU, `stat -f` means *filesystem status* and succeeds printing a mount point — the bug that once shipped macOS-green and Linux-CI-red) but deliberately **not its call sites**, which default to `0`; a `stat` stub printing `/` must make the field OMITTED, not empty, not `0`, not the raw string. **Sessions segment by the `cc_session_id` field, never by filename** — a log file is named for a `status:` that can read `running` for weeks, so one file spans many sessions; no emitter change was needed. **A real defect was caught by the design:** `jq -e .` takes its exit status from the last OUTPUT value, so a valid document whose value is `null`/`false`/`0` exits 1 and would have been reported malformed; it uses `jq empty`, bulk-first then per-file only on failure, because jq's bulk error does not name the file and "the reason is named" requires naming the offender. **The schema block is annotated YAML, not a JSON example** — the self-test parses the required-key set back OUT of it (the `emit-block-parses` precedent), so a doc/validator divergence fails CI; a fenced JSON example would carry no required/optional marking and silently imply every field is mandatory, contradicting the omit-absent-evidence rule. Each required key removed in turn is rejected by name, with controls proving the intact payload passes and an optional-key removal still passes. **The tests are hermetic by construction, and that is the load-bearing property:** `.gitignore`'s `.supervisor/*` means eight of the nine inputs are absent from every fresh clone and worktree — measured, a fresh worktree has zero `logs/*.jsonl` — while CI runs the suite in exactly that environment, so a real-tree assertion would compare 0 to 0 and go silently green. The real-tree run is a local-only corroboration that prints `SKIPPED — <reason>`, is reported separately from passes, and reads through a symlink mirror so the developer's checkout is provably byte-unchanged. **Containment is hashed with `find`, never git:** `.supervisor/` is *ignored*, not untracked, and a probe written into it is reported by neither `git status --porcelain` nor `--ignored`, so a git-based tree check could never fail — the test measures that blindness rather than asserting it. Determinism diffs two **raw, unfiltered** runs under two **different** injected timestamps, because filtering the timestamp line first would never exercise the filter (consecutive runs share a wall-clock second) and would pass *with* the defect. Mutation controls prove both invariants can turn red: a variant writing one byte outside the output directory, and a variant embedding a second clock read — each gated on still being `bash -n` clean and still reaching its own success line. **Two defects were found inside this release, both recorded because the summary above would otherwise assert them away.** `exit 0 always` was FALSE on every fresh clone and worktree — macOS bash 3.2 aborts expanding an EMPTY array under `set -u` while bash 4+ on Linux does not, so it was **macOS-red / Linux-green**, the inverse of the `stat`-flavour trap and invisible to CI (`${arr[@]+"${arr[@]}"}` fixes it). And the projector **twice proved a zero it never measured**: `count_json_docs` lacked the readability guard its sibling `count_glob` already had (nullglob makes an unreadable directory look empty, so it emitted `counted: 0` beside an `mtime_epoch` implying a real reading), and the session reader's bulk `cat` silently dropped an unreadable log while reporting `lines_malformed: 0`. The root cause was the suite, not the script — it made no input unreadable anywhere, so all three *existing* `! -r` branches had never executed; the new coverage was written first and observed failing 15 assertions against the unfixed script (105 → 155, then → **187** when two further branches were found that the suite could not have failed on: blank log lines, unreachable even from real data, and `generated_at_epoch: null` on a failed clock read — both verified correct before being pinned). Counts unchanged. Full narrative in `CHANGELOG.md`.
