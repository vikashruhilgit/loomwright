...
**v15.12.0 — orientation memos + owned repo map: committed per-area memo substrate + memos-first read seams + completion-tail proposals + `/dreaming` promotion + `orientation_source` ledger attribution (additive):** (1) **Orientation-memo substrate** — NEW committed store `.agent/orientation/` (one ≤1000-char memo per area; machine-parsed `written_at | head_sha | areas` header) + fail-safe advisory reader `read-orientation.sh` (hostile-marker skip, git-log staleness demote-never-drop, ≤3000-char output, always exit 0) + confirm-gated sole writer `add-orientation.sh` (slug-containment + injection-marker REJECT, atomic write + read-back verify; non-TTY without `--confirm` = dry-run). (2) **NEW `loomwright/scripts/build-repo-map.sh`** — owned FLAT repo-map builder (Tier B zero-dep floor + best-effort tree-sitter Tier A; bounded, advisory, fail-safe) productizing the parked CODE_GRAPH_OWNERSHIP tier WITHOUT the failed PageRank ranker. (3) **Memos-first read seams** — Launch Pad / Orchestrator consult orientation memos first (seam documented in `skills/brain-context/SKILL.md`). (4) **Completion-tail proposal seam** — Phase 4.5 completion tail (self-heal-advisory Part 2 step 2.6) MAY write per-area proposals to gitignored `.supervisor/orientation-proposals/` on PASS/loop-skipped/ESCALATED only; fail-safe, never gating, NEVER writes the committed store. (5) **`/dreaming` promotion** — per-item Accept promotes a proposal via `add-orientation.sh … --confirm` (literal argv) then deletes it; Reject deletes without writing. (6) **`orientation_source` ledger field** — `emit-token-ledger.sh` emits the additive field (`memos|repo_map|graphify|none`) from `LOOMWRIGHT_ORIENTATION_SOURCE`; invalid/unset ⇒ omitted (fail-safe). **Counts UNCHANGED: 14 agents / 21 commands / 41 skills / 22 hooks; no `schema_version` bump.** Additive on top of v15.11.0.

> 📜 **Full release history** (v15.11.0 → v14.0.0 and earlier) lives in [`CHANGELOG.md`](CHANGELOG.md). CLAUDE.md keeps only the two most recent release notes.
...
- Plugin manifest: `loomwright/.claude-plugin/plugin.json` (v15.13.0)
