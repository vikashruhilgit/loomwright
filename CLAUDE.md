...
**v14.29.0 — Learning Loop Phase 3 residual: CI miss-class vocabulary + drift-taxonomy split + opt-in advisory red-team (additive):** Ships the residual of Learning Loop Phase 3 (its core landed in v14.21.0) as three **additive, advisory** review-quality refinements with **no schema_version bump and counts unchanged (14 agents / 18 commands / 55 skills / 19 hooks)**. (R4) The CI review prompt (`.github/workflows/claude-code-review.yml`) gains a named six-miss-class vocabulary so review comments classify *how* a defect was missed rather than just flagging it. (R3) The previously merged `drift` review category is split into two named classes — **count/version/restated-list drift** and **cross-reference precision drift** — surfaced in `quality-checklist` / `pr-postmortem` so reviewers triage the two failure modes distinctly. (R1) Supervisor Phase 4.5 gains an **opt-in, default-OFF, fail-safe, NON-gating** advisory red-team lens (`--red-team` / `--no-red-team`, `.red_team_high_risk` config) that runs only on high-risk integrated diffs and records its findings in `SUPERVISOR_RESULT.summary` + the job Outcome block — it can NEVER block a PR or change a `heal_decision`. Additive on top of v14.28.0.
