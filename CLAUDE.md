...
**v14.38.0 — fix the inline-path until-mergeable review-drain gap v14.37.0/#70 left open (additive):** v14.37.0 wired the post-PR until-mergeable review-drain producer write into `agents/supervisor.md`, but that prompt is loaded only by the *delegated* `supervisor-runner` subagent — the inline `/supervisor` & `/autonomous` main-thread paths never read it, so on those paths the hook's session-scope gate still saw no truthful `state.md`/`state.json` and the drain stayed silently un-dispatched. **Fix A (load-bearing):** the canonical lowercase `## Session` write (`- status: running`, `- branch:`) plus the Phase 4.5 completion-tail status flip are now ALSO directives in the **loaded** `commands/supervisor.md` Phase 1 ACQUIRE (the file BOTH inline entry points read), mirrored with the existing `agents/supervisor.md` write as a sync pair — best-effort/non-fatal. **Fix B producer (`/autonomous`-only):** inline ACQUIRE additionally writes two top-level `state.json` ACQUIRE signals — `current_branch` and a non-terminal `current_status: "running"` — carved out of the `_v1_note` advisory-only rule as deterministic hook-readable fields (the rest of `state.json` stays advisory). **Fix B consumer:** `hook-dispatch-on-pr-create.sh` now resolves the active session from ONE coherent source — a non-terminal + branch-matching `state.md`, ELSE a UNIQUE active autonomous `state.json` (branch + brief-in-`in-progress/` + non-terminal `current_status` + null `ended_at`) — so a **stale terminal `state.md`** left by a prior session no longer short-circuits dispatch ahead of the `state.json` fallback. Still fail-closed / anti-hijack (no current-git-branch fallback; #67/#70 preserved); the hook still ALWAYS `exit 0`, and its self-test grows 19 → 29 (headline stale-terminal-`state.md` + valid `state.json` → dispatch, plus stale / multiple / wrong-brief / neither-source fail-closed cases and a Source-1 status-presence guard). All fail-safe invariants preserved (state writes + hook never gate, never change `heal_decision`, never block the PR/run). **No new agent/command/skill/hook; counts unchanged at 14 agents / 18 commands / 55 skills / 20 hooks; no `schema_version` bump.** Additive on top of v14.37.0.

> 📜 **Full release history** (v14.37.0 → v14.0.0 and earlier) lives in [`CHANGELOG.md`](CHANGELOG.md). CLAUDE.md keeps only the two most recent release notes.
...
- Plugin manifest: `ai-agent-manager-plugin/.claude-plugin/plugin.json` (v14.39.0)
