...
**Latest change:** **a run you can watch, and a page that never claims more than it recorded.** The projector made the run state legible but nothing rendered it, so a stall was still discovered only when a human noticed nothing had happened for a while. `/setup ui` is that view: a three-file, framework-free bundle — no bundler, no Node, no package manager, no web font — copied into the ui directory and served with `python3 -m http.server --bind 127.0.0.1`, regenerating `floor.json` on an interval, with five pipeline stages above one lane per agent in the newest session. **Motion is evidence, never decoration:** exactly ONE timer exists on the page, and a lane's shuttle advances only when THAT lane's recorded `events` count changed between two renders — proven in a real browser with a control, where bumping one lane's count advanced only that lane and left the others still. The single stated exemption is the pulse on a non-stalled lane, whose ABSENCE is the stall signal, so it is state-driven even though a keyframe drives its frames; the static suite pins the budget by OCCURRENCE COUNT with a mutation control that adds a second timer, because a grep for the string would have passed on a page full of them. **The page never claims more than the record supports, and the omissions are the design.** There is no liveness and no "currently running agent": no spawn event exists in any log, so it could only be inferred from another record's recency — a guess — and the page renders that sentence permanently under the lanes. `agent_type` is carried by a small minority of events, so an untyped lane is drawn `identity unknown` rather than assigned a plausible role, and `read_only` is a genuine TRI-STATE — true, false, or OMITTED when the agent declares no disallowed-tools line — whose third value renders "read-only unknown", never silently as "not read-only". A lane's number is EVENTS, not turns and not progress. **Absent, empty, unavailable and stale are four distinct renders, never a blank page, a spinner or a console error**, and the page earns the idle one rather than assuming it: `no run in flight` is claimed only when the session surface was actually counted and carried a view with nothing in it, while every other no-lane shape reports `session data unavailable` with the projector's own reason. The first three were browser-verified against committed fixtures with the console read, the fourth by the static suite and its fixture; `?stale=<seconds>` ships beside `?stall=<seconds>` for a measured reason — a committed fixture's generation time is always in the past, so without it every fixture renders under the stale banner and nothing else can be demonstrated. **Loopback is the whole security posture and is deliberately not negotiable at the command line:** the bind address is passed on every path with no flag to change it, the page declares a self-only content policy and fetches one relative path, an egress scanner with a mutation control covers all three bundle files, and a real network log of a page load showed the loopback address only — which matters because the projection carries branch names, session ids and agent ids. **The engine is marker-gated because the target directory is user-supplied:** apply and remove both WITHHOLD on a directory this module did not create, and remove additionally refuses the filesystem root, the home directory and the plugin install dir — a typo pointing at a real directory must not cost the user that directory. Every branch exits 0 (missing interpreter, busy port REPORTED and never silently moved, missing projector each named by reason), the fail-safe convention the status-line engine set. Residue is proven by hashing TWO whole trees with `find` before and after a full apply → serve → stop → remove — the fixture parent and a fixture git repo used as the working directory — with an anti-vacuity check that the one permitted write actually happened, since the containment assertion would also pass had nothing ever regenerated. **The projector extension is additive and omits rather than defaults:** an agents roster read from frontmatter, newest-session per-agent rows, and subtask rows, every field dropped when absent. Its suite stays hermetic by construction — the agents directory is redirected into the fixture on every run, so the EMPTY fixture still yields an absent surface and the anti-vacuity control stays green; a real-directory default would have counted the real agents there and turned that control red. **The registration surfaces finally have a gate:** the command-sync check guards one file and doc-currency derives no module count, so a release-surface parity group now asserts the registry row, the module flow, the bundled-status recipe and zero stale module-count residue — and the ratchet allowances were MEASURED after staging every new file, never hand-typed. Full narrative in `CHANGELOG.md`.
