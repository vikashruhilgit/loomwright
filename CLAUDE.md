...
**v15.14.0 — curation/anti-rot: supersession + retraction across the three curated stores + whole-stack advisory budget + freeze rule (additive):** Lifts the item-02 half of the standing advisory-surface freeze (00-overview.md §"Standing policy"). (1) **Supersession + retraction** — `read-rules.sh`/`add-rule.sh` gain a declarative `--supersedes` flag on add plus a `--retract --target/--reason` action; `read-orientation.sh`/`add-orientation.sh` gain the same `--supersedes`/`--retract` shape (fixing a greedy-capture bug in the line-1 header parse that was silently disabling staleness detection — the field now parses per-key); `write-lessons.sh` gains a `supersede` verb composing PRE-CHECK → RETRACT → ADD (in that order — add-then-retract would destroy an unrelated lesson via the add path's evict-oldest-from-front eviction). All three readers stay single-hop, demote-never-crash, always-exit-0. (2) **`## Corpus health` extended** (not duplicated) with rules/orientation decay flagging — flag-only, never auto-delete. (3) **Whole-stack advisory budget** — additive `advisory_total`/`advisory_total_kind` fields in `emit-token-ledger.sh` plus an `/insights` advisory-budget subsection, reconciled against the pre-existing era-bucket `advisory_tokens` compute-spend proxy (documented as a distinct measure, not superseded). (4) **`/dreaming`** gains per-item Supersede/Retract actions (alongside Accept/Reject/Edit) plus a documented idempotent one-line harness-memory pointer write, executed at repo root only (never from a worker worktree). (5) **The freeze rule** — the standing advisory-surface freeze is now committed as a `.agent/rules/` entry (category `process`, enforcement `advisory`) via `add-rule.sh … --confirm`, so it survives sessions. Lessons retraction and the postmortem-corpus curation tool already shipped in v15.7.0 and were deliberately NOT rebuilt here. **Counts UNCHANGED: 14 agents / 21 commands / 41 skills / 22 hooks; no `schema_version` bump.** Additive on top of v15.13.0.

**v15.13.0 — pointers-not-payloads + shared agent prefix + async downpricing (additive):** (1) **Pointer audit + conversions** — NEW `loomwright/docs/POINTER_AUDIT.md` enumerates every Task-spawn / inter-agent handoff site pasting file-backed content; paste sites converted to pointers (absolute path + bounded summary + "Read only the sections you need") or documented inline as justified exceptions; Batch-API routing recorded as a roadmap note only (no code — subscription runtime has no API-key path). (2) **Shared agent prefix + fail-CLOSED CI gate** — all 14 `agents/*.md` open with a byte-identical shared block sourced from NEW canonical `loomwright/docs/shared-agent-prefix.md` (deliberately `docs/`-resident so agent-count/budget gates don't count it as a 15th agent); NEW `scripts/check-shared-prefix.sh` proves byte-identity in CI, fails CLOSED (no `|| true`), with a self-test proving 1-char-drift detection; budget raises landed in `prompt-token-budgets.json` + the `ARCHITECTURE_CONTRACTS.md` mirror per the gate's raise rule. **HONEST CACHE FRAMING:** the block buys consistency/dedup/smaller prompt inventory, NOT cross-agent cache hits (tools render before the system prompt — different agent types diverge at position 0); the real cache win is SAME-ROLE respawns via volatile-last ordering. The block applies to `agents/*.md` ONLY — bullets deduplicated out of an agent's Critical Rules stay RETAINED in the mirrored `commands/*.md` lists. (3) **Cost Profiles async-analysis rows** — `ARCHITECTURE_CONTRACTS.md` §Cost Profiles gains an "Async analysis surfaces" table: `/dreaming` reflection spawns (code-reviewer / red-team-reviewer / qa-executor, REFLECTION mode only) default to `model: "sonnet"` at spawn time, `/dreaming --full-model` restores `inherit`; forward/gating/voter roles NOT downgraded; `/pr-postmortem` and `/insights` verified to spawn no models today (recorded, no rows). (4) **`shared_prefix` ledger marker** — `emit-token-ledger.sh` emits additive `"shared_prefix": true` only when `LOOMWRIGHT_SHARED_PREFIX=1`; any other/unset value omits the field (fail-safe, always exit 0; self-test extended with marker-present/-absent cases; documented in `docs/TELEMETRY.md` beside `orientation_source`). **Counts UNCHANGED: 14 agents / 21 commands / 41 skills / 22 hooks; no `schema_version` bump.** Additive on top of v15.12.0.

> 📜 **Full release history** (v15.12.0 → v14.0.0 and earlier) lives in [`CHANGELOG.md`](CHANGELOG.md). CLAUDE.md keeps only the two most recent release notes.
...
- Plugin manifest: `loomwright/.claude-plugin/plugin.json` (v15.14.0)
...
| Orchestrator | user-facing | user | Reads CLAUDE.md (+ Beads when active) → EPIC / TASK / SUBTASK with skill references. Defaults to one task per `skills/supervisor-readiness/SKILL.md` §"Decomposition Threshold" (split only for a named reason); paired review subtasks are threshold-conditional (mandatory above threshold, Phase 4.5 integrated review is the gate below it — see `agents/orchestrator.md` §"Review Gate Policy"). **Beads-optional:** a `## Persistence Mode` block branches on `beads_active` (probe `test -d .beads && bd --version`); when absent, skips all `bd` and writes the task tree to `.supervisor/requirements/{slug}-plan.md` — the threshold-conditional review policy applies in both modes. Detection logic already lived in the shared `context-setup` skill; this wires output to it (matching Code Reviewer's long-standing Beads-optional pattern) |
...
- Single-Agent Path (exactly 1 subtask, the default below `skills/supervisor-readiness/SKILL.md` §"Decomposition Threshold"): skips worktrees and Execute Manager entirely, one worker executes all acceptance criteria, no per-subtask Code Reviewer — Phase 4.5's holistic review is the single review
