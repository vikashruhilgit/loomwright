...
**v15.8.0 — Fable-parity: quarantined Agent SDK runner spike + opt-in `--sdk-runner` Phase 3 seam + opt-in `--multi-voter-heal` Phase 4.5 verification + pre-registered FABLE_PARITY_EVAL protocol:** Closes the prompt-vs-code orchestration gap in the smallest falsifiable way (roadmap item 11 DEFERRED → SPIKED). (1) **Quarantined `loomwright/sdk-spike/` (UNCOUNTED — referenced ONLY by the opt-in `--sdk-runner` seam, default OFF; `dist/` is gitignored so nothing resolves until a one-time build; no default/always-on path reaches it; absent from every plugin manifest):** a TypeScript port of ONLY Execute Manager's Phase 3 poll loop via `@anthropic-ai/claude-agent-sdk` (pinned `^0.3.202`, committed lockfile) — brief parsing, `requires`-driven wave scheduling replacing the prompt poll loop, one `query()` per worker/reviewer with schema-forced WORKER_RESULT v2 / CODE_REVIEW_RESULT v3, worktree lifecycle with commit-before-remove on `sdk-spike/subtask-N` branches, fail-closed structured-output handling, EXECUTE_RESULT-equivalent output; offline `--dry-run` + self-test (20/20). (2) **`--sdk-runner` (EXPERIMENTAL, opt-in, default OFF):** Phase 3 shells out to the built runner instead of Task-spawning execute-manager; **fail CLOSED** with `error: "sdk_runner_unavailable"` if node/runner absent — never a silent fallback; explicit FINALIZE delta (branches-ahead verification, skip in-worktree commit, delete runner branches after merge); byte-identical default path with flag off. (3) **`--multi-voter-heal` (opt-in, default OFF; `.multi_voter_heal` config; `--no-multi-voter-heal` wins):** Phase 4.5 review spawns an independent `red-team-reviewer` verification vote alongside `code-reviewer`; a BLOCKING/HIGH `new` finding is fixed ONLY if it survives the other lens's refute check (≤1 refute spawn per lens per iteration; refuted findings logged, not fixed); `findings_raised`/`findings_refuted`/`findings_fixed` counters in `SUPERVISOR_RESULT.summary` (additive prose, no schema bump); gate shape, bounds, and never-merge unchanged; authority incl. the standalone `--red-team` interaction: `skills/self-heal-advisory/SKILL.md` Part 2 §Multi-voter verification. (4) **Cost profiles:** `phase45-multi-voter-verification` row in `ARCHITECTURE_CONTRACTS.md` §Cost Profiles (default inherit; `--cheap` ⇒ sonnet); routing comments in both reviewer frontmatters (comment-only). (5) **Spike record + pre-registered eval:** `docs/SPIKES/SDK_RUNNER_SPIKE.md` (doc-verified capability matrix, parity matrix, provisional GO/NO-GO — GO for a v16 runner ONLY if the eval's arm 3 wins, else cut) + `docs/SPIKES/FABLE_PARITY_EVAL.md` (5 requirements × 3 arms — bare CC / Loomwright default / Loomwright + both opt-ins; 15-run hard cap; SDK runner graduates ONLY if arm 3 beats arm 2 on defects or rounds without >1.5× token cost; Loomwright must beat arm 1 or losing layers get cut). **Counts UNCHANGED: 14 agents / 21 commands / 41 skills / 22 hooks; no `schema_version` bump.** Additive on top of v15.7.0.

**v15.7.0 — knowledge-corpus curation: human-gated churn-ledger retract/supersede + staleness, lessons `retract` tombstones, `/insights` corpus health, pre-registered advisory-loop eval scaffold:** The curation half of the Twin — the knowledge corpora can now forget, human-gated and auditable. (1) NEW `scripts/curate-postmortem.sh` — the human-gated **sole curator writer** for the churn ledger: `retract|supersede --target <key> --reason <text> [--replacement <pr_url>] --confirm` appends ONE jq-built `source:"curation"` record (`target_key` matches a data line's `automate_key` OR `pr_url` by exact equality) to `.supervisor/postmortem/results.jsonl`; WITHOUT `--confirm` it prints the exact would-append line and exits 1 (dry-run — never writes unattended); append-only, ADDITIVE (POSTMORTEM_RESULT stays `schema_version: 1`; old readers fail-safe-skip via the `changed_paths` overlap filter). (2) `read-postmortem.sh` now EXCLUDES curation-targeted entries (presence-disciplined — a record with a missing/null/empty `target_key` curates NOTHING; targeted entries stay live) and drops data entries staler than `CHURN_STALE_DAYS` (env, default 180 days; missing/unparseable `ts` = FRESH, fail-open); curation records never count as churn hits; EMPTY⇒silent + always-exit-0 preserved. (3) `write-lessons.sh` gains a `retract` verb (positional `<category> <text>` or `--hash <64-hex>`; exit 4 on an absent/untrusted target) appending a chain-valid `action:"retract"` provenance tombstone — never a silent deletion — plus an atomic heading-preserving LESSONS.md rewrite; `read-lessons.sh` walks the chain last-action-wins (retract untrusts, re-add re-trusts) and logs tombstoned lines with a distinct `RETRACTED` label. (4) `/insights` gains an advisory `## Corpus health` section (churn ledger entries/curated/stale; lessons entries/retracted/stale; degrades gracefully, never gates). (5) NEW `docs/SPIKES/ADVISORY_LOOP_EVAL.md` — the pre-registered counterfactual-eval protocol for the advisory seams (metrics + decision rule declared BEFORE any run; paired runs pending post-merge). NEW `scripts/test-curate-postmortem.sh` + extended `test-lessons.sh`/`test-insights.sh`. **Counts UNCHANGED: 14 agents / 21 commands / 41 skills / 22 hooks; no `schema_version` bump.** Additive on top of v15.6.0.

> 📜 **Full release history** (v15.6.0 → v14.0.0 and earlier) lives in [`CHANGELOG.md`](CHANGELOG.md). CLAUDE.md keeps only the two most recent release notes.
...
- Plugin manifest: `loomwright/.claude-plugin/plugin.json` (v15.8.0)
...
22 hooks centralized in `hooks.json` (the v12.2.0 webhook hook brought the count 13 → 14; v13.0.0 / v14.0.0 added none — v14's `--notify` gate-event webhooks are emitted inline by the autonomous-loop, not by a hook; **v14.1.0 added 2 events / 3 entries** — `PreToolUse[AskUserQuestion]` (notify-desktop + send-webhook) and `Notification` (notify-desktop) — bringing the count 14 → 17; **v14.2.0 added 2 entries** — a `SessionStart` → `session-resume.sh` and a `launch-pad-runner` SubagentStop → `validate-launch-pad-result.py` — bringing the count 17 → 19; **v14.34.0 added 1 entry** — a `PostToolUse[Bash]` → `hook-dispatch-on-pr-create.sh` backstop — bringing the count 19 → 20; **v14.47.0 added 1 entry** — a `SessionStart` → `set-otel-resource-attrs.sh` per-project OTel labeler — bringing the count 20 → 21; **v15.5.0 added 1 entry** — a `WorktreeRemove` → `worktrees.log` logger mirroring `WorktreeCreate` — bringing the count 21 → 22). Prompt-based validation uses fast haiku model with 30s timeout. WorktreeCreate / WorktreeRemove / StopFailure / telemetry / webhook / notification / resume / launch-pad-result hooks use `type: command` for zero-latency. **`|| true` convention (v15.5.0):** every `type: command` hook string carries `|| true` as belt-and-suspenders for missing-interpreter/syntax-error edges — valid ONLY because all of them are always-exit-0 fail-safe emitters/validators (`validate-launch-pad-result.py` included: it decides via stdout JSON, exit-0-by-contract). A future blocking gate (non-zero-exit) added as a `type: command` hook must NOT carry `|| true` — that would silently neuter it (see §Failure-Mode Invariants).
