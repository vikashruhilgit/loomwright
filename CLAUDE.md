...
**v14.33.0 — `knowledge_sources_used` measurement close-out in `/insights` (Learning Loop Phase 2B, additive):** Closes the measurement half of the v14.28.0 memory APPLY path: the `knowledge_sources_used` array (emitted on the `session_end` JSONL line since v14.28.0) was written but never read. `scripts/build-insights.sh` now reads and aggregates it, surfacing a new **`## Knowledge sources (memory APPLY)`** dashboard section — runs-reporting-a-source count, top source tags by frequency, and per-version usage — plus a per-run note line; the section is **suppressed when no run reports a source** (the System-Twin-hard-signal precedent) and tolerant of older logs (absent ⇒ none used). The stale "build-insights does NOT yet aggregate / deferred follow-up" notes in `docs/RESULT_SCHEMAS.md` and `agents/supervisor.md` are reconciled. The optional `LAUNCH_PAD_RESULT.knowledge_sources_used` marker remains **explicitly deferred** (the `validate-launch-pad-result.py` four-field discipline + Launch Pad emits no `session_end` line, so it would not feed `/insights`). **Advisory / non-gating; no `schema_version` bump; counts unchanged at 14 agents / 18 commands / 55 skills / 19 hooks.** Additive on top of v14.32.0.
...
- Plugin manifest: `ai-agent-manager-plugin/.claude-plugin/plugin.json` (v14.33.0)
