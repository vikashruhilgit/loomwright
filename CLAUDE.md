...
**Latest change:** **the plugin had been recording convention violations for months and never converting them into conventions.** **At the time of this release** the ledger held 225 findings, of which **107 (48%) were `convention_mismatch`** — the largest class — and **60 of the 95 self-heal misses (63%)** were that same class, the class review is worst at catching. Those are a point-in-time measurement, not a standing fact: the ledger grows with every PR and nothing re-checks these numbers here, so `docs/RULES_BASELINE.md` is their append-only home, pins the ledger sha they were measured at, and is the place to read them current. (These figures were themselves corrected once, on 2026-08-15: they had been measured from a working tree carrying uncommitted ledger records, so no reader could reproduce them. The 107/60 headline survived; only denominators moved.) `.agent/rules/` held **one** rule. This release closes the loop in three moves. **A new `harvest-conventions.sh` triages both intake sources** — the ledger's `convention_mismatch` entries and the existing `.claude/agent-memory/` corpus — into exactly one of three destinations per candidate, with the reason recorded. It is a **dry run by default with no write mode at all**, and it composes `add-rule.sh` with **stdin detached** (`< /dev/null`), because a no-`--confirm` call is *not* a dry run on a TTY: that writer's `[ -t 0 ] && [ -t 1 ]` branch prompts and writes on `y`, which is exactly how `/dreaming` runs. Batches report **coverage / dedupe rate / scope fidelity**, name the finding ids behind every rule, and self-report a near-1:1 batch as a **failure of distillation**; `check` stays `null` (rules are DATA, no new gate). **`/dreaming` gains its first remote-affecting action and says so at the contract line** rather than three sections down: accepted rules are delivered as a **pull request**, behind **two distinct per-item gates** — the Accept that authorises *content*, and a separate **Pre-push confirmation** that authorises the *push*. Neither implies the other; nothing clearing both means no branch exists at all. The commit carries `.agent/rules/*.json` only, staged by path, and `/dreaming` **never merges**. The same change wires the pending agent-memory-proposals queue into per-item promotion. The "NOT wired here yet" disclaimer was retired where it appeared — in `dreaming.md`, in `write-agent-memory.sh`'s header, and in `AGENT_GUIDELINES.md`'s §"Agent memory write permission", each found by a separate pass. **No claim is made that the sweep was exhaustive**: two of the three were found only after a check scoped to the first reported clean, and the assertion of completeness is itself what made the survivor findable. A retired claim is retired where it is *found*; if another copy turns up, it is a copy to retire, not a contradiction of this note. **A new `/setup rules` module seeds a repo that has accumulated nothing**, with conventions that are *portable*; seeded-vs-learned is **data, not prose** — it rides in `provenance.source`, because the rule schema is frozen and `add-rule.sh` rejects unknown arguments, so a labelling member would have been a create-shaped path with no writer behind it. Finally, **the measurement that can falsify all of it**: `measure-heal-signal.sh --distribution` prints per-class findings/misses shares over **raw** records (a different view from the same script's per-PR confusion matrix), writes nothing at all, and **preserves the wrapper's exit contract unchanged** — asserted twice per case, once with the flag and once without, since a contract that holds on only the new path is not the contract. The baseline lands in `docs/RULES_BASELINE.md` as a committed plain doc, deliberately **not** routed through a sole writer (that writer refuses to run from a worktree, needs a human for `--confirm`, and FIFO-evicts — three reasons the writer route was unexecutable, recorded rather than quietly worked around). It **states its own limits in itself**: 83 of 84 records are `agent_generated_guess: true`, N is small, and there is **no control arm** — so a rising share after rules land is a signal to check whether rules are being injected and read at all, **never proof they failed**. Counts unchanged. Full narrative in `CHANGELOG.md`.
