...
**Latest change:** **the plugin had no visual surface, and the legend nobody generated had drifted.** `grep -rn "statusLine"` over the tracked tree returned **0 hits** — the status-line surface the host offers was entirely unused, so the only way to know a run's phase was to open `.supervisor/state.md` by hand, which is exactly what a user will not do while a long run is in flight. Three things ship. **`status-line.sh`** prints ONE line (phase · branch · COMPLETED-over-total subtasks · age of the newest logged event), read-only, dependency-free, always exit 0, with every field **omitted rather than guessed** when its source is absent. It carries **no active-agent field**, and that absence is the honest answer rather than a gap: the spawn emitter that would make such a field truthful closed NO-GO, so no spawn event exists in any log and inferring liveness from another record's recency would be a guess wearing the costume of a measurement — asserted mechanically in both the output and a whole-word source grep, with a control proving a log line that DOES carry those fields changes nothing. **Time comes from the record, never the filesystem:** the age is parsed from the log line's own `ts` try-BSD-then-GNU and validated numeric before any arithmetic; `stat` is invoked nowhere, because `stat -f %m` is BSD and *succeeds with garbage* on Linux, which would put a plausible wrong number on screen rather than failing visibly. A fixture whose `ts` is three days old must render `3d ago` although the file's mtime is seconds old — the assertion an mtime implementation cannot pass. **A new script-backed `/setup statusline` module** is the only one that writes the user-scope settings document: backup-first, parse-gated (unparseable ⇒ ABORT with no write *and* no backup), atomic, byte-compare idempotent. **The pre-existing status line is the point** — the owner's live settings already carried one, and the source AC protected only keys *unrelated* to `statusLine`, which is precisely the key being written, so a naive deep-merge would have silently destroyed it; a foreign status line is now PRESERVED and REPORTED, replacing it takes an explicit flag, and `remove` restores the recorded prior verbatim. **A real defect was caught by its own self-test:** writing that record inside the jq pipeline (`.statusLine = $sl | .[$k] = (.statusLine // null)`) reads correct and is WRONG — jq evaluates left to right, so the first assignment has already landed and the "record" became our OWN line, leaving `remove` handing back a copy of the thing it was undoing. Captured in the shell and passed with `--argjson` now, with a companion case pinning that the record survives re-application. **The colour legend is generated output:** it had silently fallen short of one row per colour-declaring agent and nothing mechanical noticed, so it is now derived from frontmatter between a namespaced sentinel pair, with `--check` folded into `check-doc-currency.sh`. The honest limit is stated rather than papered over — the CSS table IS a map inside the generator; the claim is only that it is a *published standard keyed by hex*, so it cannot drift relative to frontmatter. Because every live hex is a CSS name the fallback is unreachable in production, so **two fixtures pin both halves**: a non-CSS hex must render `—`, and a CSS name from outside the set in use (`#FA8072` → `Salmon`) must still resolve — without the second, a subset implementation passes the first trivially. **The mutation control reaches across trees and mutates the CALLER**, since the suites live where CI's glob runs and the wiring does not: deleting the single invocation line from a copy of the gate must make it WRONGLY PASS a genuinely-drifted fixture, gated on the mutant being non-empty, different, `bash -n` clean and still reaching its own success line — because a syntax error also exits non-zero, so rc alone is no evidence. Vendor-coupling allowances were **measured**, never hand-typed, and the manifest's note is written *without* the literal token so declaring them could not trip the manifest's own allowance — the second-order trap its prior self-raise recorded. Counts unchanged. Full narrative in `CHANGELOG.md`.
