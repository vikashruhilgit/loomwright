...
**Latest change:** **the accumulated judgement, made browsable — never a scoreboard.** The Floor answered "what is happening now"; the larger asset was what the system had already *learned*, and it was readable only by opening files — the committed rules store is machine-readable and human-hostile, and the postmortem ledger is the single best record of *why* this system produces churn, with nothing displaying it. Three pieces ship, strictly additive over the existing projection contract. **The projector** extends the rules and postmortem surfaces with optional detail: rules gain tri-state optional fields decided by key **presence**, never truthiness — absent, present-with-`null`, and a real value are three distinguishable renders — and a supersession walk that DELIBERATELY DIVERGES from the routing reader's semantics (it hides nothing, resolves transitively and in order, and REPORTS a dangling pointer instead of ignoring it, because a curation-history browser needs the opposite answer from a routing reader) while inheriting that reader's one non-negotiable property regardless: every walk is the same bounded, non-recursive pass per node, so a malformed or cyclic pointer graph can never hang the real store. The postmortem detail publishes a class/flow-stage distribution over a basis that is PINNED and named as a literal field, because the ledger carries a second, disagreeing representation and publishing the wrong one silently would be the fabricated-single-truth defect this file has recorded before. An unparseable rules file is NAMED with its own reason, never silently folded into "clean". **A real regression was caught only by hand** after the detail passes shipped — redundant re-scans of the same two stores slowed the projector by several times over, fixed by consolidating onto shared passes, with every emitted number verified byte-identical to an independent out-of-band measurement. The runtime bound that now prevents a repeat is a **ratio against a real artefact** — the pre-fix commit itself, not a hand-built mutant that might represent nothing — because an absolute wall-clock ceiling is unusable in both directions (CI hardware differs from a laptop) and timing the small fixtures would go green straight through the very regression it exists to catch. **The bundle** adds a rules browser and a churn view reading only those projection fields, with the basis read off the projection rather than restated as a literal. Every correlation shown is a labelled observation carrying its evidence; nothing is ranked, scored, or sorted by desirability, and an uncomputable correlation is omitted rather than emitted as a zero that would read as "measured no violations". The same four absent/empty/unavailable/stale renders apply, each with its own fixture, and a new read-only assertion closes the gap the prior release's read-only check had left uncreated — no new timer, fetch, or endpoint. **The catch-up digest** gains an opt-in publish flag that emits its existing content a second time as a shareable, point-in-time snapshot, framed with its own generation time and an explicit disclaimer that it does not poll. **Byte-identity of the default path is proven mechanically, not by inspection:** the pre-flag script is pinned at a real commit and diffed against the post-change default output on identical fixture inputs, with a mutation control proving the diff itself can discriminate, and the one field that legitimately varies run-to-run — the wall-clock generation timestamp, which already varied before this change — is named and excluded rather than silently smoothed over. Absent the flag, no new code path executes, and the command doc is updated in the same commit that adds the flag. Full narrative and measured deltas in `CHANGELOG.md`.
