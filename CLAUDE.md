...
**Latest change:** the **citation-drift guard now polices every live `file.ext:<N>` in the repo**, not one line in one file. The previous release's guard was scoped to a single shape — `session-resume.sh:<N>` citations that also said "no-arg" on the same line — so the sibling instance in that same change, a comment citing a traversal reject ~30 lines off its real guard, was invisible to it. That guard is retired here into `scripts/test-citation-drift.sh`, which **found twelve already-rotted citations on its first run** (all corrected). Three checks, each claiming only what it can prove: **(A)** a line-count floor, kept because it is free but explicitly weak — it caught **none** of the motivating defects, which all cited a line that existed and merely said something else; **(B)** opt-in `[pins: <literal>]` anchors verified within ±3 lines, the check with teeth — auto-deriving anchors from prose was prototyped on the real corpus first and measured at 1 hit / 11 false alarms, so it was rejected in favour of an author-chosen literal that is false-positive-free by construction; and **(C)** a ratchet that fails any *new* bare `file:N`, pushing authors toward pins or descriptive anchors — the only form that cannot rot. `CHANGELOG.md` and `docs/SPIKES/` are excluded as frozen historical surfaces, and unresolvable citations skip rather than fail. Because the retired guard shipped **vacuous** (its citation and keyword sat on two lines, so its grep matched nothing), every check here is mutation-tested through the same code path as the real scan — which caught three bugs in the guard itself before it shipped, one of which silently truncated the scanned set instead of failing. Counts unchanged. Full narrative in `CHANGELOG.md`.
