...
**Version + counts:** authoritative in `plugin.json` (see `Plugin Layout` below for the per-directory breakdown) — not restated here. **Latest change:** `scripts/automate-helpers.sh`'s **`learning-emit` idempotency key is now degradation-aware**, closing a silent-invisibility defect (observed 2026-08-06 on PR #126). The key was `run_id`+item+`pr_url`+`source`, so a FIRST emit that degraded (the one `gh pr view` fetch failed, or the caller omitted the fetch args) wrote `changed_paths: []` — a line `read-postmortem.sh` can NEVER return — and then blocked every later emit that DID carry the data. The key gains a trailing `complete`\|`degraded` component, so the one newly-permitted append is **complete-after-degraded**; a degraded retry after a complete line and a duplicate complete line both still skip, and legacy four-component keys are matched by prefix. `learning-emit` still ALWAYS exits 0 and never gates, the ledger stays append-only, and `curate-postmortem.sh` remains the SOLE curator writer. The degraded line is left in place; because `repo`/`number` are derived from `pr_url` independently of the fetch that degrades, it shares the `repo`#`number` join key with its correction, so `build-loop-evidence.sh`'s postmortem join was changed from first-wins to **floor-raising** (max `review_rounds`, tie-break latest `ts`) — matching the discipline `measure-heal-signal.py` already applied — and can no longer report a stale low round count. Counts unchanged. See `CHANGELOG.md` v15.26.0.
