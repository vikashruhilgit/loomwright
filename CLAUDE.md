...
**Version + counts:** authoritative in `plugin.json` (see `Plugin Layout` below for the per-directory breakdown) — not restated here. **Latest change:** the subtask-id scheme is now pinned to plain numeric, 1-based, sequential (`1, 2, 3, …`) as an explicit rule at the producer (`loomwright/skills/supervisor-readiness/SKILL.md` §"Subtask Structure", mirrored in `loomwright/agents/launch-pad.md` and `loomwright/commands/launch-pad.md`), and a new `normalizeSubtaskIds` helper in `loomwright/sdk-spike/src/runner.ts` normalizes legacy alpha-suffixed ids (`1a`/`1b`) onto that same scheme via ONE shared id→id map built after the existing natural sort, rewriting both a subtask's own id and every `requires[].from` that references it — closing the "how arm 3 ended up with every dependency edge missing" failure mode while leaving the parser's tolerant accepted-input regexes unchanged. **Scope:** the producer rule applies to the DEFAULT path (every new brief is authored numeric); the normalizer is confined to the quarantined, UNCOUNTED `sdk-spike/` runner behind the opt-in `--sdk-runner` seam (default OFF), so the default Supervisor execution path is unchanged. See `loomwright/sdk-spike/src/runner.ts`'s `normalizeSubtaskIds` doc comment and `CHANGELOG.md` v15.23.0.
