...
**Latest change:** the curation cadence's three numbers now speak **one unit**. The previous release shipped a readiness count (*files newer than a wall-clock stamp*), a consumption window (*the N most-recent files*, `--sessions` default 5) and a record (*wall-clock now*) in three files with nothing asserting a relationship between them — so `pending=61 ready=yes` described a run that reads 5 and permanently retired the other 56. Four consequences, all in `scripts/curation-status.sh`: **(1)** `record dreaming` now takes the session ids actually consumed and `pending` is the set complement — a **watermark provably cannot** express "the newest N are done, the older tail is not", since /dreaming leaves behind the *older* tail and no value of T counts an older tail; **(2)** the count names the `window=` that will drain it, pinned to `commands/dreaming.md`'s Parameters table by a cross-file test; **(3)** `pending` counts logs carrying reflection **signal** (measured: 29 of 62 logs were pure `token_ledger`/`subtask_complete`, ~96% of corpus bytes, 0% of signal), with **different predicates for /dreaming and /insights** because the two consumers genuinely differ, both failing OPEN — `/insights` keeps its watermark, exact for it since it re-reads the whole corpus every run; **(4)** GATHER selects the most recent **unconsumed** logs via the new `unconsumed` subcommand, because plain mtime-recency made the tail permanently unreachable (measured 8→3→3→3; now 8→3→0). `record` also refuses to overwrite an unreadable state file rather than silently destroying the consumed history. Full narrative — including the zsh `producer | grep -q` census inversion and the mutant that exposed dead code rather than a missing test — in the top entry of `CHANGELOG.md`.
