...
|---|---|---|---|
| Launch Pad | user-facing | user | Phase 2.5 feasibility (GO/CAUTION/NO-GO); Phase 5.5 mandatory Plan Review (max 3 spawns per session); writes briefs to `.supervisor/jobs/pending/`. **Requirement-file input:** Phase 2 step 0 — when the `goal:`/`feature:`/`problem:` value is a path to an existing `.md` (resolves via `test -f`, typically a Beads-absent Product Owner story under `.supervisor/requirements/`), Launch Pad reads it as the requirement source; non-path values stay literal-string goals. Closes the PO→Launch Pad handoff gap in Beads-optional mode |
