...
**v15.9.0 — token ledger + insights Token economics + spawn-contract prompt-cache discipline (additive):** (1) **Fail-SAFE `token_ledger` SubagentStop emitter** (`scripts/emit-token-ledger.sh`) chained onto existing `send-telemetry.sh` hook lines (stdin fan-out — **22 hooks unchanged**; no new matcher entries). Prefers real usage when present; otherwise a transcript-byte proxy (`proxy:true`, never invents token counts). Authority: `docs/TELEMETRY.md` §Token ledger; self-test `scripts/test-token-ledger.sh`. (2) **`/insights` `## Token economics`:** advisory rollup of `token_ledger` JSONL (real vs proxy) after Cost; degrades when absent; dollars stay on ccusage. (3) **Spawn-contract stable-prefix reorder** in `skills/async-orchestration/SKILL.md` Part 2 (skill → 1.2.0) + durable **Stable-prefix-first spawn prompts** rule in `AGENT_GUIDELINES.md` — put stable prompt content before volatile interpolations so prompt-cache prefixes survive. **Counts UNCHANGED: 14 agents / 21 commands / 41 skills / 22 hooks; no `schema_version` bump.** Additive on top of v15.8.0.
...
> 📜 **Full release history** (v15.7.0 → v14.0.0 and earlier) lives in [`CHANGELOG.md`](CHANGELOG.md). CLAUDE.md keeps only the two most recent release notes.
...
- Plugin manifest: `loomwright/.claude-plugin/plugin.json` (v15.9.0)
