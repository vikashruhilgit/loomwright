...
**v14.28.0 — Memory APPLY path + `knowledge_sources_used` telemetry (Learning Loop Phase 1+2):** Wires the Agent Manager's *already-written* memory (verified LESSONS + `PROJECT_MEMORY` + per-agent memory) into Code Reviewer / Launch Pad / Supervisor as an **advisory, fail-safe, NON-gating APPLY path**, and adds one additive optional telemetry marker — without any new storage surface or any gate change. Code Reviewer gains an explicit bounded read-only memory-consult step at Context Setup (its own per-agent memory + `read-project-memory.sh`); Launch Pad (Phase 3 ANALYZE) and Supervisor (Phase 1 ACQUIRE) gain a sibling `read-lessons.sh` consult whose considered verified lessons may be **cited**. Every new read is **strictly subordinate to `CLAUDE.md`** (on conflict CLAUDE.md wins), fail-safe (reader always exits 0; absent/empty output ⇒ logged no-op), and can NEVER block a run or change a verdict / `heal_decision`. A new optional additive `knowledge_sources_used` array records the sources a run actually used — emitted both nested on `SUPERVISOR_RESULT` / `CODE_REVIEW_RESULT` and as a flat field on the Supervisor `session_end` JSONL line (the surface `/insights` reads); absent ⇒ valid (old logs unaffected). The memory-hierarchy precedence (`CLAUDE.md` > `PROJECT_MEMORY` > `LESSONS` > per-agent > brain/wiki) is documented in `AGENT_GUIDELINES.md`; the additive field is documented in `docs/RESULT_SCHEMAS.md`. **No new agent / command / skill / hook (counts unchanged at 14 agents / 18 commands / 55 skills / 19 hooks); no schema_version bumps** (`knowledge_sources_used` follows the `branch_base` / `pr_state` / `preflight_sync` additive precedent). Additive on top of v14.27.0.
...
- Plugin manifest: `ai-agent-manager-plugin/.claude-plugin/plugin.json` (v14.29.0)
