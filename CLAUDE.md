...
**Latest change:** **the harvester proposed a rule the store already had.** `harvest-conventions.sh` prints `rules store (read for context, NEVER written)` and it did read it — for context. It never compared its **proposals** against it. On a real `/dreaming` run, proposal 1 of the batch (theme `restated-count-version`, 36 motivating findings) restated a rule `.agent/rules/process.json` has carried since 2026-08-03; `/dreaming` is the batch's ONLY consumer and would have offered the duplicate for Accept, putting two differently-worded copies of one convention in one file — what the `process` cross-surface-sync rule prohibits. Caught by hand; nothing mechanical caught it. **Exact-string matching could not have caught it either** — one claim, two wordings, no shared sentence — so the new **store dedupe pass** reuses the distinctive-term overlap the `pw=` project-wide test already uses, through **one shared `term_overlap`** (the "same approach" claim is only true if there is nothing to diverge; the pw refactor is verified byte-identical on the real corpus). The threshold is justified by a **measured gap, not a number**: the true duplicate scored 63% against the live rule while the four other emitted statements scored 17/16/8/0%. A match is an **already-covered DEFERRAL naming the covering rule id and the overlap** — reported exactly as a corroborating corpus entry is, never silently dropped — and it **spends no cap slot**; every emitted proposal prints the nearest live rule it did NOT match, so the floor is falsifiable from the report in both directions. **The deferred theme's findings stay in every denominator:** coverage still measures against them (a deferral LOWERS coverage rather than shrinking what it is measured against), the theme is itemised in the unmapped remainder with `already covered` as its stated reason, and new `already covered by the live store` + **COMBINED convention coverage** figures show what the store already answers. Two downstream branches moved with it — the batch listing now prints whenever it holds anything (an all-covered run emits 0 rules, and a `cat` behind an `$EMITTED` test was skipping the very deferral blocks the diagnostic said were "named above"), and the empty-batch diagnostic gained a third **measured** cause. **Honest limits, in the script's header:** the check is LEXICAL, not semantic — it can miss a duplicate in wholly different vocabulary — and advisory, never a gate; an unparseable rule file is fail-safe but NAMED, with the pass reported UNVERIFIED against it rather than silently clean; `docs/HARVEST_DRYRUN_SAMPLE.md` is a pinned transcript that PREDATES the pass and now says so instead of being regenerated. All three mechanisms mutation-verified (removing the pass re-proposes the duplicate; crediting deferred findings to the batch flatters coverage to 100%; making the already-covered continuation line unreachable loses the second cause from a combined empty batch — a REVIEW FINDING: that line was reachable and correct and had never been EXECUTED, since every rule-seeding fixture ran at an unexhausted cap and the only `--cap 0` fixture seeded no rule), with the fixture's live rule extracted from the script rather than hand-typed. Counts unchanged. Full narrative in `CHANGELOG.md`.
