...
**Version + counts:** authoritative in `plugin.json` (see `Plugin Layout` below for the per-directory breakdown) — not restated here. **Latest change:** project memory gains a **correction path** — `write-project-memory.sh --retract <id>` / `--fact "<corrected>" --supersedes <id>`. A hand edit was never an option (the read gate hashes the fact text, so an edited line silently stops matching its `add` and is DROPPED without a trace), so `--retract` now deletes the line AND appends a chain-valid `retract` provenance entry that `read-project-memory.sh` honors by revoking that `content_hash`; `--supersedes` commits the correction and the retraction atomically. A later `add` of the same text re-trusts it — a retract is a point-in-time revocation, not a blocklist. **Two defects in that new path are closed in the same release:** (1) the dedup guard's `&& [ -z "$RETRACT_ID" ]` disabled the presence CHECK itself rather than only its early exit, so a `--supersedes` whose replacement text already existed elsewhere appended a **duplicate line**; presence is now one `fact_present` boolean gating only the ADD half (bare `--fact` short-circuits exactly as before; a supersede retracts and skips the duplicate append plus its spurious `add` entry — skipping an `add` cannot orphan the chain, since `append_prov` chains off the current tail). (2) a `--supersedes` whose replacement was **byte-identical** to the target DELETED the fact outright (identical lines ⇒ `grep -vxF` removes both; one shared `content_hash` ⇒ the retract revokes what the add just trusted) while printing "stored" and "retracted" and exiting 0 — silent data loss; that call is not a correction at all and now **fails CLOSED (exit 2) before any temp state exists**, leaving the fact present and readable. **Three follow-up hardening items close in the same release:** the id gate now matches its own "validated strictly" comment (**exactly 8 lowercase hex chars**, stated positively — the old `*[!0-9a-f]*` accepted any length, so `--retract a` died later at the lookup with a misleading "no memory entry"); `--supersedes` given **without** a replacement `--fact` no longer silently degrades to a plain retraction but **fails CLOSED (exit 2) at validation time, state byte-identical** — a supersede with no replacement is an indistinguishable synonym for retract, which is exactly how the sibling store `write-lessons.sh:125` already rejected it, so the two mirrored stores now agree deliberately (the caller's *spelling* is tracked in a separate `SUPERSEDES_USED` flag so `RETRACT_ID` and `--retract` — silent, exit 0 — are byte-identical to before; argument-shape errors are ordered ahead of every state lookup, so `--supersedes <unknown-id>` reports the missing replacement and the abort precedes every `mktemp`); and the unknown-id abort gains its missing ADD-half branch (`--fact "<new>" --supersedes <unknown-id>` ⇒ exit 2, replacement **not** written, both store files byte-identical, no temp files, reader unchanged). **Finally, one CLASS is closed at the root:** every match on the entry shape `- [<id>] <text>` in the writer was an **unanchored `grep -F`**, so a match could land in the MIDDLE of another entry's free text — reachable, because fact text is arbitrary prose and this repo's own `PROJECT_MEMORY.md` quotes bracketed ids inline. It cost a brand-new fact whose rendered line was a *substring* of an existing entry (discarded as "already present", exit 0) and made `--retract` delete the **wrong line** when a decoy quoting the target id sorted first (`grep -m1` captured the decoy; provenance still recorded the real id — state and provenance disagreed). Dedup is now whole-line **`grep -qxF`**; the retract lookup is an **anchored ERE** `^- \[$RETRACT_ID\] `, safe *only because* the 8-lowercase-hex id gate admits no regex metacharacter — an interlock now stated at the gate. Every other entry-shape match in the writer was audited (all already anchored), and `read-project-memory.sh`'s two trusted-set matches are already `-x`, so the class does not reach the reader. Also folded in: both `--retract` arms now clear `SUPERSEDES_USED` (last-flag-wins for the *spelling*, as it already was for the value), and the id-gate comment no longer claims a `sed` address that does not exist. `test-project-memory.sh` grows a group 9 plus these branches and a group 10 for the anchoring class (22 → 47 → 64 → 79 assertions); mutation control against the pre-fix writer: 39 passed / 8 failed, and each fix reverted in turn fails only its own assertions (45/2, 40/7, 46/1, 61/3, 61/3, 63/1) — reverting the bare-`--supersedes` abort to an interim warn-and-succeed path fails 15 (64/15). Counts unchanged. See `CHANGELOG.md` v15.27.0.
