...
**v14.39.0 — fix the `until_mergeable_dispatched` observability false-negative + the 0-byte dispatch-log ambiguity (additive):** The until-mergeable drain's observability signal could LIE about its own dispatch on the inline path. `until_mergeable_dispatched` was authored from the Supervisor's own "did I run Phase 4.5 step 5.5" knowledge and never reconciled against the actual dispatch artifact — so when the inline Supervisor skipped step 5.5 but the `PostToolUse[Bash]` hook backstop dispatched the drain (out-of-band, invisible to the main thread), the field recorded `false` while the drain was live, making a real run misreport "not dispatched." Compounding it, the detached `claude -p` drain buffers stdout until exit, so its `RUN_LOG` is 0 bytes while in flight — indistinguishable from a failed no-op. **Fix 1 (dispatcher):** `dispatch-pr-review.sh` now writes a synchronous, non-empty, self-documenting `RUN_LOG` header — a machine-greppable `DISPATCHED` token + `url=` + `until_mergeable=` flag + `runner=`, plus a separate `# marker:` line and a plain-language note that an empty body below means IN FLIGHT, not failed — BEFORE the detached launch, so an in-flight drain is never an ambiguous 0-byte file. **Fix 2 (Supervisor, both prompts):** `until_mergeable_dispatched` is now RECONCILED against the on-disk per-PR dispatch marker (match the marker body for the exact PR URL — hash-implementation-agnostic and prefix-safe), NOT keyed on whether step 5.5 itself ran. A marker exists IFF the dispatcher ran for this PR via EITHER step 5.5 OR the hook backstop, so `true` is truthful regardless of which path fired and `false` is recorded only when no marker exists. The inline `commands/supervisor.md` — which had NO recording directive at all — gains the same marker-reconciliation, mirrored with `agents/supervisor.md` Phase 4.5 step 5.5 as a sync pair. **Fix 3 (schema):** `docs/RESULT_SCHEMAS.md` clarifies the field reflects the marker (hook OR step 5.5), not the agent's own action. Self-test `test-dispatch-pr-review.sh` grows 18 → 20 (real-launch stubbed-`claude` header present + greppable; opt-out header records `until_mergeable=0`). All fail-safe invariants preserved (the header write + reconciliation are best-effort, never gate, never change `heal_decision`, never block the PR/run). **No new agent/command/skill/hook; counts unchanged at 14 agents / 18 commands / 55 skills / 20 hooks; no `schema_version` bump.** Additive on top of v14.38.0.

> 📜 **Full release history** (v14.38.0 → v14.0.0 and earlier) lives in [`CHANGELOG.md`](CHANGELOG.md). CLAUDE.md keeps only the two most recent release notes.
...
- Plugin manifest: `ai-agent-manager-plugin/.claude-plugin/plugin.json` (v14.40.0)
