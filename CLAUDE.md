...
**Latest change:** **the graphify/bridge enrichment tier is retired — the staleness trap its own north-star doc warned about is the one it fell into.** The measurement that settled it: `brain_context` appears zero times across all recorded session logs (not one run ever consumed the tier), the graph was built once and never rebuilt, the bridge survived only as a long-stale orphan its reader emitted nothing against, and the spike's headline gate score has no control arm (every strict hit is anchored by a shared file a plain path join would also have found). The tier is removed at every seam — the `brain-context` ladder now reads **memos → repo-map → nothing**, and each removed consult was already a self-gating no-op with the graph absent, so every caller's success path is byte-identical; that fallback is **traced per seam, not assumed**, and the same trace covers the distilled-store "flag-only, never auto-delete" surfaces, which were checked and correctly left standing because under the new rule they are the "never" row and stay true. This is a **user-facing removal**: the `/setup` twin module (cold-start graph + bridge + starter-CLAUDE.md bootstrap) is gone, and the external-brain `LOOMWRIGHT_BRAIN_ROOT` path retires with the rung — the zero-uses measurement covers the whole rung and the wiki path had no separate consumer. A measured correction recorded rather than absorbed: `scripts/twin-graph.sh` and its test are **RETAINED, byte-untouched** — they serve the live System Twin contract store (`.supervisor/twin/`), not the retired tier the source requirement's list lumped them into. D9's blanket no-auto-delete rule becomes a **regenerability split** — Derived artifacts (rebuildable from the repo) may be auto-deleted freely, Distilled stores (human-gated judgment) never — amended in `docs/SPIKES/FINAL_STATE_GOAL.md` with the original sentence quoted and the amendment dated. And the reversal condition is citable from a fresh clone, not from anyone's scratch directory: the validation harness is committed verbatim as a runnable closure under `docs/SPIKES/code-graph-harness/`, any revival — graph or external brain — must be a cheap incremental refresh that passes it, and the spike evidence docs are banner-marked RETIRED, never deleted. Counts unchanged. Full narrative in `CHANGELOG.md`.
