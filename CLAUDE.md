...
**Latest change:** **a lifecycle step only a living agent could take, and the probe that lied about the wreckage.** The brief move `in-progress/` → `done/` is completion-tail step 2 and the requirement close-out is step 2.5 — both PROMPT-INSTRUCTED, executed by the agent, not by code. When the Phase 4.5 reviewer died 4× with a server error the tail was never reached, and a brief whose PR had merged and shipped sat in `in-progress/` indefinitely. **No prompt change fixes that** — an instruction cannot be made atomic against the agent dying before it is read — so the tail is deliberately left ALONE and the missing reconciliation is added instead — the **second** such reconciler, not the first: `stamp-requirement-status.sh` has closed out step 2.5 since 2026-08-04 on the same argument, but reads `jobs/done/` EXCLUSIVELY, so a stranded brief structurally blocks it from firing at all. Repairing the move unblocks it; the two compose without overlap. `loomwright/scripts/reconcile-jobs.sh` classifies each in-progress brief `stranded_merged` / `stranded_closed` / `unknown` and finishes the move under `--repair`. It is **offline by construction** (never calls the forge CLI — a SessionStart hook consumes it on every resume) and **never repairs `unknown`**: a stranded brief carries no PR URL, because the `## Outcome` block that would carry one is exactly what did not get written. `session-resume.sh` stops asserting — Stranded (with the repair command) and UNVERIFIED are separate sections, and the resume hint is suppressed unless something is genuinely unverified; an absent reconciler falls back to a neutral listing, never to the old claim. **A third defect, found while scoping — real but narrower than first framed (it affects the PROMPT path only; the mechanized reconciler deliberately writes `brief-shipped`, never `done`):** step 2.5 stamped the status in a bullet while `is_done()` reads only the `## Status:` heading, so a SUCCESSFULLY closed-out requirement stayed re-enqueueable forever — never observed because step 2.5 had never once completed here (zero sentinels on disk). `done_with_escalation` was silently re-pickable too (`\b` does not match before `_`). **Two of this change's own tests were vacuous and are recorded, not quietly fixed:** one tried to hide an absolutely-pathed script with `PATH`; the other hand-typed the contract format it was meant to detect drift in — the same defect the previous release found, one release later. Fixtures are now extracted from the skill, with a control asserting the extractor found anything at all. All five mechanisms mutation-verified. **Honest limit, stated in the script's own header:** the tail is unchanged, so strands still happen — this repairs them, it does not prevent them. Counts unchanged. Full narrative in `CHANGELOG.md`.
