...
**Latest change:** **the floor in one command, and one floor for every project.** The view was reachable only the long way — starting and stopping it lived behind a *configuration* command, so daily operations went through a configuration dashboard and in practice pushed the owner back to running the engine by hand in a terminal, which is the exact friction the module existed to remove. A direct command now owns the operational half plus a project registry, while install and uninstall stay with the configuration command — the two verbs that materialise and delete a directory of files outside the repo, both marker-gated behind an explicit confirm. **The split is asserted, not merely described:** the two files' documented verb sets are parsed and required to UNION to exactly the engine's own dispatch table, so a verb the engine gains and neither file documents FAILS. The engine's set is parsed out of the dispatch `case` itself — pinned, because the file has several and reading the wrong one silently yields a nonsense set, and read from the ARM LABELS rather than the fallback arm's error string, which is a second in-file enumeration that can drift on its own. **The obvious mutation control provably does not work and is documented as such:** deleting one of the module verbs from the new command's doc leaves the union unchanged, because the configuration command already documents all five — so the controls used are deleting a verb documented ONLY in the new file, and appending an undocumented verb to a scratch copy of the engine's dispatch. **One server on one port now shows any registered project, chosen in the page** rather than by which directory a terminal was launched in. A project appears **only because a human added it**; the registry lives BESIDE the ui directory so a module teardown cannot take the project list with it, and the drop-a-project verb is deliberately not named the same as the tear-down verb — one word meaning both is a data-loss shape — with the project tree hashed before and after to prove it touches nothing. The selected project regenerates on the configured interval and the others one at a time on a slower cadence, because one projector run costs about a second and regenerating all of them every tick starves the loop at two projects: measured against a stub projector of known cost, with a control proving the naive shape really is over budget. Per project, ready / never-regenerated / unavailable / unreadable stay four distinct renders, and the registry's own *absent* and *unparseable* stay two different sentences. Selecting a project **sends nothing** — it changes which already-written file the page fetches — and the page is still loopback-only with no write verb of any kind. The release-surface parity group gained the new command's registration rows and a residue sweep for the superseded command count, and the reference doc's exhaustive write list now names the served index, so that section stays exhaustive with a mechanical subject rather than a promise. Full narrative in `CHANGELOG.md`.
