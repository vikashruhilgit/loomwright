...
**v14.13.0 — Clickable desktop notifications (opt-in, macOS):** Makes the plugin's pause-event desktop banner **clickable** when [`terminal-notifier`](https://github.com/julienXX/terminal-notifier) is installed — a click brings the **Claude desktop app to the foreground** (default `activate` mode; for a single active session that lands you back on it). A new **self-tested pure helper** `scripts/notify-click-target.sh` (tested by `scripts/test-notify-click-target.sh`; neither counted by the doc-currency gate) decides the click action and builds an opt-in `claude://claude.ai/resume?session=<uuid>` deep link (reverse-engineered from `Claude.app`: host `claude.ai`, path `resume`, UUID-validated `session` param → `importCliSession`). `notify-desktop.sh` prefers `terminal-notifier` — calls are **detached** so the hook never blocks (this host has no `timeout`/`gtimeout`), `-group`-coalesced, and **omit `-sender`** — and falls back to the existing `osascript` banner when it is absent, so behavior is **byte-identical (zero regression)** without `terminal-notifier`. **New env var** `AI_AGENT_MANAGER_NOTIFY_CLICK` (`activate` (default) | `resume` | `auto` | `off`). **macOS 26 caveats** (verified live): impersonating Claude's bundle id via `-sender` is silently dropped by the notification service (so it is unused — the banner shows terminal-notifier's icon), and `terminal-notifier`'s `-open` click callback no longer fires (deprecated `NSUserNotification` delegate), so `resume` can't navigate from a *click* there — `activate` is the reliable default; the deep-link route itself is valid (reachable via a direct `open`). True per-session navigation from a click on macOS 26 would need a custom signed helper app (out of scope). **No new agent / command / skill / hook (still 13 / 15 / 50 / 19)** and **no schema change.** Additive on top of v14.12.0.
...
> 📜 **Full release history** (v14.11.0 → v14.0.0 and earlier) lives in [`CHANGELOG.md`](CHANGELOG.md). CLAUDE.md keeps only the two most recent release notes.
...
- Plugin manifest: `ai-agent-manager-plugin/.claude-plugin/plugin.json` (v14.13.0)
