...
**Version + counts:** authoritative in `plugin.json` (see `Plugin Layout` below for the per-directory breakdown) — not restated here. **Latest change:** the findings ledger (`.supervisor/postmortem/results.jsonl`) becomes a **committed, GATED third store** under `/setup memory`, so the churn corpus travels with the repo. It could not simply be un-ignored: the ledger is **structurally cross-repo** (a `/pr-postmortem` append lands in the CURRENT working `.supervisor/`, never the analysed repo's), so this plugin's own held 7 `vendsy/hub` records among 88, and a naive add would have generalised that contamination into every user's repo. Four decisions carry the design: `apply` fails closed in the **WRITE** dimension only (refuse + named-slug status line, still `exit 0` — the helper's FAIL-SAFE CONTRACT forbids a non-zero exit); a **third `gated` verdict class** exists because folding the ledger into the unconditional intended set made a *correct* refusal self-report as `not configured` and emit remediation pointing at the module's own `.supervisor/*` line; **emitting** a negation needs an affirmative examined pass while **withdrawing** one needs an affirmative examined refuse, so every could-not-examine outcome PRESERVES rather than silently reverting on a diagnosis never made; and every ledger assertion is **jq-only**, because the corpus mixes compact and spaced JSON and a compact grep both under-counts and lets a spaced-form foreign record through. Suites 198 → 295 and 32 → 37. Full narrative — including the three fail-opens review and CI found, and the allowlist-does-not-travel limit — in `CHANGELOG.md` v15.28.0.
