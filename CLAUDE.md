...
**Latest change:** **the sole writer ate the blank line between sections.** `write-lessons.sh` rebuilds the target `## <category>` section with awk, and the `sec_start > 0` arm — a SECOND entry into an EXISTING category — reconstructed it from its `^- \[` lines **alone**. `sec_end` is the line immediately before the next `## ` heading, which in this format is the **blank separator**: it matched neither the entry loop nor the remainder loop (which resumes at `sec_end + 1`, the heading itself), so it was dropped on every write into a category that is not last in the file. The asymmetry was real, not stylistic — the sibling `sec_start == 0` arm prints `""` explicitly before a brand-new heading, so the two arms of one rebuild disagreed on the format the file is in. **Deterministic and compounding:** once a separator is gone it never comes back, so the committed `.supervisor/memory/LESSONS.md` eroded a little further every `/dreaming` session. The rebuild now **replays the section in place** — `idx[]` remembers each entry's original line number, survivors print where they were, the new entry prints immediately after the last pre-existing one, and every non-entry line is carried through — leaving the front-first eviction (one `evictfile` id per evicted entry), the supersede path, and a genuinely-last section untouched. **The suite gap was precise, which is why this survived:** the existing "two categories independent" case creates two FRESH categories, so BOTH of its writes take the `sec_start == 0` arm and the buggy arm was never executed; no test in the file mentioned "blank" or "separator". The new case adds a second entry into a non-last category and asserts the separator survives, with the control that one existed beforehand (without it the assertion could pass against a file that never had one) and the counter-case that a genuinely-last section gains **no spurious trailing blank** — restoring symmetry must not become over-correction. Its in-file **mutation control** deletes the carry-through arm, which reverts the rebuild to byte-equivalent old output, and is itself gated on non-empty + differs-from-original + `bash -n` + the mutant having actually written its fixtures, per the `[fa32a308]` lesson that a mutant is evidence only if it is valid; reverting the whole hunk externally flips that one assertion and no other. **The already-damaged store is repaired whitespace-only:** LESSONS.md is a sole-writer store whose provenance index keys on a content hash, so a hand-edited entry line would leave `read-lessons.sh` dropping it **silently** — blank lines are not entries, and every entry verifiably still reads back. **Honest limits:** this is cosmetic, not a correctness defect — the reader always parsed the file correctly without the separators, and all three affected entries still read back before the repair. The store this landed against moved mid-change (the `/dreaming` write PR merged while this was in flight, taking the damage from one instance to three), which is itself the compounding argument: the repair is a re-runnable whitespace pass, not a hand-edit, precisely so a store that keeps moving can be repaired again. Counts unchanged. Full narrative in `CHANGELOG.md`.
