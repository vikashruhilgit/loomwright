...
**Latest change:** `applies_to` **stops being a comment and becomes a real path filter.** `read-rules.sh` accepted the touched paths as positional args and DISCARDED them — every rule reached every agent on every task, which makes a rule that fires everywhere indistinguishable from one that fires nowhere. It now routes: a new shell function **`rule_applies`** matches each touched path against a rule's globs with a native bash `case`, and `add-rule.sh` gains a repeatable **`--applies-to <glob>`** so the scope can be authored. Three things shape it. **Everything ambiguous fails OPEN** — a malformed `applies_to` (non-array / non-string element / empty array) *and* a **zero-arg call**: `scripts/session-resume.sh`'s `rules_nudge()` calls the reader with no arguments and fires its "no house rules found" nudge when stdout is EMPTY, so treating an empty path set as "nothing matches" would make every repo that HAS rules start nudging as if it had none — a silent regression in a firing surface this file calls pinned. Zero args is an absence of scope, not a negative match. **Order is fixed at `validate/dedup → supersession → routing`**: supersession is a property of the STORE (identical every call), routing of the CALL, so running routing first would let a RETIRED rule RESURRECT on any call whose path set routed its replacement away. **Matching is a `case` glob in the SHELL, never in `jq`** — portable by construction (no `shopt`, no flavour divergence), at the documented cost that `*` and `**` are equivalent and BOTH cross `/`; this is explicitly NOT `.gitignore` syntax and `.gitignore` is not emulated. Because a test asserting only "the rule IS emitted" passes identically against the old no-op reader, the non-match case asserts **absence from stdout**, and a **mutation control** (the routing predicate replaced by `true`) was run and observed to make that assertion fail. `rules-check.sh` deliberately stays repo-wide and takes no path scope — routing is an emission filter for advisory injection, `/rules check` is an audit, and a routed-out rule has not stopped being true. Counts unchanged. Full narrative in `CHANGELOG.md`.
