...
**v14.34.0 — `PostToolUse[Bash]` PR-create backstop for the until-mergeable review drain (additive):** Closes the inline-execution gap where the until-mergeable drain (v14.32.0) is triggered ONLY by Supervisor Phase 4.5 prompt **step 5.5** — agent-prompt logic, not a hook — so on the inline `/autonomous`/`/supervisor` path (where the Supervisor IS the main thread and `SubagentStop(supervisor-runner)` never fires) the drain silently never dispatches if the agent skips that prose step. A new fail-safe `PostToolUse[Bash]` hook (`scripts/hook-dispatch-on-pr-create.sh`) fires on the actual `gh pr create` Bash tool call and **reuses the existing `dispatch-pr-review.sh`** (no new dispatch logic) — the agent cannot "forget" it. The hook is **session-scope gated** so it never hijacks unrelated manual PRs: it dispatches ONLY when ALL THREE durable-lifecycle terms hold — an in-progress job exists AND `.supervisor/state.md` Status is not completed/failed AND the PR's head branch matches the active session's feature branch (gate consumes EXISTING unskippable Phase 1 ACQUIRE writes, NOT a new prompt-written marker that would reintroduce the very step-5.5 fragility). It is **fail-safe (always exits 0)** — a PostToolUse hook must never break the originating Bash call — and the dispatcher's per-PR **idempotency marker** guarantees exactly ONE dispatch if BOTH the hook and step 5.5 fire. Counts now **14 agents / 18 commands / 55 skills / 20 hooks**; additive, no `schema_version` bump; ships +1 fixture + self-test (both uncounted `test-*`/fixture scripts). Additive on top of v14.33.0.
...
- Plugin manifest: `ai-agent-manager-plugin/.claude-plugin/plugin.json` (v14.34.0)
