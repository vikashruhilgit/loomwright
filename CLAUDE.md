...
**Latest change:** **every write to a curated store is now validated before it lands.** One shared helper, `scripts/validate-entry.sh`, carries five checks (duplicate, contradiction, provenance, dead reference, cross-repo reference) and is sourced by all six sole writers — including the new sixth, `scripts/write-agent-memory.sh`, which **rebuilds** each agent store's `MEMORY.md` from the files on disk on every write instead of maintaining it, so an entry can only be missing from the index if it is also missing from the directory. A broken helper degrades into a **named refusal**, never a crash and never a silent unvalidated append: the load guard requires the source to succeed, all five functions to be present, and a sentinel to match, because bash defines every function above a syntax error before aborting the parse, so a truncated helper would otherwise pass a one-function probe. `|| true` is forbidden on that source line. Three things about this run are worth carrying forward. **The cross-repo check's direction was inverted in the source requirement** — it said to derive the check from the allowlist's "known foreign slugs", but the allowlist is this repo's OWN keep-list, so a literal implementation would have refused citations of this repo and passed genuinely foreign ones, going green on every criterion while behaving backwards; caught at Plan Review, before any code. **The first wired implementation falsely refused 12 of 21 live curated entries (57%), every one a false positive** — resolvable-only-under-the-plugin-dir paths, glob- and tilde-shaped paths that must skip rather than refuse, and six ordinary English prose pairs read as `owner/repo` slugs. Hand-written fixtures were all green; only replaying the real corpus found it, so that replay now ships as a permanent regression test, and for the two over-recognising checks the bias is stated in the code: **a false refusal is worse than a miss**, because it blocks a legitimate write. And a **residual bound is stated rather than hidden**: a bare all-lowercase `word/word` whose owner is not a known owner (`otherco/othersvc`) is a deliberate miss — it is shape-identical to `budget/zone`, and refusing that shape is exactly what refused six live entries — so the check recognises only *marked* slugs, a paired test pins both directions, and **the cross-repo check is nowhere described as complete coverage**. Agent memory also gained a stated capability contract: `memory: project` agents propose, never write, on a surprise-only trigger, said in the root `AGENT_GUIDELINES.md` and all six prompts under one identical heading — stated in prose because the tool surface gives the wrong answer (Bash is unrestricted by the harness, so a store write is reachable regardless of `disallowedTools`), and asserted by a test that matches the rule text, not the heading. **Then two of those five checks stopped refusing.** `dead-reference` and `cross-repo` are now **ADVISORY** — they report on stderr and always return 0 — while `duplicate`, `contradiction` and `provenance` are unchanged and still refuse. This **contradicts scope 2 of the source requirement** ("a validation failure is a refusal to write") and is an owner decision taken on measurement, so it is stated rather than softened: **six consecutive review rounds found false refusals and every one was in those same two checks.** That is structural — the three blocking checks key on input whose shape the writer controls, the two advisory ones scan free English prose for paths and repo names, where a citation and an ordinary sentence are frequently the same characters. The last round decided it: the fix for "gitignored but real files are unresolvable" made the **verdict machine-dependent**, green on the author's working tree (where `.supervisor/state.md` exists) and red on every clean checkout, and the corpus-replay guard could not see it because it replayed the working tree. A gate whose answer depends on which files are lying around is not a gate. What is sold, plainly: a genuinely dead reference or foreign citation is now **written, with a warning**, and only a human acting on that warning will fix it — so the reporting is the load-bearing half. Every finding prints a greppable `ADVISORY:` marker carrying the same detail the refusal did (no advisory token begins with `REFUSE_`), and all six writers surface it again at the call site. `validate_entry_all` absorbs advisory statuses rather than trusting two long functions never to regress, the contract sentinel bumped because "rc 0" no longer means "all five passed", and the corpus replay now resolves paths against a **`git archive` extraction** — the method that exposed the defect — with a constructed probe, because deriving one from whatever gitignored files exist is that same defect one level up. **Then three of those same writers turned out to accept flags they never implemented, and write anyway.** `write-lessons.sh`, `write-project-memory.sh` and `write-system-contract.sh` ended their argument loop in a catch-all that silently dropped every unmatched argument and reported success. All three derive their store from the **current directory**, never from anything the caller passes, so a store-redirecting argument parses, is discarded, and the write lands in whatever repo the caller was standing in. Found the hard way: a review reproduction passed `--repo <temp repo>` believing it isolated the store, and a junk entry landed in this repo's real `LESSONS.md` and provenance chain, caught only by a later `git status`. The reach is ordinary — **`--repo` and `--store` are REAL flags on `write-agent-memory.sh` and `add-orientation.sh`**, so reaching for one on a sibling writer is house vocabulary, not a typo. All three now refuse with exit 2, naming the argument and themselves, writing nothing; `write-lessons.sh` also refuses stray positionals (`add` takes none, the curation verbs take exactly two) with its documented positional forms unchanged. **Do not harmonise these parsers with `validate-entry.sh`'s `_ve_parse_args`, which ignores unknown flags on purpose** — right for a validator on a fail-safe path, wrong for a writer, where the cost of silence is the store rather than a diagnostic; the contrast is stated at all three guards. **No `--repo` flag was added**: rejecting the unknown and adding a real store-root flag are separate decisions, and adding one would make the ignored-argument class invisible again. The other three writers already refused but nothing asserted it; all six now have a test, and two mutation controls caught defects in the tests rather than the code — stripping only the flag arm left the mutant refusing because the dropped flag's VALUE hit the positional arm, and a softened guard DROPS a stray flag rather than promoting it, so the stray must follow a complete positional set or the mutant dies on the usage check instead of writing. Counts unchanged. Full narrative in `CHANGELOG.md`.
