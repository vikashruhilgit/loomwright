...
**v15.10.0 — `check-token-budget.sh`: CI-enforced per-agent prompt-inventory budgets (additive):** (1) **NEW `scripts/check-token-budget.sh`** (repo-root CI validator) — for each `loomwright/agents/*.md`, computes the OFFLINE PROXY weight (`bytes/4`) of the agent prompt PLUS every frontmatter-preloaded skill's `SKILL.md` (frontmatter-bounded parsing), compares to a declared per-agent budget, prints a per-agent proxy report, and **fails CLOSED (exit 1)** on any breach / missing preloaded `SKILL.md` / undeclared budget. Deterministic, offline, bash-3.2 safe. It is a **PROXY on prompt-inventory growth, NOT live tokenizer inflation** — never calls `count_tokens`; every number is labeled "proxy tokens." (2) **NEW `loomwright/docs/prompt-token-budgets.json`** — the single authoritative budget source the gate reads (keyed by agent filename stem; `proxy_bytes_per_token`, per-agent `budget`/`measured`/`note`, and the raise rule: raise in the SAME PR that breaches, with a one-line `note`, visible in diff). Initial budgets = measured + ~10% headroom (a ratchet, not a diet). (3) **`ARCHITECTURE_CONTRACTS.md` §"Prompt Token Budgets"** mirrors the JSON for humans. (4) **NEW `scripts/test-check-token-budget.sh`** — hermetic offline self-test (pass / breach / missing-preloaded-skill / no-budget / frontmatter-bounded / empty-agents-dir / inline-flow-style-skills / orphaned-budget / live-repo cases) via `TOKEN_BUDGET_*` env overrides; the gate also fails CLOSED on an inline/flow-style `skills:` list and on an orphaned budget entry. (5) **Wired into `.github/workflows/ci.yml`** as its own self-test-then-gate step (repo-root `scripts/test-*.sh` is not covered by the plugin-internal `loomwright/scripts/test-*.sh` loop). **Counts UNCHANGED: 14 agents / 21 commands / 41 skills / 22 hooks; no `schema_version` bump.** Additive on top of v15.9.0.

> 📜 **Full release history** (v15.9.0 → v14.0.0 and earlier) lives in [`CHANGELOG.md`](CHANGELOG.md). CLAUDE.md keeps only the two most recent release notes.
...
- Plugin manifest: `loomwright/.claude-plugin/plugin.json` (v15.11.0)
