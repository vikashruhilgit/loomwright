...
**Loomwright** is a Claude Code plugin with 12 agent roles (7 user-facing + 5 internal) for plan-first readiness, parallel execution, requirements, planning, code review, commits, adversarial audits, and standalone PR review-and-heal (dual-agent QA moved to the selvedge@atelier companion plugin). Supervisor and Launch Pad use `.supervisor/` exclusively for state; Orchestrator and Product Owner can optionally use Beads.
...
**Latest change:** **a fourth plugin, `selvedge`, is registered and deliberately empty — and the three loomwright-pinned CI gates now discover plugins instead of hard-coding one.** The scaffold ships `plugin.json` + `README.md` and nothing else: `agents/`, `skills/`, and `hooks/` arrive **populated, atomically**, in a later slice, because creating them empty is not neutral — `check-shared-prefix.sh` treats an empty agents dir as a false green and `check-skills-index-sync.sh` fails loudly on a `skills/` with no index. `check-token-budget.sh`, `check-shared-prefix.sh`, and `check-contract-parity.sh` now resolve plugins through `.claude-plugin/marketplace.json`, layered **under** their existing per-gate env overrides so every prior caller is byte-identical, and each keeps two branches that are **different on purpose**: *skip silently* when a plugin ships no relevant tree, *fail loudly* when the tree exists but is malformed (agents present with no budgets file of its own). Budgets are **per plugin**; the shared-prefix canonical stays single and loomwright-owned, and that cross-plugin dependency is now stated in the script header **and enforced in code** — a manifest without loomwright fails loudly rather than falling back to a second copy. Two measured corrections recorded rather than absorbed: `.source` values resolve against the manifest's **grandparent** dir (a single `dirname` yields `.claude-plugin/`, which contains no plugin), and the `MANIFEST` row format **gained no column** — a second out-of-tree consumer re-parses that same heredoc with a four-column read, so the plugin dimension rides inside the already-namespaced matcher instead, verified by diffing that consumer's output before and after. CI's hard-gate loop moved out of the workflow file into a harness with its own self-test, because loop logic inlined in YAML can only be pushed and watched, never tested. Every widened loop is **mutation-controlled**: deleting its new branch fails a named, bounded set of cases. Counts unchanged. Full narrative in `CHANGELOG.md`.
...
The repo is a **marketplace wrapper** containing four sibling plugins (loomwright, stackpack, mysql-mcp, selvedge):
